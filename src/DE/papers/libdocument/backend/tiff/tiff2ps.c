...
static void PSHead (TIFF2PSContext *, TIFF *, uint32_t, uint32_t, double, double, double, double);
static void PSTail (TIFF2PSContext *);

#if defined(EXP_ASCII85ENCODER)
static int Ascii85EncodeBlock (TIFF2PSContext *, uint8_t *ascii85_p, unsigned f_eod, const uint8_t *raw_p, int raw_l);
#endif

#define IMAGEOP(ctx) ((ctx)->useImagemask && ((ctx)->bitspersample == 1)) ? "imagemask" : "image"

TIFF2PSContext *
tiff2ps_context_new (const gchar *filename)
{
	TIFF2PSContext *ctx;
...
void
tiff2ps_context_finalize (TIFF2PSContext *ctx)
{
	PSTail (ctx);
	fclose (ctx->fd);
	g_free (ctx->filename);
...
checkImage (TIFF2PSContext *ctx, TIFF *tif)
{
	...
	case PHOTOMETRIC_LOGLUV:
		...
		ctx->photometric = (ctx->photometric == PHOTOMETRIC_LOGL) ? PHOTOMETRIC_MINISBLACK : PHOTOMETRIC_RGB;
		ctx->bitspersample = 8;
		break;
...
G_GNUC_PRINTF (6, 7)
static void
PhotoshopBanner (TIFF2PSContext *ctx, uint32_t w, uint32_t h, int bs, int nc, const char *startline, ...)
{
	va_list args;
 ...
 */
static void
setupPageState (TIFF2PSContext *ctx, TIFF *tif, uint32_t *pw, uint32_t *ph, double *pprw, double *pprh)
{
 ...
 */
static int
PlaceImage (TIFF2PSContext *ctx, double pagewidth, double pageheight, double imagewidth, double imageheight, int splitpage, double lm, double bm, int cnt)
{
	double xtran = 0;
	double ytran = 0;
	double xscale = 1;
...
void
tiff2ps_process_page (TIFF2PSContext *ctx, TIFF *tif, double pw, double ph, double lm, double bm, gboolean cnt)
{
	uint32_t w, h;
	float ox, oy;
	double prw, prh;
...
void
PSHead (TIFF2PSContext *ctx, TIFF *tif, uint32_t w, uint32_t h, double pw, double ph, double ox, double oy)
{
	time_t t;
...
static int
checkcmap (TIFF2PSContext *ctx, TIFF *tif, int n, uint16_t *r, uint16_t *g, uint16_t *b)
{
	(void) tif;
	while (n-- > 0)
...
int
Ascii85EncodeBlock (TIFF2PSContext *ctx, uint8_t *ascii85_p, unsigned f_eod, const uint8_t *raw_p, int raw_l)
