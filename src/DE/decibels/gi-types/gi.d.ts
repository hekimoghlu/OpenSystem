declare module "gi://Adw" {
    export * as default from "adw1";
}

declare module "gi://AppStream" {
    export * as default from "appstream1";
}

declare module "gi://AppStreamCompose" {
    export * as default from "appstreamcompose1";
}

declare module "gi://Atk" {
    export * as default from "atk1";
}

declare module "gi://Atspi" {
    export * as default from "atspi2";
}

declare module "gi://cairo" {
    export * as default from "cairo1";
}

declare module "gi://CudaGst" {
    export * as default from "cudagst1";
}

declare module "gi://DBus" {
    export * as default from "dbus1";
}

declare module "gi://DBusGLib" {
    export * as default from "dbusglib1";
}

declare module "gi://Dex" {
    export * as default from "dex1";
}

declare module "gi://fontconfig" {
    export * as default from "fontconfig2";
}

declare module "gi://freetype2" {
    export * as default from "freetype22";
}

declare module "gi://GCab" {
    export * as default from "gcab1";
}

declare module "gi://Gck?version=1" {
    export * as default from "gck1";
}

declare module "gi://Gcr?version=3" {
    export * as default from "gcr3";
}

declare module "gi://GDesktopEnums" {
    export * as default from "gdesktopenums3";
}

declare module "gi://Gdk?version=3.0" {
    export * as default from "gdk3";
}

declare module "gi://GdkPixbuf" {
    export * as default from "gdkpixbuf2";
}

declare module "gi://GdkPixdata" {
    export * as default from "gdkpixdata2";
}

declare module "gi://GdkWayland" {
    export * as default from "gdkwayland4";
}

declare module "gi://GdkX11?version=3.0" {
    export * as default from "gdkx113";
}

declare module "gi://Gee" {
    export * as default from "gee0";
}

declare module "gi://Geoclue" {
    export * as default from "geoclue2";
}

declare module "gi://Gio" {
    export * as default from "gio2";
}

declare module "gi://GioUnix" {
    export * as default from "giounix2";
}

declare module "gi://GIRepository?version=2.0" {
    export * as default from "girepository2";
}

declare module "gi://GL" {
    export * as default from "gl1";
}

declare module "gi://GLib" {
    export * as default from "glib2";
}

declare module "gi://GLibUnix" {
    export * as default from "glibunix2";
}

declare module "gi://GModule" {
    export * as default from "gmodule2";
}

declare module "gi://GObject" {
    export * as default from "gobject2";
}

declare module "gi://Graphene" {
    export * as default from "graphene1";
}

declare module "gi://Gsk" {
    export * as default from "gsk4";
}

declare module "gi://Gst" {
    export * as default from "gst1";
}

declare module "gi://GstAllocators" {
    export * as default from "gstallocators1";
}

declare module "gi://GstAnalytics" {
    export * as default from "gstanalytics1";
}

declare module "gi://GstApp" {
    export * as default from "gstapp1";
}

declare module "gi://GstAudio" {
    export * as default from "gstaudio1";
}

declare module "gi://GstBadAudio" {
    export * as default from "gstbadaudio1";
}

declare module "gi://GstBase" {
    export * as default from "gstbase1";
}

declare module "gi://GstCheck" {
    export * as default from "gstcheck1";
}

declare module "gi://GstCodecs" {
    export * as default from "gstcodecs1";
}

declare module "gi://GstController" {
    export * as default from "gstcontroller1";
}

declare module "gi://GstCuda" {
    export * as default from "gstcuda1";
}

declare module "gi://GstDxva" {
    export * as default from "gstdxva1";
}

declare module "gi://GstGL" {
    export * as default from "gstgl1";
}

declare module "gi://GstGLEGL" {
    export * as default from "gstglegl1";
}

declare module "gi://GstGLWayland" {
    export * as default from "gstglwayland1";
}

declare module "gi://GstGLX11" {
    export * as default from "gstglx111";
}

declare module "gi://GstInsertBin" {
    export * as default from "gstinsertbin1";
}

declare module "gi://GstMpegts" {
    export * as default from "gstmpegts1";
}

declare module "gi://GstMse" {
    export * as default from "gstmse1";
}

declare module "gi://GstNet" {
    export * as default from "gstnet1";
}

declare module "gi://GstPbutils" {
    export * as default from "gstpbutils1";
}

declare module "gi://GstPlay" {
    export * as default from "gstplay1";
}

declare module "gi://GstPlayer" {
    export * as default from "gstplayer1";
}

declare module "gi://GstRtp" {
    export * as default from "gstrtp1";
}

declare module "gi://GstRtsp" {
    export * as default from "gstrtsp1";
}

declare module "gi://GstSdp" {
    export * as default from "gstsdp1";
}

declare module "gi://GstTag" {
    export * as default from "gsttag1";
}

declare module "gi://GstTranscoder" {
    export * as default from "gsttranscoder1";
}

declare module "gi://GstVa" {
    export * as default from "gstva1";
}

declare module "gi://GstVideo" {
    export * as default from "gstvideo1";
}

declare module "gi://GstVulkan" {
    export * as default from "gstvulkan1";
}

declare module "gi://GstVulkanWayland" {
    export * as default from "gstvulkanwayland1";
}

declare module "gi://GstVulkanXCB" {
    export * as default from "gstvulkanxcb1";
}

declare module "gi://GstWebRTC" {
    export * as default from "gstwebrtc1";
}

declare module "gi://Gtk?version=3.0" {
    export * as default from "gtk3";
}

declare module "gi://GUdev" {
    export * as default from "gudev1";
}

declare module "gi://HarfBuzz" {
    export * as default from "harfbuzz2";
}

declare module "gi://IBus" {
    export * as default from "ibus1";
}

declare module "gi://JavaScriptCore?version=4.1" {
    export * as default from "javascriptcore4";
}

declare module "gi://Json" {
    export * as default from "json1";
}

declare module "gi://Libproxy" {
    export * as default from "libproxy1";
}

declare module "gi://libxml2" {
    export * as default from "libxml22";
}

declare module "gi://Manette" {
    export * as default from "manette0";
}

declare module "gi://Nice" {
    export * as default from "nice0";
}

declare module "gi://Notify" {
    export * as default from "notify0";
}

declare module "gi://Pango" {
    export * as default from "pango1";
}

declare module "gi://PangoCairo" {
    export * as default from "pangocairo1";
}

declare module "gi://PangoFc" {
    export * as default from "pangofc1";
}

declare module "gi://PangoFT2" {
    export * as default from "pangoft21";
}

declare module "gi://PangoOT" {
    export * as default from "pangoot1";
}

declare module "gi://Polkit" {
    export * as default from "polkit1";
}

declare module "gi://Rsvg" {
    export * as default from "rsvg2";
}

declare module "gi://Secret" {
    export * as default from "secret1";
}

declare module "gi://Soup" {
    export * as default from "soup3";
}

declare module "gi://Tracker" {
    export * as default from "tracker3";
}

declare module "gi://Tsparql" {
    export * as default from "tsparql3";
}

declare module "gi://Vulkan" {
    export * as default from "vulkan1";
}

declare module "gi://win32" {
    export * as default from "win321";
}

declare module "gi://xfixes" {
    export * as default from "xfixes4";
}

declare module "gi://xft" {
    export * as default from "xft2";
}

declare module "gi://xlib" {
    export * as default from "xlib2";
}

declare module "gi://Xmlb" {
    export * as default from "xmlb2";
}

declare module "gi://xrandr" {
    export * as default from "xrandr1";
}

declare module "gi://Gck?version=2" {
    export * as default from "gck2";
}

declare module "gi://Gcr?version=4" {
    export * as default from "gcr4";
}

declare module "gi://GdkX11?version=4.0" {
    export * as default from "gdkx114";
}

declare module "gi://Gdk?version=4.0" {
    export * as default from "gdk4";
}

declare module "gi://GIRepository?version=3.0" {
    export * as default from "girepository3";
}

declare module "gi://JavaScriptCore?version=6.0" {
    export * as default from "javascriptcore6";
}

declare module "gi://Gtk?version=4.0" {
    export * as default from "gtk4";
}
