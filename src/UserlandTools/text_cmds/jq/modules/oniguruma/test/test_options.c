...
extern int main(int argc, char* argv[])
{
  ...
  n(ONIG_OPTION_WORD_IS_ASCII, "\\w", "ã‚");
  x2(ONIG_OPTION_NONE, "\\d", "ï¼‘", 0, 3);
  n(ONIG_OPTION_DIGIT_IS_ASCII, "\\d", "ï¼‘");
  x2(ONIG_OPTION_SPACE_IS_ASCII, "\\s", " ", 0, 1);
  x2(ONIG_OPTION_NONE, "\\s", "ã€€", 0, 3);
  n(ONIG_OPTION_SPACE_IS_ASCII, "\\s", "ã€€");

  x2(ONIG_OPTION_POSIX_IS_ASCII, "\\w\\d\\s", "c3 ", 0, 3);
  n(ONIG_OPTION_POSIX_IS_ASCII, "\\w|\\d|\\s", "ã‚ï¼”ã€€");
