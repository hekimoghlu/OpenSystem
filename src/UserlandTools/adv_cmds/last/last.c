...
#ifdef __APPLE__
	setutxent_wtmp(1);      /* one means chronological order */
...
printentry(struct utmpx *bp, struct idtab *tt)
{
	...
	case BOOT_TIME:
#ifdef __APPLE__
		xo_emit("{:user/%-42s/%s}", "reboot time");
#else
...
ttyconv(char *arg)
{
	...
	if (strlen(arg) == 2) {
		...
		if (!strcmp(arg, "co"))
#ifdef __APPLE__
			(void)strlcpy(mval, "console", 8);
#else
			(void)strcpy(mval, "console");
#endif	/* __APPLE__ */
		else {
#ifdef __APPLE__
			(void)strlcpy(mval, "tty", 4);
			(void)strlcpy(mval + 3, arg, 3);
#else
