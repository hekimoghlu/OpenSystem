...
#ifdef __APPLE__
	    "usage: sysctl [-bdeFhilNnoqtWx] [ -B <bufsize> ] [-f filename] name[=value] ...",
	    "       sysctl [-bdeFhlNnoqtWx] [ -B <bufsize> ] -a");
#else
...
main(int argc, char **argv)
{
...
#ifdef __APPLE__
	while ((ch = getopt(argc, argv, "AabB:def:FhilNnoqtwWxX")) != -1) {
#else
	while ((ch = getopt(argc, argv, "AabB:def:FhilNnoqtTwWxX")) != -1) {
		...
		case 'B':
#ifdef __APPLE__
			Bflag = (int)strtol(optarg, NULL, 0);
#else
...
#ifdef __APPLE__
			errx(1, "invalid value '%s'", newvalstr);
#else
...
#ifdef __APPLE__
static int
S_xswusage(size_t l2, void *p)
{
	struct xsw_usage *xsu = (struct xsw_usage *)p;

	if (l2 != sizeof(*xsu)) {
		warnx("S_xswusage %ld != %ld", l2, sizeof(*xsu));
		return (1);
	}
	fprintf(stdout,
...
T_dev_t(size_t l2, void *p)
{
	...
	if (l2 != sizeof(*d)) {
		warnx("T_dev_T %ld != %ld", l2, sizeof(*d));
		return (1);
	}
	if ((int)(*d) != -1) {
...
#ifdef __APPLE__
	if (honor_skip && (kind & CTLFLAG_MASKED)) {
		return (1);
	}
...
#ifndef __APPLE__
	/*
	 * rdar://problem/83864182 - Darwin doesn't have a perfect analog for
	 * CTLFLAG_SKIP.  CTLFLAG_MASKED is close, except it's historically
	 * applied to leaf nodes and CTLFLAG_SKIP may be applied on non-leaf
	 * nodes to avoid their children as well.
	 */
...
#ifdef __APPLE__
				/*
				 * rdar://problem/9683086 - suppress warnings
				 * with a round trip through `void *`.
				 */
				umv = *(u_int *)(void *)p;
				mv = *(int *)(void *)p;
#else
...
#ifndef __APPLE__
	/* rdar://problem/77205344 - UNREACHABLE */
	free(oval);
#endif
	return (1);
}
...
#ifdef __APPLE__
		i = show_var(name2, (u_int)l2, true);
#else
