...
printcol(const char *s, size_t *col, const size_t col_max)
{
	...
	for (; *s && *col < col_max; ++s) {
		...
		default:
			 ...
			 * output towards our column count.  We may still
			 * get the width wrong if the locale is set wrong
			 * and the terminal interprets it differently, but
			...
			if (width > 0)
				*col += width;
			/*
			 * Print all but the last byte, so that we don't
			 * advance one past where we're supposed to be
			 * printing and require a weird step back one
			 * byte.
			 */
			while (nbytes > 1) {
				nbytes--;
