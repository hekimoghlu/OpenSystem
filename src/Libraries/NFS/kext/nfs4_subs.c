...
nfs4_map_domain(char *id, char **atp)
{
	...
	dsnode = zalloc(get_zone(NFS_NAMEI));
	...
	NFS_ZFREE(get_zone(NFS_NAMEI), dsnode);
...
nfs4_addv4domain(char *id, size_t *idlen)
{
	...
	if (have_domain) {
		...
		nfs4domain = zalloc(get_zone(NFS_NAMEI));
		...
		NFS_ZFREE(get_zone(NFS_NAMEI), nfs4domain);
	} else if (at == NULL) {
		/*
...
nfs4_guid2id(guid_t *guidp, char *id, size_t *idlen, int isgroup)
{
	...
	if (nfs_idmap_ctrl & NFS_IDMAP_CTRL_USE_IDMAP_SERVICE) {
		...
		if (*idlen < MAXPATHLEN) {
			id1buf = zalloc(get_zone(NFS_NAMEI));
			id1 = id1buf;
			id1len = MAXPATHLEN;
		} else {
			id1 = id;
	...
	if (id1buf) {
		NFS_ZFREE(get_zone(NFS_NAMEI), id1buf);
	}
	...
	struct nfs_fs_locations *nfslsp)
{
	...
	if (NFS_BITMAP_ISSET(bitmap, NFS_FATTR_FS_LOCATIONS)) {
		...
		if (nfslsp->nl_numlocs > 0) {
			nfslsp->nl_locations = kalloc_type(struct nfs_fs_location *, nfslsp->nl_numlocs, Z_WAITOK);
			if (nfslsp->nl_locations) {
				bzero(nfslsp->nl_locations, sizeof(struct nfs_fs_location *) * nfslsp->nl_numlocs);
			} else {
				error = ENOMEM;
			}
		}
		...
		for (loc = 0; loc < nfslsp->nl_numlocs; loc++) {
			...
			for (serv = 0; serv < fsl->nl_servcount; serv++) {
				...
				if (nfs_uaddr2sockaddr(fss->ns_name, (struct sockaddr*)&ss)) {
					fss->ns_addrcount = 1;
					fss->ns_addresses = kalloc_type(char *, Z_WAITOK | Z_ZERO);
					if (!fss->ns_addresses) {
						error = ENOMEM;
					}
					nfsmout_if(error);
