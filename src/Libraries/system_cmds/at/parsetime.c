...
#ifdef __APPLE__
	/* APPLE: If minutes are the same assume tomorrow was meant
	 */
 ...
 * APPLE: |NEXT MINUTES|HOURS|DAYS|WEEKS|MONTHS|YEARS|
...
#ifdef __APPLE__
	    /* now is optional prefix for PLUS/NEXT(APPLE) tree */
	    switch (token()) {
		case PLUS:
		    plus(&runtime);
		    break;
