...
bool have_openssl(void)
{
  /* Check both libcrypto and libssl.
   *
   * NB: Don't rely on these being weak imports because the compiler can
   *     optimize them away as tautological comparisons.  It does not know
   *     that we will be linking libcrypto and libssl weakly, so we use
   *     dlsym() directly ourselves.
   */
  void *libcrypto_symbol = dlsym(RTLD_NEXT, "OPENSSL_load_builtin_modules");
  void *libssl_symbol = dlsym(RTLD_NEXT, "SSL_load_error_strings");
  return (!!libssl_symbol && !!libcrypto_symbol);
}
#endif
#endif
