...
public final class Subprocess {
  ...
  public func run() async throws -> Result {
    ...
    guard rc == 0 else {
      throw Error.osError(Errno(rawValue: rc), "posix_spawn: â€œ\(plan.command)â€")
    }
  ...
  public enum Error: Swift.Error, CustomStringConvertible {
    ...
    public var description: String {
      ...
      case .decode(let bytes, let truncated):
        ...
        msg += "]"
        return msg
      case .noOutputRequested(let cmd): return "No capturing of output was requested: â€œ\(cmd)â€"
      case .hasNotRun(let cmd): return "The command has not yet been run: â€œ\(cmd)â€"
