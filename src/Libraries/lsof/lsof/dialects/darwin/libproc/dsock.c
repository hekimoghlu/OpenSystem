...
#if	defined(AF_VSOCK)
static void
vsock_print_addr(buf, bufl, cid, port)
	char *buf;
	size_t bufl;
	uint32_t cid;
	uint32_t port;
{
	if (cid == VMADDR_CID_ANY && port == VMADDR_PORT_ANY) {
		...
		(void) snpf(buf, bufl, "*:%u", port);
	} else if (port == VMADDR_PORT_ANY) {
		(void) snpf(buf, bufl, "%u:*", cid);
	} else {
		(void) snpf(buf, bufl, "%u:%u", cid, port);
	}
}
 ...
 */
	 ...
	 */
		if (si->psi.soi_kind != SOCKINFO_VSOCK) {
			(void) snpf(Namech, Namechl, "no further information on vsock socket");
			break;
		}

		const unsigned int so_state = Lf->lts.ss;
		const int so_options = Lf->lts.opt;
	 ...
	 */
		...
		if (so_state & SOI_S_ISCONNECTING) {
			state = "CONNECTING";
		} else if (so_state & SOI_S_ISCONNECTED) {
			state = "ESTABLISHED";
		} else if (so_state & SOI_S_ISDISCONNECTING) {
			state = "CLOSING";
		} else if (so_options & SO_ACCEPTCONN) {
			state = "LISTEN";
			onlyLocal = true;
		} else {
			state = "CLOSED";
