...
extern int main(int argc, char* argv[])
{
  ...
  x2(".\\wã†\\W..ãž", "ãˆã†ã† ã†ãžãž", 0, 19);
  ...
  x2("^ã‚€$", "ã‚€", 0, 3);
  x2("^\\w$", "ã«", 0, 3);
  x2("^\\wã‹ããã‘ã“$", "zã‹ããã‘ã“", 0, 16);
  x2("^\\w...ã†ãˆãŠ$", "zã‚ã„ã†ã†ãˆãŠ", 0, 19);
  x2("\\w\\w\\s\\WãŠãŠãŠ\\d", "aãŠ  ãŠãŠãŠ4", 0, 16);
  x2("\\AãŸã¡ã¤", "ãŸã¡ã¤", 0, 9);
  ...
  x2("é‡*", "é‡", 3, 3);
  x2("å­*", "å­å­å­", 9, 9);
  x2("é¦¬*", "é¹¿é¦¬é¦¬é¦¬é¦¬", 15, 15);
  n("å±±+", "");
  x2("æ²³+", "æ²³", 0, 3);
  x2("æ™‚+", "æ™‚æ™‚æ™‚æ™‚", 9, 12);
  x2("ãˆ+", "ãˆãˆã†ã†ã†", 3, 6);
  x2("ã†+", "ãŠã†ã†ã†ã†", 12, 15);
  x2(".?", "ãŸ", 3, 3);
  x2(".*", "ã±ã´ã·ãº", 12, 12);
  x2(".+", "ã‚", 0, 3);
  x2(".+", "ã„ã†ãˆã‹\n", 9, 12);
  ...
  x2("ã‚ã„|ã„ã†", "ã‚ã„", 0, 6);
  x2("ã‚ã„|ã„ã†", "ã„ã†", 0, 6);
  x2("ã‚’(?:ã‹ã|ãã)", "ã‚’ã‹ã", 0, 9);
  x2("ã‚’(?:ã‹ã|ãã)ã‘", "ã‚’ããã‘", 0, 12);
  x2("ã‚ã„|(?:ã‚ã†|ã‚ã‚’)", "ã‚ã‚’", 0, 6);
  x2("ã‚|ã„|ã†", "ãˆã†", 3, 6);
  x2("ã‚|ã„|ã†ãˆ|ãŠã‹ã|ã|ã‘ã“ã•|ã—ã™ã›|ã|ãŸã¡|ã¤ã¦ã¨ãªã«|ã¬ã­", "ã—ã™ã›", 0, 9);
  n("ã‚|ã„|ã†ãˆ|ãŠã‹ã|ã|ã‘ã“ã•|ã—ã™ã›|ã|ãŸã¡|ã¤ã¦ã¨ãªã«|ã¬ã­", "ã™ã›");
  x2("ã‚|^ã‚", "ã¶ã‚", 3, 6);
  x2("ã‚|^ã‚’", "ã‚’ã‚", 3, 6);
  x2("é¬¼|\\Gè»Š", "ã‘è»Šé¬¼", 6, 9);
  x2("é¬¼|\\Gè»Š", "è»Šé¬¼", 3, 6);
  x2("é¬¼|\\Aè»Š", "bè»Šé¬¼", 4, 7);
  x2("é¬¼|\\Aè»Š", "è»Š", 0, 3);
  x2("é¬¼|è»Š\\Z", "è»Šé¬¼", 3, 6);
  ...
  n("(?>ã‚|ã‚ã„ãˆ)ã†", "ã‚ã„ãˆã†");
  x2("(?>ã‚ã„ãˆ|ã‚)ã†", "ã‚ã„ãˆã†", 0, 12);
  x2("ã‚?|ã„", "ã‚", 3, 3);
  x2("ã‚?|ã„", "ã„", 3, 3);
  x2("ã‚?|ã„", "", 0, 0);
  x2("ã‚*|ã„", "ã‚ã‚", 6, 6);
  x2("ã‚*|ã„*", "ã„ã‚", 6, 6);
  x2("ã‚*|ã„*", "ã‚ã„", 6, 6);
  x2("[aã‚]*|ã„*", "aã‚ã„ã„ã„", 13, 13);
  x2("ã‚+|ã„*", "", 0, 0);
  x2("ã‚+|ã„*", "ã„ã„ã„", 9, 9);
  x2("ã‚+|ã„*", "ã‚ã„ã„ã„", 12, 12);
  x2("ã‚+|ã„*", "aã‚ã„ã„ã„", 13, 13);
  n("ã‚+|ã„+", "");
  x2("(ã‚|ã„)?", "ã„", 3, 3);
  x2("(ã‚|ã„)*", "ã„ã‚", 6, 6);
  x2("(ã‚|ã„)+", "ã„ã‚ã„", 6, 9);
  x2("(ã‚ã„|ã†ã‚)+", "ã†ã‚ã‚ã„ã†ãˆ", 6, 12);
  x2("(ã‚ã„|ã†ãˆ)+", "ã†ã‚ã‚ã„ã†ãˆ", 12, 18);
  x2("(ã‚ã„|ã†ã‚)+", "ã‚ã‚ã„ã†ã‚", 9, 15);
  x2("(ã‚ã„|ã†ã‚)+", "ã‚ã„ã‚’ã†ã‚", 9, 15);
  x2("(ã‚ã„|ã†ã‚)+", "$$zzzzã‚ã„ã‚’ã†ã‚", 15, 21);
  x2("(ã‚|ã„ã‚ã„)+", "ã‚ã„ã‚ã„ã‚", 12, 15);
  x2("(ã‚|ã„ã‚ã„)+", "ã„ã‚", 3, 6);
  x2("(ã‚|ã„ã‚ã„)+", "ã„ã‚ã‚ã‚ã„ã‚", 15, 18);
  x2("(?:ã‚|ã„)(?:ã‚|ã„)", "ã‚ã„", 0, 6);
  x2("(?:ã‚*|ã„*)(?:ã‚*|ã„*)", "ã‚ã‚ã‚ã„ã„ã„", 18, 18);
  x2("(?:ã‚*|ã„*)(?:ã‚+|ã„+)", "ã‚ã‚ã‚ã„ã„ã„", 15, 18);
  x2("(?:ã‚+|ã„+){2}", "ã‚ã‚ã‚ã„ã„ã„", 12, 18);
  x2("(?:ã‚+|ã„+){1,2}", "ã‚ã‚ã‚ã„ã„ã„", 15, 18);
  x2("(?:ã‚+|\\Aã„*)ã†ã†", "ã†ã†", 0, 6);
  n("(?:ã‚+|\\Aã„*)ã†ã†", "ã‚ã„ã†ã†");
  x2("(?:^ã‚+|ã„+)*ã†", "ã‚ã‚ã„ã„ã„ã‚ã„ã†", 21, 24);
  x2("(?:^ã‚+|ã„+)*ã†", "ã‚ã‚ã„ã„ã„ã„ã†", 18, 21);
  x2("ã†{0,}", "ã†ã†ã†ã†", 12, 12);
  x2("ã‚|(?i)c", "C", 0, 1);
  x2("(?i)c|ã‚", "C", 0, 1);
  x2("(?i:ã‚)|a", "a", 0, 1);
  n("(?i:ã‚)|a", "A");
  ...
  x2("(?:å¤¢?)+?", "å¤¢å¤¢å¤¢", 9, 9);
  x2("(?:é¢¨+)?\?", "é¢¨é¢¨é¢¨", 9, 9);
  x2("(?:é›ª+)?\?éœœ", "é›ªé›ªé›ªéœœ", 9, 12);
  x2("(?:ã‚ã„)?{2}", "", 0, 0);
  x2("(?:é¬¼è»Š)?{2}", "é¬¼è»Šé¬¼è»Šé¬¼", 15, 15);
  x2("(?:é¬¼è»Š)*{0}", "é¬¼è»Šé¬¼è»Šé¬¼", 15, 15);
  x2("(?:é¬¼è»Š){3,}", "é¬¼è»Šé¬¼è»Šé¬¼è»Šé¬¼è»Š", 6, 24);
  n("(?:é¬¼è»Š){3,}", "é¬¼è»Šé¬¼è»Š");
  x2("(?:é¬¼è»Š){2,4}", "é¬¼è»Šé¬¼è»Šé¬¼è»Š", 6, 18);
  x2("(?:é¬¼è»Š){2,4}", "é¬¼è»Šé¬¼è»Šé¬¼è»Šé¬¼è»Šé¬¼è»Š", 18, 30);
  x2("(?:é¬¼è»Š){2,4}?", "é¬¼è»Šé¬¼è»Šé¬¼è»Šé¬¼è»Šé¬¼è»Š", 18, 30);
  x2("(?:é¬¼è»Š){,}", "é¬¼è»Š{,}", 0, 9);
  x2("(?:ã‹ãã)+?{2}", "ã‹ããã‹ããã‹ãã", 9, 27);
  x3("(ç«)", "ç«", 0, 3, 1);
  x3("(ç«æ°´)", "ç«æ°´", 0, 6, 1);
  x2("((æ™‚é–“))", "æ™‚é–“", 0, 6);
  x3("((é¢¨æ°´))", "é¢¨æ°´", 0, 6, 1);
  x3("((æ˜¨æ—¥))", "æ˜¨æ—¥", 0, 6, 2);
  x3("((((((((((((((((((((é‡å­))))))))))))))))))))", "é‡å­", 0, 6, 20);
  x3("(ã‚ã„)(ã†ãˆ)", "ã‚ã„ã†ãˆ", 0, 6, 1);
  x3("(ã‚ã„)(ã†ãˆ)", "ã‚ã„ã†ãˆ", 6, 12, 2);
  x3("()(ã‚)ã„ã†(ãˆãŠã‹)ããã‘ã“", "ã‚ã„ã†ãˆãŠã‹ããã‘ã“", 9, 18, 3);
  x3("(()(ã‚)ã„ã†(ãˆãŠã‹)ããã‘ã“)", "ã‚ã„ã†ãˆãŠã‹ããã‘ã“", 9, 18, 4);
  x3(".*(ãƒ•ã‚©)ãƒ³ãƒ»ãƒž(ãƒ³()ã‚·ãƒ¥ã‚¿)ã‚¤ãƒ³", "ãƒ•ã‚©ãƒ³ãƒ»ãƒžãƒ³ã‚·ãƒ¥ã‚¿ã‚¤ãƒ³", 15, 27, 2);
  x2("(^ã‚)", "ã‚", 0, 3);
  x3("(ã‚)|(ã‚)", "ã„ã‚", 3, 6, 1);
  x3("(^ã‚)|(ã‚)", "ã„ã‚", 3, 6, 2);
  x3("(ã‚?)", "ã‚ã‚ã‚", 9, 9, 1);
  x3("(ã¾*)", "ã¾ã¾ã¾", 9, 9, 1);
  x3("(ã¨*)", "", 0, 0, 1);
  x3("(ã‚‹+)", "ã‚‹ã‚‹ã‚‹ã‚‹ã‚‹ã‚‹ã‚‹", 18, 21, 1);
  x3("(ãµ+|ã¸*)", "ãµãµãµã¸ã¸", 15, 15, 1);
  x3("(ã‚+|ã„?)", "ã„ã„ã„ã‚ã‚", 15, 15, 1);
  x3("(ã‚ã„ã†)?", "ã‚ã„ã†", -1, -1, 1);
  x3("(ã‚ã„ã†)*", "ã‚ã„ã†", -1, -1, 1);
  x3("(ã‚ã„ã†)+", "ã‚ã„ã†", 0, 9, 1);
  x3("(ã•ã—ã™|ã‚ã„ã†)+", "ã‚ã„ã†", 0, 9, 1);
  x3("([ãªã«ã¬][ã‹ãã]|ã‹ãã)+", "ã‹ãã", 0, 9, 1);
  x3("((?i:ã‚ã„ã†))", "ã‚ã„ã†", 0, 9, 1);
  x3("((?m:ã‚.ã†))", "ã‚\nã†", 0, 7, 1);
  x3("((?=ã‚ã‚“)ã‚)", "ã‚ã‚“ã„", 0, 3, 1);
  x3("ã‚ã„ã†|(.ã‚ã„ãˆ)", "ã‚“ã‚ã„ãˆ", 0, 12, 1);
  x3("ã‚*(.)", "ã‚ã‚ã‚ã‚ã‚“", 12, 15, 1);
  x3("ã‚*?(.)", "ã‚ã‚ã‚ã‚ã‚“", 12, 15, 1);
  x3("ã‚*?(ã‚“)", "ã‚ã‚ã‚ã‚ã‚“", 12, 15, 1);
  x3("[ã„ã†ãˆ]ã‚*(.)", "ãˆã‚ã‚ã‚ã‚ã‚“", 15, 18, 1);
  ...
  x2("(ç©º?)\\1", "ç©ºç©º", 6, 6);
  x2("(ç©º?\?)\\1", "ç©ºç©º", 6, 6);
  x2("(ç©º*)\\1", "ç©ºç©ºç©ºç©ºç©º", 15, 15);
  x3("(ç©º*)\\1", "ç©ºç©ºç©ºç©ºç©º", 15, 15, 1);
  x2("ã‚(ã„*)\\1", "ã‚ã„ã„ã„ã„", 0, 15);
  x2("ã‚(ã„*)\\1", "ã‚ã„", 0, 3);
  x2("(ã‚*)(ã„*)\\1\\2", "ã‚ã‚ã‚ã„ã„ã‚ã‚ã‚ã„ã„", 30, 30);
  x2("(ã‚*)(ã„*)\\2", "ã‚ã‚ã‚ã„ã„ã„ã„", 21, 21);
  x3("(ã‚*)(ã„*)\\2", "ã‚ã‚ã‚ã„ã„ã„ã„", 21, 21, 2);
  x2("(((((((ã½*)ãº))))))ã´\\7", "ã½ã½ã½ãºã´ã½ã½ã½", 9, 15);
  x3("(((((((ã½*)ãº))))))ã´\\7", "ã½ã½ã½ãºã´ã½ã½ã½", 9, 9, 7);
  x2("(ã¯)(ã²)(ãµ)\\2\\1\\3", "ã¯ã²ãµã²ã¯ãµ", 0, 18);
  x2("([ã-ã‘])\\1", "ãã", 0, 6);
  x2("(\\w\\d\\s)\\1", "ã‚5 ã‚5 ", 0, 10);
  n("(\\w\\d\\s)\\1", "ã‚5 ã‚5");
  x2("(èª°ï¼Ÿ|[ã‚-ã†]{3})\\1", "èª°ï¼Ÿèª°ï¼Ÿ", 0, 12);
  x2("...(èª°ï¼Ÿ|[ã‚-ã†]{3})\\1", "ã‚aã‚èª°ï¼Ÿèª°ï¼Ÿ", 0, 19);
  x2("(èª°ï¼Ÿ|[ã‚-ã†]{3})\\1", "ã†ã„ã†ã†ã„ã†", 0, 18);
  x2("(^ã“)\\1", "ã“ã“", 0, 6);
  n("(^ã‚€)\\1", "ã‚ã‚€ã‚€");
  n("(ã‚$)\\1", "ã‚ã‚");
  n("(ã‚ã„\\Z)\\1", "ã‚ã„");
  x2("(ã‚*\\Z)\\1", "ã‚", 3, 3);
  x2(".(ã‚*\\Z)\\1", "ã„ã‚", 3, 6);
  x3("(.(ã‚„ã„ã‚†)\\2)", "zã‚„ã„ã‚†ã‚„ã„ã‚†", 0, 19, 1);
  x3("(.(..\\d.)\\2)", "ã‚12341234", 0, 11, 1);
  x2("((?i:ã‚vãš))\\1", "ã‚vãšã‚vãš", 0, 14);
  x2("(?<æ„šã‹>å¤‰|\\(\\g<æ„šã‹>\\))", "((((((å¤‰))))))", 6, 9);
  x2("\\A(?:\\g<é˜¿_1>|\\g<äº‘_2>|\\zçµ‚äº†  (?<é˜¿_1>è¦³|è‡ª\\g<äº‘_2>è‡ª)(?<äº‘_2>åœ¨|è©è–©\\g<é˜¿_1>è©è–©))$", "è©è–©è‡ªè©è–©è‡ªåœ¨è‡ªè©è–©è‡ªè©è–©", 0, 39);
  x2("[[ã²ãµ]]", "ãµ", 0, 3);
  x2("[[ã„ãŠã†]ã‹]", "ã‹", 0, 3);
  n("[[^ã‚]]", "ã‚");
  n("[^[ã‚]]", "ã‚");
  ...
  x2("[[^ã‚&&ã‚]&&ã‚-ã‚“]", "ã„", 0, 3);
  n("[[^ã‚&&ã‚]&&ã‚-ã‚“]", "ã‚");
  x2("[[^ã‚-ã‚“&&ã„ã†ãˆãŠ]&&[^ã†-ã‹]]", "ã", 0, 3);
  n("[[^ã‚-ã‚“&&ã„ã†ãˆãŠ]&&[^ã†-ã‹]]", "ã„");
  x2("[^[^ã‚ã„ã†]&&[^ã†ãˆãŠ]]", "ã†", 0, 3);
  x2("[^[^ã‚ã„ã†]&&[^ã†ãˆãŠ]]", "ãˆ", 0, 3);
  n("[^[^ã‚ã„ã†]&&[^ã†ãˆãŠ]]", "ã‹");
  x2("[ã‚-&&-ã‚]", "-", 0, 1);
  x2("[^[^a-zã‚ã„ã†]&&[^bcdefgã†ãˆãŠ]q-w]", "ãˆ", 0, 3);
  x2("[^[^a-zã‚ã„ã†]&&[^bcdefgã†ãˆãŠ]g-w]", "f", 0, 1);
  x2("[^[^a-zã‚ã„ã†]&&[^bcdefgã†ãˆãŠ]g-w]", "g", 0, 1);
  n("[^[^a-zã‚ã„ã†]&&[^bcdefgã†ãˆãŠ]g-w]", "2");
  x2("a<b>ãƒãƒ¼ã‚¸ãƒ§ãƒ³ã®ãƒ€ã‚¦ãƒ³ãƒ­ãƒ¼ãƒ‰<\\/b>", "a<b>ãƒãƒ¼ã‚¸ãƒ§ãƒ³ã®ãƒ€ã‚¦ãƒ³ãƒ­ãƒ¼ãƒ‰</b>", 0, 44);
  x2(".<b>ãƒãƒ¼ã‚¸ãƒ§ãƒ³ã®ãƒ€ã‚¦ãƒ³ãƒ­ãƒ¼ãƒ‰<\\/b>", "a<b>ãƒãƒ¼ã‚¸ãƒ§ãƒ³ã®ãƒ€ã‚¦ãƒ³ãƒ­ãƒ¼ãƒ‰</b>", 0, 44);
  x2("\\n?\\z", "ã“ã‚“ã«ã¡ã¯", 15, 15);
  x2("(?m).*", "é’èµ¤é»„", 9, 9);
  x2("(?m).*a", "é’èµ¤é»„a", 9, 10);
  ...
  x2("(?y{w})\\y\\X\\y", "ã‚±ãƒ³", 0, 6); // WB13
  x2("(?y{w})\\y\\X\\y", "ã‚±ãƒ³\xE2\x80\xAFã‚¿", 0, 12); // WB13a, WB13b
  x2("(?y{w})\\y\\X\\y", "\x21\x23", 1, 2); // WB999
  x2("(?y{w})\\y\\X\\y", "å±±ã‚¢", 3, 6);
  x2("(?y{w})\\y\\X", "3.14", 0, 4);
  x2("(?y{w})\\y\\X", "3 14", 2, 4);
  ...
  x2("(?i).*ã‚stã„\\z", "tttssssã‚\xC5\xBFtã„", 7, 16); // U+017F
  x2("(?i).*ã‚stã„\\z", "tttssssã‚\xEF\xAC\x85ã„", 7, 16); // U+FB05
  x2("(?i).*ã‚stã„\\z", "tttssssã‚\xEF\xAC\x86ã„", 7, 16); // U+FB06
  x2("(?i).*\xC5\xBFt\\z", "tttssssst", 7, 9); // U+017F
  x2("(?i).*\xEF\xAC\x85\\z", "tttssssã‚st", 10, 12); // U+FB05
  x2("(?i).*\xEF\xAC\x86ã„\\z", "tttssssstã„", 7, 12); // U+FB06
  x2("(?i).*\xEF\xAC\x85\\z", "tttssssã‚\xEF\xAC\x85", 10, 13);
