...
#ifdef __APPLE__
	/* rdar://problem/5900695: -A0 should not be treated as -A1 */
	if (after != 0 && friday != -1) {
		int weeks = after / 5;
		int days = after % 5;

		/*
		 * rdar://problem/5900695: -A should take into account any
		 * weekends during, e.g., -A 10.
		 */
		after += 2 * weeks;
		if (days && tp.tm_wday <= friday &&
