...
#ifdef __APPLE__
void
scan_enqueue(const char *line, size_t len)
{

	/*
	 * XXX This doesn't do anything to make the line numbers look even
	 * remotely sane, but we just assume for now that `locale -k` will
	 * produce valid output.
	 */
	for (char *ep = &line[len - 1]; ep >= line; ep--) {
		unscanc((unsigned char)*ep);
	}
}
#endif
...
scan_done(void)
{
	...
	/*
	 * We must provide a default LC_CTYPE if one hasn't been provided; most
	 * of that is taken care of in init_ctype(), so we really just need to
	 * write it out if we haven't yet.
	 */
	if (!ctype_dumped && !bsd) {
		category = T_CTYPE;
...
consume_token(void)
{
	...
	/* perhaps it's encoded constant? */
		...
		/* Octal if unspecified. */
		base = 8;

		/*
		 * Skip the backslash, as well as any prefix that we might be
		 * processing here.
		 */
		token++;
		if (*token == 'd') {
		...
		if (*token != '\0') {
			/*
			 * If we just had \d, \x, or \, then we shouldn't eat
			 * this as a T_CHAR.
			 */
			num = strtol(token, &eptr, base);
			if (*eptr == '\0' && num >= 0 && num <= WCHAR_MAX) {
				yylval.wc = (wchar_t)num;
				return (T_CHAR);
			}
		}

		/* Return to the original position; probably a symbolic name. */
		token = otoken;
#endif
	}
