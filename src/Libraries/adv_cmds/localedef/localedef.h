...
#ifdef __APPLE__
void scan_done(void);
void scan_enqueue(const char *, size_t);
#endif
...
#ifdef __APPLE__
void werr(const char *, ...) __printflike(1, 2);
#else
...
#ifdef __APPLE__
/* Match Illumos' port and make internal faults fatal. */
#define	INTERR	errf("internal fault (%s:%d)\n", __FILE__, __LINE__)
#else
