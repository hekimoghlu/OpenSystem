		  ...
		  pk_client_params **ret_params)
{
    ...
    heim_array_t signer_certs = NULL;
    ...
    *ret_params = NULL;
	...
	krb5_set_error_message(context, ret, "failed to merge trust anchors");
	goto out;
    }
	...
	/* if not anonymous, pick up certs */
	if (signer_certs) {
	    ret = KRB5_KDC_ERR_CANT_VERIFY_CERTIFICATE;

	    len = heim_array_get_length(signer_certs);
	    if (len == 0)
		goto out;

	    eval = heim_array_copy_value(signer_certs, 0);
	    if (eval == NULL)
		goto out;

	    len = hx509_evaluate_get_length(eval);
	    if (len == 0) {
		hx509_evaluate_free(eval);
		goto out;
	    }

	    cp->cert = hx509_evaluate_get_cert(eval, 0);
	    if (cp->cert == NULL) {
		hx509_evaluate_free(eval);
		goto out;
	    }

	    cp->tacert = hx509_evaluate_get_ta(eval);
	    hx509_evaluate_free(eval);
	    if (cp->tacert == NULL)
		goto out;
	}
...
static krb5_error_code
create_fxkey(krb5_context context,
	     METHOD_DATA *md,
	     krb5_keyblock *reply_key,
	     ...
	     krb5_keyblock *sessionkey)
{
    ...
    krb5_data pepper1, pepper2, data;
    EncryptedData encdata;
    krb5_error_code ret;
    krb5_keyblock kxkey;
    size_t size = 0;

    ret = krb5_generate_random_keyblock(context, sessionetype, &kxkey);
    if (ret)
	return ret;

    ret = krb5_crypto_init(context, reply_key, 0, &reply_crypto);
    if (ret)
	goto out;
 ...
 */pepper2.data = "KEYEXCHANGE";
    ...
    ret = krb5_crypto_fx_cf2(context, kxkey_crypto, reply_crypto,
			     &pepper1, &pepper2, sessionetype,
			     sessionkey);
    if (ret)
	goto out;

    ASN1_MALLOC_ENCODE(EncryptionKey, data.data, data.length, &kxkey, &size, ret);
    if (ret)
	goto out;
    if(data.length != size)
	krb5_abortx(context, "internal error in ASN.1 encoder");
    ...
    ASN1_MALLOC_ENCODE(EncryptedData, data.data, data.length, &encdata, &size, ret);
    ...
    ret = krb5_padata_add(context, md, KRB5_PADATA_PKINIT_KX, data.data, data.length);
    if (ret)
	free(data.data);

 out:
    if (ret)
	krb5_set_error_message(context, ret, "Failed adding PKINIT_KX %d padata", ret);
...
krb5_error_code
_kdc_pk_mk_pa_reply(kdc_request_t r, pk_client_params *cp)
{
	...
	if (cp->keyex == USE_RSA) {
	    ...
	    ret = krb5_generate_random_keyblock(r->context, r->sessionetype,
						&r->session_key);
	    if (ret) {
		free_PA_PK_AS_REP(&rep);
		goto out;
...
#endif
	    default: krb5_abortx(r->context, "unknown keyex");
	    }
	    ...
	    ret = pk_mk_pa_reply_dh(r->context, r->config,
				    cp,
				    &info,
				    &kdc_cert);
	    if (ret) {
	    ...
	    ret = create_fxkey(r->context, &r->outpadata, &cp->reply_key, r->sessionetype, &r->session_key);
	    if (ret) {
		free_PA_PK_AS_REP(&rep);
		goto out;
	    }
...
#define use_btmm_with_enckey 1
	...
	kdc_log(r->context, r->config, 0, "PK-INIT using %s %s", type, other);
	...
	krb5_abortx(r->context, "PK-INIT internal error");


    ret = krb5_padata_add(r->context, &r->outpadata, pa_type, buf, len);
    if (ret) {
	krb5_set_error_message(r->context, ret,
    ...
    if (r->config->pkinit_kdc_ocsp_file) {
	...
	if (ocsp.expire == 0 && ocsp.next_update > kdc_time) {
	    ...
	    fd = open(r->config->pkinit_kdc_ocsp_file, O_RDONLY);
	    if (fd < 0) {
...
out:
    ...
    if (ret == 0)
	ret = krb5_copy_keyblock_contents(r->context, &cp->reply_key, &r->reply_key);
    return ret;
}
		     ...
		     char **subject_name)
{
    ...
    const HDB_Ext_PKINIT_cert *pc;
    krb5_error_code ret;
    hx509_name name, issuer = NULL, taname = NULL;
    size_t i;

    if (cp->cert == NULL) {

	*subject_name = strdup("anonymous client");
    ...
    ret = hx509_cert_get_subject(cp->tacert, &taname);
    if (ret) {
	hx509_name_free(&name);
    ...
    if (pku2uInitiatorAssertion) {
	if (pku2uInitiatorAssertion->element != choice_InitiatorName_nameNotInCert) {
	    kdc_log(context, config, 5,
		    "init name assertion not nameNotInCert");
	    goto fail;
	}
	/* XXX check that assertion is in the cert */
    }
	...
	for (i = 0; i < acl->len; i++) {
	    ...
	    ret = hx509_parse_name(context->hx509ctx, acl->val[i].subject, &n);
	    if (ret)
		continue;

	    ret = hx509_name_cmp(name, n);
	    hx509_name_free(&n);
	    if (ret)
		continue;
		...
		ret = hx509_name_cmp(issuer, n);
		hx509_name_free(&n);
		if (ret)
		    continue;
	    }
	    /* Don't support anchor checking right now */
	    if (acl->val[i].anchor) {
		ret = hx509_parse_name(context->hx509ctx,
				       *acl->val[i].anchor, &n);     
		if (ret)
