						   ...
						   ntsid_t **ntsid, rpc_binding *binding)
{
	...
	if (rpc_status != rpc_s_ok) {
        os_log_debug(OS_LOG_DEFAULT, "RPC to lsarpc gave rpc status of %#08x", rpc_status);
		/* Need a routine that converts rpc status to nt status */
		return (STATUS_UNSUCCESSFUL);
    } else if (!NT_SUCCESS(nt_status)) {		
        os_log_debug(OS_LOG_DEFAULT, "RPC to lsarpc gave nt status of %#08x", nt_status);
		return nt_status;
	}
	...
	if ((TranslatedSids.Entries == 0) || 
        ...
        os_log_debug(OS_LOG_DEFAULT, "No Relative Id (RID)?");
...
NTSTATUS GetNetworkAccountSID(const char *ServerName, char **account, char **domain, ntsid_t **ntsid)
{
    ...
    if (binding.get() == NULL) {
        os_log_debug(OS_LOG_DEFAULT, "make_rpc_binding failed");
	...
	if (!AccountName && (NT_SUCCESS(nt_status))) {
        os_log_debug(OS_LOG_DEFAULT, "Server return a NULL account name");
		nt_status = STATUS_NO_SUCH_USER; 
	}
	if (!NT_SUCCESS(nt_status)) {
        os_log_debug(OS_LOG_DEFAULT, "Couldn't get the account name: %d", nt_status);
		errno = ENOENT;
		goto done;
	}
	...
	if (!NT_SUCCESS(nt_status)) {
         os_log_debug(OS_LOG_DEFAULT, "Failed to get the sid using the fully qualified account name");
		nt_status = GetAccountNameSID(UTF16ServerName, AccountName, ntsid, &binding);
	}
	if (!NT_SUCCESS(nt_status)) {
         os_log_debug(OS_LOG_DEFAULT, "Couldn't get the account sid: %d", nt_status);
		errno = ENOENT;
		goto done;
	}
	if (account) {
