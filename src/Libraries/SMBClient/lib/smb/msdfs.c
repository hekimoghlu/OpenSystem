                                    ...
                                    CFStringRef referralStr)
{
	...
	if (error && (ntError == STATUS_PATH_NOT_COVERED)) {
		...
		os_log_error(OS_LOG_DEFAULT, "%s Dfs Check path on %s return ntstatus = 0x%x, syserr = %s",
					 __FUNCTION__, referral, ntError,
                     strerror(error));
	}
	free(referral);
								 ...
								 const char *referral)
{
    ...
    if (!pathConsumed || (utf16Len <= pathConsumed)) {
		if (utf16Len != pathConsumed) {
			os_log_debug(OS_LOG_DEFAULT, "%s PathConsumed (%d) != utf16Len (%ld)",
						 __FUNCTION__, pathConsumed, utf16Len);
		}
        return NULL;
    }
	...
	if (error) {
		os_log_error(OS_LOG_DEFAULT, "%s failed, syserr = %s",  __FUNCTION__,
                     strerror(error));
	}
							   ...
							   size_t referralLen)
{
	...
	if (!pathConsumed || (referralLen <= pathConsumed)) {
		os_log_debug(OS_LOG_DEFAULT, "%s PathConsumed = %ld referral len = %ld",
					 __FUNCTION__, pathConsumed, referralLen);
        return NULL;
    }
                      ...
                      CFMutableDictionaryRef *outReferralDict)
{
	...
	if (!referral_dict) {
		...
		error = ENOMEM;
		os_log_error(OS_LOG_DEFAULT, "%s Dictionary create failed: for referral %s, syserr = %s",
					 __FUNCTION__, dfs_referral_str,
                     strerror(error));
		goto done;
	}
	...
	if (error) {
		os_log_error(OS_LOG_DEFAULT, "%s: Failed to get path consumed, syserr = %s",
    ...
    error = smb_get_uint16le(mdp, &curr_ptr, &bytes_unparsed,
                             &number_referrals);
	if (error) {
		os_log_error(OS_LOG_DEFAULT, "%s: Failed to get the number of referrals, syserr = %s",
					 __FUNCTION__, strerror(error));
		goto done;
	}
    ...
    if (!number_referrals) {
		 ...
		 */
		error = ENOENT;
		os_log_debug(OS_LOG_DEFAULT, "%s number_referrals is zero, syserr = %s",
					 __FUNCTION__, strerror(error));
		goto done;
    }
	...
	if (error) {
		os_log_error(OS_LOG_DEFAULT, "%s Failed to get the referral header flags, syserr = %s",
					 __FUNCTION__, strerror(error));
		goto done;
	}
	...
	for (ii = 1; ii <= number_referrals; ii++) {
		...
		if (!referral_info) {
			error = ENOMEM;
			os_log_error(OS_LOG_DEFAULT, "%s: Creating referral info failed, syserr = %s",
						 __FUNCTION__, strerror(error));
			break;
		}
        ...
        error = smb_get_uint16le(mdp, &curr_ptr, &bytes_unparsed,
                                 &version_number);
		if (error) {
			os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's version failed, syserr = %s",
						 __FUNCTION__, ii, strerror(error));
        ...
        error = smb_get_uint16le(mdp, &curr_ptr, &bytes_unparsed,
                                 &referral_size);
		if (error) {
			os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's size failed, syserr = %s",
						 __FUNCTION__, ii, strerror(error));
			CFRelease(referral_info);
			break;
		}
		...
		if (referral_size < REFERRAL_ENTRY_HEADER_SIZE ) {
			error = EPROTO;
			os_log_error(OS_LOG_DEFAULT, "%s: Bad referral %d's size got %d, syserr = %s",
						 __FUNCTION__, ii, referral_size,
                         strerror(error));
			CFRelease(referral_info);
			break;
		}
		...
		if (error) {
			os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's server type failed, syserr = %s",
						 __FUNCTION__, ii, strerror(error));
			CFRelease(referral_info);
			break;
		}		
		...
		if (error) {
			os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's entry flags failed, syserr = %s",
						 __FUNCTION__, ii, strerror(error));
			CFRelease(referral_info);
			break;
		}
		...
		if (version_number == DFS_REFERRAL_V1) {
			...
			if (error) {
				os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's share name failed, syserr = %s",
							 __FUNCTION__, ii, strerror(error));
				CFRelease(referral_info);
				break;
			}
			...
			if (version_number == DFS_REFERRAL_V2) {
                ...
                error = smb_get_uint32le(mdp, &curr_ptr, &bytes_unparsed,
                                         &proximity);
				if (error) {
					os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's proximity failed, syserr = %s",
								 __FUNCTION__, ii,
                                 strerror(error));
					CFRelease(referral_info);
					break;
            ...
            error = smb_get_uint32le(mdp, &curr_ptr, &bytes_unparsed,
                                     &time_to_live);
			if (error) {
				os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's time to live failed, syserr = %s",
							 __FUNCTION__, ii, strerror(error));
				CFRelease(referral_info);
				break;
			}
			...
			if (referral_entry_flags & NAME_LIST_REFERRAL) {
				...
				if (error) {
					os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's special name failed, syserr = %s",
								 __FUNCTION__, ii,
                                 strerror(error));
					CFRelease(referral_info);
					break;
				...
				if (error) {
					os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's number of expaned names failed, syserr = %s",
								 __FUNCTION__, ii,
                                 strerror(error));
					CFRelease(referral_info);
					break;
				...
				if ((num_of_expanded_names) && expanded_name_offset) {
                    ...
                    if (!expanded_name_array) {
                        os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's failed, internal error CFArrayCreateMutable failed", __FUNCTION__, ii);
                        CFRelease(referral_info);
                        break;
                    }
				...
				os_log_debug(OS_LOG_DEFAULT, "%s: Skipping referral %d's NAME_LIST_REFERRAL!",
							 __FUNCTION__, ii);
			}
            else {
				/*
				...
				if (error) {
					os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's DFS path failed, syserr = %s",
								 __FUNCTION__, ii,
                                 strerror(error));
                    if (referral_info) {
                        CFRelease(referral_info);
				...
				if (error) {
					os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's DFS alternate path failed, syserr = %s",
								 __FUNCTION__, ii,
                                 strerror(error));
					CFRelease(referral_info);
					break;
				...
				if (error) {
					os_log_error(OS_LOG_DEFAULT, "%s: Getting referral %d's network address offset failed, syserr = %s",
								 __FUNCTION__, ii,
                                 strerror(error));
					CFRelease(referral_info);
					break;
				...
				if (referral_size) {
                     ...
                     * an error.
					 */
					os_log_debug(OS_LOG_DEFAULT, "%s: Referral %d's has a left over size of %d, syserr = %s",
								 __FUNCTION__, ii,
			...
			os_log_error(OS_LOG_DEFAULT, "%s: Referral %d's UNKNOWN VERSION, syserr = %s",
						 __FUNCTION__, ii, strerror(error));
		}
                       ...
                       CFMutableDictionaryRef *outReferralDict)
{
	...
	while (bufferOverflow && (error == 0)) 
	{
		...
		if ((error == 0) && bufferOverflow) {
			...
			if (newSize >= MAX_DFS_REFFERAL_SIZE) {
				...
				os_log_debug(OS_LOG_DEFAULT, "%s retrying with receive buffer size of %ld \n",
	...
	if (!error) {
		...
		os_log_debug(OS_LOG_DEFAULT, "%s failed, syserr = %s", __FUNCTION__,
                     strerror(error));
	}
...
int testGettingDfsReferralDict(struct smb_ctx * inConn, const char *referral)
{
	...
	for (ii = DFS_REFERRAL_V1; ii <= DFS_REFERRAL_V4; ii++) {
		...
		if (error) {
			os_log_error(OS_LOG_DEFAULT, "%s DFS_REFERRAL_V%d failed, syserr = %s",
						 __FUNCTION__, ii, strerror(error));
		}
							 ...
							 CFStringRef referralStr, CFURLRef referralURL)
{
	...
	if (url == NULL) {
		os_log_error(OS_LOG_DEFAULT, "%s creating url failed, syserr = %s",
					 __FUNCTION__, strerror(error));
		goto done;
	}
	...
	if (newConn == NULL) {
		os_log_error(OS_LOG_DEFAULT, "%s creating newConn failed, syserr = %s",
					 __FUNCTION__, strerror(error));
		goto done;
	}
	...
	openOptions = CFDictionaryCreateMutable(kCFAllocatorDefault, 0, 
											...
											&kCFTypeDictionaryValueCallBacks);
	if (!openOptions) {
		os_log_error(OS_LOG_DEFAULT, "%s creating openOptions failed, syserr = %s",
                     __FUNCTION__, strerror(error));
		goto done;
	}
	...
	if (error) {
		os_log_debug(OS_LOG_DEFAULT, "%s get server info to %s failed, syserr = %s",
                     __FUNCTION__, newConn->serverName,
                     strerror(error));
		goto done;
	}
	...
	if (error) {
		os_log_error(OS_LOG_DEFAULT, "%s clone failed, syserr = %s", 
					 __FUNCTION__, strerror(error));
		goto done;
	}
	...
	if (error) {
		os_log_debug(OS_LOG_DEFAULT, "%s open session to %s failed, syserr = %s",
                     __FUNCTION__, newConn->serverName,
                     strerror(error));
		goto done;
	}
										  ...
										  CFMutableDictionaryRef *outReferralDict)
{
	...
	*outReferralDict = NULL;
	...
	if (error) {
		os_log_debug(OS_LOG_DEFAULT, "%s connectToReferral failed, syserr = %s",
					 __FUNCTION__, strerror(error));
		goto done;
	}
	...
	if (error) {
		os_log_debug(OS_LOG_DEFAULT, "%s share connect failed, syserr = %s",
					 __FUNCTION__, strerror(error));
		goto done;
	}
	...
	error = smb2io_get_dfs_referral(newConn, referralStr, DFS_REFERRAL_V4,
                                    outReferralDict);
	if (error) {
		os_log_debug(OS_LOG_DEFAULT, "%s DFS_REFERRAL_V4 failed, syserr = %s",
					 __FUNCTION__, strerror(error));
		goto done;
	}
	...
	if (error) {
        ...
        if (inUnConsumedStr != NULL) {
            ...
            if (new_referral_str == NULL) {
                os_log_error(OS_LOG_DEFAULT, "%s: CFStringCreateMutableCopy failed",
                             __FUNCTION__);
                goto done;
            }
            ...
            if (!error) {
                ...
                os_log_error(OS_LOG_DEFAULT, "%s: getDfsReferralDictFromReferral w/o unconsumed failed (%s)",
                             __FUNCTION__, strerror(error));
            }
        }
		goto done;
	...
	for (ii = 0; ii < numberOfReferrals; ii++) {
        ...
        if ((add_unconsumed == 1) && (inUnConsumedStr != NULL)) {
            new_referral_str = CFStringCreateMutableCopy(NULL, 1024,
                                                         referralStr);
            if (new_referral_str == NULL) {
                os_log_error(OS_LOG_DEFAULT, "%s: CFStringCreateMutableCopy 2 failed",
                             __FUNCTION__);
            }
            else {
		...
		if (referralHeaderFlags & kDFSStorageServer) {
			 ...
			 */
			if (((newConn->ct_session_caps & SMB_CAP_DFS) != SMB_CAP_DFS) ||
...
getDfsRootServers(struct smb_ctx * inConn, CFStringRef serverReferralStr)
{
    ...
    if (error) {
		os_log_debug(OS_LOG_DEFAULT, "%s server connect failed, syserr = %s",
					 __FUNCTION__, strerror(error));
		goto done;
    }
...
createURLAndReplaceServerName(CFURLRef inUrl, CFStringRef serverRefferalStr)
{
    ...
    if (error) {
		os_log_debug(OS_LOG_DEFAULT, "%s smb_url_to_dictionary failed, syserr = %s",
                     __FUNCTION__, strerror(error));
        goto done;
    }
    ...
    if (error) {
		os_log_debug(OS_LOG_DEFAULT, "%s smb_dictionary_to_url failed, syserr = %s",
                     __FUNCTION__, strerror(error));
        goto done;
    }
                        ...
                        CFMutableArrayRef dfsReferralDictArray)
{
	...
	*outConn = NULL;
	...
	if ((inConn->ct_session_caps & SMB_CAP_DFS) != SMB_CAP_DFS) {
		/* Server doesn't understand DFS so we are done. */
        if (error) {
            /* Tree connect failed, log it and return the error */
            os_log_debug(OS_LOG_DEFAULT, "%s: smb_share_connect failed and no DFS, syserr = %s",
                         __FUNCTION__, strerror(error));
        }
    ...
    if (referralStr == NULL) {
        ...
        os_log_debug(OS_LOG_DEFAULT, "%s: createReferralString failed",
                     __FUNCTION__);
        goto done;
    }
    ...
    if (!error) {
        ...
        os_log_debug(OS_LOG_DEFAULT, "%s: processDfsReferralDictionary failed (%s)",
                     __FUNCTION__, strerror(error));
    }
    ...
    if (!error) {
        ...
        if (!error) {
            ...
            if (error) {
                os_log_debug(OS_LOG_DEFAULT, "%s: processDfsReferralDictionary (root/referral) failed (%s)",
                             __FUNCTION__, strerror(error));
            }
        }
        else {
            os_log_debug(OS_LOG_DEFAULT, "%s: connectToReferral (root) failed (%s)",
                         __FUNCTION__, strerror(error));
        }
        smb_ctx_done(tmpConn);
    }
    else {
        os_log_debug(OS_LOG_DEFAULT, "%s: processDfsReferralDictionary (root) failed (%s)",
                     __FUNCTION__, strerror(error));
    }
    ...
    if ((error) && (inConn->serverIsDomainController == FALSE)) {
        ...
        if (error) {
            ...
            os_log_debug(OS_LOG_DEFAULT, "%s: No AD and no domain controllers found",
                         __FUNCTION__);
            goto done;
        }
    }
...
tree_connect_worked:
	...
	error = processDfsReferralDictionary(inConn, outConn, referralStr, NULL,
                                         &loopCnt, dfsReferralDictArray);
	if (error) {
        os_log_debug(OS_LOG_DEFAULT, "%s: processDfsReferralDictionary (non AD) failed (%s)",
                     __FUNCTION__, strerror(error));
	}
    ...
    if (error) {
        os_log_error(OS_LOG_DEFAULT, "%s: mounting dfs url failed, syserr = %s",
                     __FUNCTION__, strerror(error));
    }
                   ...
                   CFMutableDictionaryRef dfsReferralDict)
{
	...
	if ((inConn->ct_session_caps & SMB_CAP_DFS) != SMB_CAP_DFS) {
		/* Server doesn't understand DFS so we are done */
        os_log_debug(OS_LOG_DEFAULT, "%s: server does not support DFS",
                     __FUNCTION__);
		goto done;
	}
	...
	if (!dfsServerDictArray) {
        os_log_debug(OS_LOG_DEFAULT, "%s: CFArrayCreateMutable failed for server dict",
                     __FUNCTION__);
		goto done;
	}
    ...
    dfsReferralDictArray = CFArrayCreateMutable(kCFAllocatorSystemDefault, 0,
                                                &kCFTypeArrayCallBacks);
	if (!dfsReferralDictArray) {
        os_log_debug(OS_LOG_DEFAULT, "%s: CFArrayCreateMutable failed",
                     __FUNCTION__);
		goto done;
	}

    error = checkForDfsReferral(inConn, &outConn, dfsReferralDictArray);
    ...
    if (error) {
        os_log_debug(OS_LOG_DEFAULT, "%s checkForDfsReferral failed syserr = %s",
                     __FUNCTION__, strerror(error));
        goto done;
    }
