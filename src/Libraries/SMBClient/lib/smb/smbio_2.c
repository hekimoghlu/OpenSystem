...
{
    if (ctx->ct_session_flags & SMBV_SMB2)
        return TRUE;
    else
        return FALSE;
}
                           ...
                           uint32_t flags, uint32_t *nt_error)
{
    ...
    if (smb_is_smb2(ctx)) {
        ...
        if (smb_ioctl_call(ctx->ct_fd, SMB2IOC_CHECK_DIR, &rq) == -1) {
            ...
            if (error) {
                os_log_debug(OS_LOG_DEFAULT, "%s: smb_ioctl_call, error %d ntstatus = 0x%x", 
                             __FUNCTION__, 
                             error, *nt_error);
            }
        }
                        ...
                        CFMutableDictionaryRef *out_referral_dict)
{
	...
	size_t rcv_buffer_size = 1024 * 64;
    ...
    if (smb_is_smb2(smbctx)) {
        ...
        /* Malloc recv buffer */
        rcv_buffer = malloc(rcv_buffer_size);
        if (rcv_buffer == NULL) {
            error = ENOMEM;
            goto bad;
        }
        bzero(rcv_buffer, rcv_buffer_size);
            ...
            char *dst, uint32_t *bytes_read)
{
    ...
    if (smb_is_smb2(smbctx)) {
        ...
        if (smb_ioctl_call(smbctx->ct_fd, SMBIOC_READ, &rwrq) == -1) {
            os_log_debug(OS_LOG_DEFAULT, "%s: smb1_ioctl_call, syserr = %s",
                         __FUNCTION__, 
                         strerror(errno));
            error = errno;                  /* Some internal error happen? */
        }
