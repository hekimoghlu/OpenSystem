...
static int IsLocalIPv4Address(uint32_t addr)
{
	...
	if ((so = socket(AF_INET, SOCK_DGRAM, 0)) < 0) {
		os_log_error(OS_LOG_DEFAULT, "%s: socket failed, syserr = %s",
	...
	if (ioctl(so, SIOCGIFCONF, (char *)&ifc) < 0) {
		os_log_error(OS_LOG_DEFAULT, "%s: ioctl (get interface configuration), syserr = %s",
					 __FUNCTION__, strerror(errno));
		goto WeAreDone;
	}
    for (ifReqIter = buffer; ifReqIter < (buffer + ifc.ifc_len);) {
        ...
        if (!SocketUtilsIncrementIfReqIter(&ifReqIter, ifr)) {
			os_log_error(OS_LOG_DEFAULT, "%s: SocketUtilsIncrementIfReqIter failed!",
						 __FUNCTION__);
            break;
        }
		ifreq = *ifr;
		...
		if (ioctl(so, SIOCGIFFLAGS, (char *)&ifreq) < 0) {
			os_log_error(OS_LOG_DEFAULT, "%s: SIOCGIFFLAGS ioctl failed, syserr = %s",
						 __FUNCTION__, strerror(errno));
			continue;
		}
...
static int IsLocalIPv6Address ( struct sockaddr_in6 *in6)
{
	...
	if (getifaddrs(&addr_list)) {
		os_log_debug(OS_LOG_DEFAULT, "%s: getifaddrs failed, syserr = %s",
					 __FUNCTION__, strerror(errno));
		return FALSE;
	}
...
int isLocalIPAddress(struct sockaddr *addr, uint16_t port, int allowLocalConn)
{
	...
	if (addr->sa_family == AF_INET) {
		...
		os_log_debug(OS_LOG_DEFAULT, "%s: Unknown address falmily %d?",
...
{	
	...
	for (res = res0; res; res = res->ai_next) {
		...
		if ((res->ai_family != PF_INET6) && (res->ai_family != PF_INET)) {
			os_log_debug(OS_LOG_DEFAULT, "Skipping address for `%s', unknown address family %d",
						 name, res->ai_family);
			continue;
		}
		...
		if ((res->ai_family == PF_INET6) && (port == NBNS_UDP_PORT_137)) {
			os_log_debug(OS_LOG_DEFAULT, "Skipping address of `%s', we don't support port 137 on IPV6 addresses",
						 name);
			continue;
		}
		/* We don't support port 139 on IPv6 addresses */		
		if ((res->ai_family == PF_INET6) && (port == NBSS_TCP_PORT_139)) {
			os_log_debug(OS_LOG_DEFAULT, "Skipping address of `%s', we don't support port 139 on IPV6 addresses",
						 name);
			continue;
		}
...
{  
	...
	*iflist = NULL;
	...
	for (i = 0; i < ifc.ifc_len;
		...
		if (ifd == NULL) {
			struct nb_ifdesc *nifd;

			for (ifd = *iflist; ifd != NULL; ifd = nifd) {
				nifd = ifd->id_next;
				free(ifd);
			}
			*iflist = NULL;
...
static int nonBlockingSocket(int family)
{
	...
	if (so < 0) {
		os_log_debug(OS_LOG_DEFAULT, "%s: socket call failed for family %d, syserr = %s",
					 __FUNCTION__, family, strerror(errno));
		return -1;
	}
	if ( (flags = fcntl(so, F_GETFL, NULL)) < 0 ) {
		os_log_debug(OS_LOG_DEFAULT, "%s: F_GETFL call failed for family %d, syserr = %s",
					 __FUNCTION__, family, strerror(errno));
		close(so);
		return -1;
	} 
	...
	if ( fcntl(so, F_SETFL, flags) < 0 ) { 
		os_log_debug(OS_LOG_DEFAULT, "%s: F_SETFL call failed for sa_family %d, syserr = %s",
					 __FUNCTION__, family, strerror(errno));
		close(so);
		return -1;
	} 
	...
	*dest = NULL;
	...
	for (ii = 0; ii < numAddresses; ii++) {
		...
		if ( (cancel) && (*cancel == TRUE) ) {
			os_log_debug(OS_LOG_DEFAULT, "%s: Connection cancelled", __FUNCTION__);
			error = ECANCELED;
			goto done;
		}
		...
		if (error < 0) {
			...
			if (errno == EINPROGRESS) {
				...
				os_log_debug(OS_LOG_DEFAULT, "%s: Connection %ld failed, family = %d, syserr = %s",
							 __FUNCTION__, ii, 
	...
	while (nfds && (totalWaitTime < kMaxTimeToWait)) { 
		...
		if (error < 0) {
			...
			if ((errno == EAGAIN) || (errno == EINTR)) {
				...
				error = errno;
				os_log_debug(OS_LOG_DEFAULT, "%s: Select call failed, syserr = %s",
		...
		if (error > 0) {
			...
			for (ii = 0; ii < numAddresses; ii++) {
				...
				if (getsockopt(conn->so, SOL_SOCKET, SO_ERROR, (void*)(&error), &dummy) < 0) {
					...
					os_log_debug(OS_LOG_DEFAULT, "%s: getsockopt failed, syserr = %s",
								 __FUNCTION__, strerror(errno));
				}
                
				if (error) {
					if (error != EINPROGRESS) {
						os_log_debug(OS_LOG_DEFAULT, "%s: Connection failed, syserr = %s",
									 __FUNCTION__, strerror(error));
    ...
    if (conn_port445 != NULL) {
        *dest = conn_port445;
        ...
        if (conn_port139 != NULL) {
            os_log_error(OS_LOG_DEFAULT, "%s: Using port 139 family = %d",
                         __FUNCTION__,
