...
SMBConvertFromUTF16ToUTF8(const uint16_t *utf16str, size_t maxLen, uint64_t options)
{
	...
	return convert_unicode_to_utf8(utf16str, maxLen, decompose_flag);
...
int SMBSetNetworkIdentity(SMBHANDLE inConnection, void *network_sid, char *account, char *domain)
{
	...
	if (smb_ioctl_call(((struct smb_ctx *)hContext)->ct_fd, SMBIOC_NTWRK_IDENTITY, &ntwrkID) == -1) {
		error = errno;
		os_log_info(OS_LOG_DEFAULT, "The SMBIOC_NTWRK_IDENTITY call failed, syserr = %s",
					 strerror(error));
		return error;
	}
	return 0;
...
SMBCreateNetBIOSName(CFStringRef proposedName)
{
	...
	if (nconverted == 0) {
	    ...
	    os_log_debug(OS_LOG_DEFAULT, "failed to compose a NetBIOS name string from '%s'",
               buf);
...
SMBRemountServer(const void	*inputBuffer, size_t inputBufferLen)
{
	...
	if (inputBufferLen == sizeof(fsid)) {
		...
		if (error) {
			os_log_debug(OS_LOG_DEFAULT, "%s: error = %d inputBuffer = %p or inputBufferLen = %zu",
				   __FUNCTION__, error, inputBuffer, inputBufferLen);
		}
	} else {
		os_log_debug(OS_LOG_DEFAULT, "%s: Bad inputBuffer = %p or inputBufferLen = %zu",
               __FUNCTION__, inputBuffer, inputBufferLen);
	}
}
