...
int32_t SMBNetFsGetMountInfo(CFStringRef in_Mountpath, CFDictionaryRef *out_MountInfo)
{
	...
	*out_MountInfo = NULL;
    ...
    /*
     * ASSUMPTIONS:
     * 1. This function is only called by the SMB Client NetFS plugin.
     * 2. NetFS.framework has already called realpath(2) on the
     *    mountpath so that its in the form of /Volumes/<blah>.
     */
