...
static int test_netbios_name_conversion(void)
{
	...
	{ CFSTR("jÃ¥mes"), CFSTR("JAMES") },
	{ CFSTR("longnameshouldbetruncated"), CFSTR("LONGNAMESHOULDB") },
	{ CFSTR("iPadÃ¦Ã¸Ã¥Ã½Ãë§ŒëŒ"), CFSTR("IPADAYMANDOL") }
    };
...
static int do_ctx_test(int type_of_test)
{
	...
	switch (type_of_test) {
		...
		case LIST_SHARE_CONNECT_TEST:
		{
			...
			urlString = SMBCreateURLString(NULL, "local", "local",  "iPadÃ¦Ã¸Ã¥Ã½Ãë§ŒëŒ._smb._tcp.local", NULL, -1);
			if (urlString) {
				error = list_shares_once(urlString);
				free(urlString);
				if (error) {
					fprintf(stdout, "iPadÃ¦Ã¸Ã¥Ã½Ãë§ŒëŒ._smb._tcp.local: Share test failed %d\n", error);
					ErrorCnt++;
				}
			} else {
