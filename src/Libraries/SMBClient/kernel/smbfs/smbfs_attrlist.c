            ...
            struct vfs_context *context)
{
        ...
        if ((!error) || (error == ENOATTR)) {
             ...
             if ((need_rsrc_fork) &&
                 ...
                 if (stream_flags & SMB_NO_RESOURCE_FORK) {
                     ...
                     ctx->f_attr.fa_fstatus |= kNO_RSRCFORK;
                 }
                 else {
             ...
             if ((ATTR_CMN_FNDRINFO & common_attr) &&
                 ...
                 if (stream_flags & SMB_NO_FINDER_INFO) {
                     ...
                     ctx->f_attr.fa_fstatus |= kNO_FINDERINFO;
                 }
             }
    ...
    if ((ATTR_CMN_USERACCESS & common_attr) &&
        !(ctx->f_attr.fa_valid_mask & FA_MAX_ACCESS_VALID) &&
        !(SS_TO_SESSION(ctx->f_share)->session_flags & SMBV_SMB2))
    {
...
update_caches:
	 ...
	 */
	if ((SS_TO_SESSION(ctx->f_share)->session_flags & SMBV_SMB2) &&
		(ctx->f_attr.fa_attr & SMB_EFA_REPARSE_POINT) &&
		(ctx->f_attr.fa_reparse_tag == IO_REPARSE_TAG_SYMLINK)) {
		tmp_error = smb2fs_smb_cmpd_reparse_point_get(ctx->f_share, ctx->f_dnp,
													  ctx->f_LocalName, ctx->f_LocalNameLen,
													  NULL, &reparse_point_len,
													  context);
		if (tmp_error == 0) {
			ctx->f_attr.fa_size = reparse_point_len;
			ctx->f_attr.fa_data_alloc = roundup(reparse_point_len,
												ctx->f_dnp->n_mount->sm_statfsbuf.f_bsize);
		}
		else {
			SMBERROR("Failed to get reparse point size %d for <%s> \n",
					 tmp_error, ctx->f_LocalName);
		}
	}
               ...
               struct vfs_context *context)
{
	...
	if (ATTR_CMN_SCRIPT & attr) {
		*((text_encoding_t *)attrbufptr) = 0x7E;
	...
	if (ATTR_CMN_OWNERID & attr) {
		 ...
		 */
        if (SMBV_HAS_GUEST_ACCESS(SS_TO_SESSION(ctx->f_share)) &&
			!(SS_TO_SESSION(ctx->f_share)->session_misc_flags & SMBV_MNT_TIME_MACHINE) &&
            !(SS_TO_SESSION(ctx->f_share)->session_misc_flags & SMBV_MNT_HIGH_FIDELITY)) {
            nuid = UNKNOWNUID;
		} 
        else {
			/* 
                              ...
                              } *ap; */
{
	...
	*(ap->a_eofflag) = 0;
    ...
    if (!dnp->d_fctx || (dnp->d_fctx->f_share != share) || (offset == 0) ||
		(offset != dnp->d_offset)) {
		smbfs_closedirlookup(dnp, 0, "readdirattr dir restart", context);
	...
	while (dnp->d_offset < offset) {
		...
		if (error) {
			smbfs_closedirlookup(dnp, 0, "readdirattr findnext error", context);
			goto done;
		}
		dnp->d_offset++;
	}
    ...
    while (uio_resid(uio)) {
 	...
 	if (error == ENOENT) {
		smbfs_closedirlookup(dnp, 0, "readdirattr EOF", context);
		*(ap->a_eofflag) = TRUE;
		error = 0;
	}
