...
/*
 * Given an object return the share or session associated with that object. If the
 * object is a share then return the parent which should be the session. In all cases
 * take a reference on the objects being return.
 */
int
smb_rq_getenv(struct smb_connobj *obj, struct smb_session **sessionp, struct smb_share **share)
{
	int error = 0;
	...
	switch (obj->co_level) {
		case SMBL_SESSION:
			if (obj->co_parent == NULL) {
				SMBERROR("zombie session %s\n", ((struct smb_session*)obj)->session_srvname);
				error = EINVAL;
			} else if (sessionp) {
				*sessionp = (struct smb_session*)obj;
		...
		case SMBL_SHARE:
			if (obj->co_parent == NULL) {
				...
				error = smb_rq_getenv(obj->co_parent, sessionp, NULL);
				if (!error && share) {
...
smb_rq_new(struct smb_rq *rqp, u_char cmd, uint16_t extraFlags2)
{
	...
	mb_put_uint8(mbp, rqp->sr_session->session_hflags);
	extraFlags2 |= rqp->sr_session->session_hflags2;
					 ...
					 int rq_flags, uint16_t flags2, vfs_context_t context)
{
	 ...
	 */
	mid = OSIncrementAtomic16((int16_t *) &rqp->sr_session->session_mid);
	if (mid == 0xffff) {
	 ...
	 */
 	if (cmd == SMB_COM_NT_TRANSACT_ASYNC) {
         ...
         */
        low_pid = OSIncrementAtomic16((int16_t *) &rqp->sr_session->session_low_pid);
        if (low_pid == 1) {
...
smb_rq_done(struct smb_rq *rqp)
{
	...
	if (rqp->sr_session) {
		smb_session_rele(rqp->sr_session, rqp->sr_context);
	}
	rqp->sr_session = NULL;
	rqp->sr_share = NULL;
...
{
	return (smb_rq_simple_timed(rqp, rqp->sr_session->session_timo));
}
			...
			uint16_t fn, vfs_context_t context)
{
	...
	error = smb_rq_getenv(obj, &ntp->nt_session, &ntp->nt_share);
	if (error)
		goto done;
	
	ntp->nt_context = context;
	/* obj is either the share or the session, so we already have a reference on it */
	ntp->nt_obj = obj;
	ntp->nt_function = fn;
done:
	if (error) {
...
smb_nt_done(struct smb_ntrq *ntp)
{
	...
	if (ntp->nt_session) {
		smb_session_rele(ntp->nt_session, ntp->nt_context);
	}
	ntp->nt_share = NULL;
	ntp->nt_session = NULL;
					 ...
					 uint16_t *setup, int setupcnt, vfs_context_t context)
{
	...
	error = smb_rq_getenv(obj, &t2p->t2_session, &t2p->t2_share);
	if (error)
		goto done;
 ...
 */
int 
smb_t2_alloc(struct smb_connobj *obj, u_short setup, 
...
smb_t2_done(struct smb_t2rq *t2p)
{
	...
	if (t2p->t2_session) {
		smb_session_rele(t2p->t2_session, t2p->t2_context);
	}
	t2p->t2_share = NULL;
	t2p->t2_session = NULL;
...
int smb_t2_request(struct smb_t2rq *t2p)
{
	...
	struct smb_session *sessionp = t2p->t2_session;
	struct mbchain *mbp;
	...
	if (sessionp->session_txmax > 0xffff)		/* maxvalue for u_short */
		txmax = 0xffff;
	else
		txmax = (uint16_t)sessionp->session_txmax;
	
	error = smb_rq_alloc(t2p->t2_obj, t2p->t_name ?
	    SMB_COM_TRANSACTION : SMB_COM_TRANSACTION2, 0, t2p->t2_context, &rqp);
	if (error)
		return error;
	rqp->sr_timo = sessionp->session_timo;
	...
	*txdoffsetp = htoles(doff);
	...
	while (leftpcount || leftdcount) {
		...
		smb_rq_bend(rqp);
		rqp->sr_state = SMBRQ_NOTSENT;
		error = smb_iod_request(sessionp->session_iod, SMBIOD_EV_NEWRQ, NULL);
		if (error)
			goto bad;
...
{
	struct smb_session *sessionp = ntp->nt_session;
	struct mbchain *mbp;
	...
	if (len + leftpcount > sessionp->session_txmax) {
		txpcount = MIN(leftpcount, sessionp->session_txmax - len);
		poff = len;
		txdcount = 0;
		doff = 0;
	} else {
		...
		len = ALIGN4(len + txpcount);
		txdcount = MIN(leftdcount, sessionp->session_txmax - len);
		doff = txdcount ? len : 0;
	}
	leftpcount -= txpcount;
	leftdcount -= txdcount;
	...
	if (totscount) {
		...
		SMBSDEBUG("%d:%d:%d\n", error, totscount, sessionp->session_txmax);
		if (error)
			goto freerq;
		mb_put_mbuf(mbp, m);
		if (totscount & 1)
	...
	if (txpcount) {
		...
		error = md_get_mbuf(&mbparam, txpcount, &m);
		SMBSDEBUG("%d:%d:%d\n", error, txpcount, sessionp->session_txmax);
		if (error)
			goto freerq;
		mb_put_mbuf(mbp, m);
	}
	...
	while (leftpcount || leftdcount) {
		...
		len = (uint32_t)check_len;
		if (len + leftpcount > sessionp->session_txmax) {
			txpcount = (uint32_t)MIN(leftpcount, sessionp->session_txmax - len);
			poff = (uint32_t)len;
			txdcount = 0;
			doff = 0;
		} else {
					 ...
					 void *nt_callback_args)
{
	 ...
	 */
	len = ALIGN4(len + 4 * 4 + 1 + 2 + ((totscount+1)&~1) + 2);
	if ((len + totpcount + totdcount) > sessionp->session_txmax) {
		error = EINVAL;
		goto free_rqp;
	} 
	txpcount = totpcount;
