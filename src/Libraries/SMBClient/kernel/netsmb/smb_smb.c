...
static 
uint32_t smb_session_maxread(struct smb_session *sessionp)
{
	...
	if (SESSION_CAPS(sessionp) & SMB_CAP_LARGE_READX) {
		...
		if (UNIX_SERVER(sessionp) && (iod->iod_saddr->sa_family != AF_NETBIOS)  &&
			 ...
			 */
			maxmsgsize = MAX_LARGEX_READ_CAP_SIZE;
		} else if (UNIX_SERVER(sessionp) && (maxmsgsize > WINDOWS_LARGEX_READ_CAP_SIZE)) {
 ...
 */
static uint32_t 
smb_session_maxwrite(struct smb_session *sessionp)
{
	uint32_t socksize = sessionp->session_sopt.sv_maxtx;
	uint32_t maxmsgsize = sessionp->session_sopt.sv_maxtx;
	uint32_t hdrsize = SMB_HDRLEN;
	 ...
	 * the length field. 
	 */
	if ((sessionp->session_hflags2 & SMB_FLAGS2_SECURITY_SIGNATURE) && (! UNIX_SERVER(sessionp))) {
		SMB_LOG_IO("%s %d - SIGNING ON\n", sessionp->session_srvname, maxmsgsize);
		return maxmsgsize;
		...
		if (UNIX_SERVER(sessionp) && (iod->iod_saddr->sa_family != AF_NETBIOS) &&
			 ...
			 */
			maxmsgsize = MAX_LARGEX_WRITE_CAP_SIZE;
		} else if (UNIX_SERVER(sessionp) && (maxmsgsize > WINDOWS_LARGEX_WRITE_CAP_SIZE)) {
...
int
smb_smb_nomux(struct smb_session *sessionp, const char *name, vfs_context_t context)
{
	if (context == sessionp->session_iod->iod_context)
		return 0;
...
int 
smb1_smb_negotiate(struct smb_session *sessionp, vfs_context_t user_context,
                   int inReconnect, int onlySMB1, vfs_context_t context)
{
	...
	error = smb_rq_alloc(SESSION_TO_CP(sessionp), SMB_COM_NEGOTIATE, 0, context, &rqp);
	if (error)
		return error;
	...
	sessionp->session_flags |= (security_mode & SMBV_SECURITY_MODE_MASK);
	...
	md_get_uint16le(mdp, &sp->sv_maxsessions);
	...
	if ((sessionp->session_misc_flags & SMBV_CLIENT_SIGNING_REQUIRED) &&
		...
		SMB_LOG_AUTH(" SMB Client requires Signing for SMB 1, server supports 0x%x\n", sessionp->session_flags);
        if (sessionp->session_flags & (SMBV_SIGNING_REQUIRED | SMBV_SIGNING)) {
            sessionp->session_flags |= SMBV_SIGNING_REQUIRED;
        } else {
            SMBERROR(" SMB Client requires signing for SMB 1, but server has signing disabled.\n");
            error = EAUTH;
            goto bad;
        }
    }
	if (sessionp->session_flags & SMBV_SIGNING_REQUIRED)
	...
	if (((sp->sv_caps & SMB_CAP_UNIX) != SMB_CAP_UNIX) &&
		...
		sessionp->session_flags |= SMBV_NT4;
	 ...
	 * clear that bit in our session_hflags2 field.
	 */
	...
	if ((sp->sv_caps & SMB_CAP_EXT_SECURITY) != SMB_CAP_EXT_SECURITY)
		sessionp->session_hflags2 &= ~SMB_FLAGS2_EXT_SEC;
	 ...
	 * bytes long, meaning "missing"). Copy challenge stuff to sessionp->session_ch (sblen bytes),
	 *
	...
	if (sblen && (sblen <= SMB_MAXCHALLENGELEN) && (sessionp->session_flags & SMBV_ENCRYPT_PASSWORD)) {
		error = md_get_mem(mdp, (caddr_t)(sessionp->session_ch), sblen, MB_MSYSTEM);
		if (error)
			goto bad;
		sessionp->session_chlen = sblen;
		toklen -= sblen; 
	}
	 ...
	 */
	sessionp->session_txmax = MIN(sessionp->session_txmax, SMB1_MAXTRANSBUFFER);
	SMBSDEBUG("CAPS = %x\n", sp->sv_caps);
	SMBSDEBUG("MAXMUX = %d\n", sp->sv_maxmux);
	SMBSDEBUG("MAXSESSIONS = %d\n", sp->sv_maxsessions);
	SMBSDEBUG("MAXTX = %d\n", sp->sv_maxtx);
	SMBSDEBUG("TXMAX = %d\n", sessionp->session_txmax);
	SMBSDEBUG("MAXWR = %d\n", sessionp->session_wxmax);
	SMBSDEBUG("MAXRD = %d\n", sessionp->session_rxmax);
    ...
    sessionp->session_hflags2 |= SMB_FLAGS2_UNICODE;
 ...
 */
uint32_t 
smb_session_caps(struct smb_session *sessionp)
{
	uint32_t caps =  SMB_CAP_LARGE_FILES | SMB_CAP_NT_SMBS | SMB_CAP_UNICODE;
	
	if (sessionp->session_hflags2 & SMB_FLAGS2_ERR_STATUS)
	...
	if (SESSION_CAPS(sessionp) & SMB_CAP_LARGE_WRITEX)
 ...
 */
void 
parse_server_os_lanman_strings(struct smb_session *sessionp, void *refptr, uint16_t bc)
{
	...
	if (SMB_UNICODE_STRINGS(sessionp)) {
	...
	if (oslen) {
		...
		if (sessionp->NativeOS) {
			...
			if ((oslen >= strlen(WIN2K_XP_UTF8_NAME)) && 
				(strncasecmp(sessionp->NativeOS, WIN2K_XP_UTF8_NAME, 
							 strlen(WIN2K_XP_UTF8_NAME)) == 0)) {
				sessionp->session_flags |= SMBV_WIN2K_XP;
			}
			/* Now see this is a Darwin smbx server */
			if ((oslen >= strlen(DARWIN_UTF8_NAME)) && 
				(strncasecmp(sessionp->NativeOS, DARWIN_UTF8_NAME, 
							 strlen(DARWIN_UTF8_NAME)) == 0)) {
				sessionp->session_flags |= SMBV_DARWIN;
			}
		}
	}
	if (lanmanlen) {
	...
	SMB_LOG_AUTH("NativeOS = %s NativeLANManager = %s server type 0x%x\n", 
			   ...
			   (sessionp->NativeLANManager) ? sessionp->NativeLANManager : "NULL", 
			   (sessionp->session_flags & SMBV_SERVER_MODE_MASK));
...
make_ntlmv2_blob(struct smb_session *sessionp, char *dom, u_int64_t client_nonce, size_t *bloblen, size_t *blob_allocsize)
{
     ...
     *
     * For now, we don't trust it - we use sessionp->session_domain and
     * sessionp->session_srvname, instead.  We upper-case them and convert
     * them to Unicode, as that's what's supposed to be in the blob.
     */
    domainlen = strlen(dom);
    srvlen = strlen(sessionp->session_srvname);
    ...
    *blob_allocsize = blobsize;
    ...
    //	blobnames = add_name_to_blob(blobnames, sessionp, (u_char *)sessionp->session_srvname,
...
smb_smb_ssnsetup(struct smbiod *iod, int inReconnect, vfs_context_t context)
{
	...
	/*
	 * Some servers can not handle signing required and Guest or Anonymous so
	 * do not require signing in this case
	 */
...
again:
	 ...
	 *
	 * NOTE: We use to copy the session_domain into an allocated buffer and then
	 * copy it into the mbuf, not sure why. Now we just copy session_domain straight
	 * in to the mbuf.
	 */
	...
	if (!(sessionp->session_flags & SMBV_USER_SECURITY)) {
		...
		plen = 1;
		uniplen = 0;
	} else if ((sessionp->session_flags & SMBV_ENCRYPT_PASSWORD) != SMBV_ENCRYPT_PASSWORD) {
		/* 
		...
		pp = smb_session_getpass(sessionp);
		plen = strnlen(pp, SMB_MAXPASSWORDLEN + 1);
		uniplen = 0;
	} else if (sessionp->session_hflags2 & SMB_FLAGS2_SECURITY_SIGNATURE) {
        /* 
        ...
        if (state == kSTATE_NTLMv2) {
             ...
             */
            smb_ntlmv2response(v2hash, sessionp->session_ch,
            ...
            pp = (char *)lm;
            uniplen = sizeof(ntlm);
            smb_ntlmresponse((u_char *)smb_session_getpass(sessionp), sessionp->session_ch, (u_char*)ntlm);
	 ...
	 */
	if (sessionp->session_flags & SMBV_ANONYMOUS_ACCESS) /*  anon */
		plen = uniplen = 0;
	...
	smb_put_dstring(mbp, SMB_UNICODE_STRINGS(sessionp), SMBFS_LANMAN,
...
bad:
	 ...
	 */
	if ((error == 0) && (sessionp->session_flags & SMBV_USER_SECURITY) && 
		!SMBV_HAS_GUEST_ACCESS(sessionp) &&
		 ...
		 */
		SMBWARNING("Got guest access, but wanted real access.\n");
		(void)smb_smb_ssnclose(sessionp, context);
		error = EAUTH;
	}
    else {
        if ((error) && (sessionp->session_flags & SMBV_USER_SECURITY) && (state == kSTATE_NTLMv2)) {
            if (sessionp->session_flags & SMBV_NO_NTLMV1) {
...
ssn_exit:
	...
	if ((sessionp->session_flags & SMBV_ENCRYPT_PASSWORD) != SMBV_ENCRYPT_PASSWORD) {
...
int
smb1_smb_ssnclose(struct smb_session *sessionp, vfs_context_t context)
{
	struct smb_rq *rqp;
	struct mbchain *mbp;
	int error;
	...
	error = smb_rq_alloc(SESSION_TO_CP(sessionp), SMB_COM_LOGOFF_ANDX, 0, context, &rqp);
	if (error)
		return error;
    smb_rq_getrequest(rqp, &mbp);
	smb_rq_wstart(rqp);
 ...
 */
static void
smb_get_share_fstype(struct smb_session *sessionp, struct smb_share *share, 
					 struct mdchain *mdp)
{
	char *tmpbuf = NULL;
	char *fsname = NULL;
	...
	SMBDEBUG("tree bc = %d\n", bc);
	if ((bc == 0) || (bc >= sessionp->session_txmax)) {
	 ...
	 */
	if (!(fs_offset & 1) && (SMB_UNICODE_STRINGS(sessionp))) {
			fs_offset += 1;
	}
	/*
...
static int 
smb1_treeconnect_internal(struct smb_session *sessionp, struct smb_share *share,  
						 ...
						 vfs_context_t context)
{
	...
	if (sessionp->session_flags & SMBV_USER_SECURITY) {
		...
		if (sessionp->session_flags & SMBV_ENCRYPT_PASSWORD) {
			plen = 24;
			smb_lmresponse((u_char *)pbuf, sessionp->session_ch, (u_char *)encpass);
			pp = encpass;
		} else {
	...
	smb_put_dmem(mbp, "\\\\", 2, NO_SFM_CONVERSIONS, SMB_UNICODE_STRINGS(sessionp), NULL);
	/*
	 ...
	 */
	error = smb_put_dmem(mbp, serverName, serverNameLen, NO_SFM_CONVERSIONS,SMB_UNICODE_STRINGS(sessionp),  NULL);
	if (error) {
		SMBERROR("error %d from smb_put_dmem for srvname\n", error);
		goto bad;
	}		
	smb_put_dmem(mbp, "\\", 1, NO_SFM_CONVERSIONS, SMB_UNICODE_STRINGS(sessionp), NULL);

	error = smb_put_dstring(mbp, SMB_UNICODE_STRINGS(sessionp), share->ss_name, SMB_MAXSHARENAMELEN+1, NO_SFM_CONVERSIONS);
	if (error) {
	...
	smb_get_share_fstype(sessionp, share, mdp);
...
smb_smb_treeconnect(struct smb_share *share, vfs_context_t context)
{
	...
	if (sessionp->ipv4v6DotName[0] == 0) {
		serverName = sessionp->session_srvname;
		srvnamelen = strnlen(serverName, SMB_MAX_DNS_SRVNAMELEN+1); 
        if (sessionp->session_flags & SMBV_SMB2) {
            error = smb2_smb_tree_connect(sessionp, share, serverName, srvnamelen,
                                          context);
       }
        else {
            error = smb1_treeconnect_internal(sessionp, share, serverName, 
                                              srvnamelen, context);
        }
	...
	if (sessionp->ipv4v6DotName[0] != 0) {
		serverName = sessionp->ipv4v6DotName;
	...
	user_ssize_t *rresid, uint16_t *available, uio_t uio, vfs_context_t context)
{
	...
	*len = MIN(SS_TO_SESSION(share)->session_rxmax, *len);
...
{
	int supportsLargeWrites = (SESSION_CAPS(SS_TO_SESSION(share)) & SMB_CAP_LARGE_FILES) ? TRUE : FALSE;
	struct smb_rq *rqp;
	struct mbchain *mbp;
	struct mdchain *mdp;
	int error;
	...
	/* session_wxmax now holds the max buffer size the server supports for writes */
	*len = MIN(*len, SS_TO_SESSION(share)->session_wxmax);
 ...
 */
int
smb1_echo(struct smb_session *sessionp, int timo, uint32_t EchoCount, 
         vfs_context_t context)
{
	struct smb_rq *rqp;
	struct mbchain *mbp;
	...
	error = smb_rq_alloc(SESSION_TO_CP(sessionp), SMB_COM_ECHO, 0, context, &rqp);
	if (error)
		return error;
    smb_rq_getrequest(rqp, &mbp);
	smb_rq_wstart(rqp);
