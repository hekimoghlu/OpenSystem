 ...
 *
 * Encoding:
 *      Each sequence of 64-bits is converted to OTP's 6 english words.
 *      For ease of readability, a newline is written after each 6 words.
 *
 * Decoding:
 *      Each 6 word sequence is converted to 64-bits. 
 */
...
typedef struct _EncoderControl_ {
    ...
    /* add conversion specific items here (otp_words encode) */
...
typedef struct _DecoderControl_ {
    ...
    /* add conversion specific items here (otp_words decode) */
...
static unsigned long    extract         _ANSI_ARGS_ ((char *s,
                                                      int start,
                                                      int length));
static void             insert          _ANSI_ARGS_ ((char *s,
                                                      int x,
                                                      int start,
                                                      int length));
static int              wsrch           _ANSI_ARGS_ ((char *w,
                                                      int low,
                                                      int high));

/* Dictionary for integer-word translations */
static char Wp[2048][4] = { /* THREADING: constant, read-only => safe */
"A",
"ABE",
"ACE",
"ACT",
...
ClientData clientData;
{
  ...
  /* initialize conversion specific items here (otp_words encode) */
...
ClientData clientData;
{
  ...
  memcpy (cp, c -> buf, sizeof c -> buf);

  for (p = 0, i = 0; i < 64; i += 2)
    p += extract (cp, i, 2);

  cp[sizeof c -> buf] = (char) (p << 6);

  for (p = 0; p <= 55; p += 11) {
    char    *w = &Wp[extract (cp, p, 11)][0];
    char     buf[5];
...
ClientData clientData;
{
  ...
  /* initialize conversion specific items here (otp_words decode) */
...
ClientData clientData;
{
  ...
  if (c -> wordCount == 6) {
    int    i = FlushDecoder (ctrlBlock, interp, clientData);
  ...
  if (!isascii (x)) {
    if (interp) {
      Tcl_ResetResult (interp);
      Tcl_AppendResult (interp, "invalid character", (char *) NULL);
    }
    return TCL_ERROR;
  }
  ...
  case ',':
    if (c -> charCount == 0) {
      if (interp) {
	Tcl_ResetResult (interp);
	Tcl_AppendResult (interp, "empty word", (char *) NULL);
      }
      return TCL_ERROR;
    }
    c -> charCount = 0;
  ...
  default:
    if (c -> charCount == 4) {
      if (interp) {
	Tcl_ResetResult (interp);
	Tcl_AppendResult (interp, "word too long", (char *) NULL);
      }
      return TCL_ERROR;
    }
    if (islower (x)) {
...
ClientData clientData;
{
  ...
  default:
    if (interp) {
      Tcl_ResetResult (interp);
      Tcl_AppendResult (interp, "too few words", (char *) NULL);
    }
    return TCL_ERROR;
  }

  memset (b, '\0', sizeof b);
  for (c -> wordCount = 0, p = 0;
       c -> wordCount < 6;
       c -> wordCount++, p += 11) {
    int    lo, hi;
    int    v;
    char  *w = &c -> words[c -> wordCount][0];

    if (strlen (w) < 4)
      lo = 0, hi = 570;
    else
      lo = 571, hi = 2047;
    if ((v = wsrch (w, lo, hi)) < 0) {
      if (interp) {
	Tcl_ResetResult (interp);
	Tcl_AppendResult (interp, "unknown word \"", w, "\"",
			  (char *) NULL);
      }
      return TCL_ERROR;
    }
  ...
  if ((p & 0x03) != extract (b, 64, 2)) {
    if (interp) {
      Tcl_ResetResult (interp);
      Tcl_AppendResult (interp, "parity error", (char *) NULL);
    }
    return TCL_ERROR;
  }
...
ClientData clientData;
{
  ...
  c -> charCount = 0;
  c -> wordCount = 0;
  memset (c -> words, '\0', sizeof c -> words);
}


/* subroutines taken from the module put.c in the original Bellcore reference
   distribution */
