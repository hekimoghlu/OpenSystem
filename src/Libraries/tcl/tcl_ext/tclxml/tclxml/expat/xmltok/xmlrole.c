	     ...
	     const ENCODING *enc)
{
  ...
  case XML_TOK_DECL_CLOSE:
    setTopLevel(state);
    return XML_ROLE_ATTLIST_DECL_CLOSE;
  case XML_TOK_NAME:
  case XML_TOK_PREFIXED_NAME:
	     ...
	     const ENCODING *enc)
{
  ...
  case XML_TOK_NAME:
    if (XmlNameMatchesAscii(enc, ptr, end, "EMPTY")) {
      state->handler = elementDeclClose;
	     ...
	     const ENCODING *enc)
{
  ...
  case XML_TOK_OR:
    ...
    /* ericm@scriptics.com:  This seems to be wrong.  I can think of two
     * good reasons why it is wrong:  first, this state machine shouldn't be
     * mangaling the input in any way; by not returning the right token, it
     * is essentially not giving the parser all the information.  Second, why
     * would you *not* want the correct content spec for mixed data models?
     * But, he was really deliberate in doing this:  the choice is propagated
     * throughout the state machine.  So he probably has a reason for it.
     */
    return XML_ROLE_NONE;
  }
  return common(state, tok);
...
/* ericm@scriptics.com:  elementDeclClose is the elementDecl close state.
 * The addition of this state allows us to explicitly recognize the close of an
 * element declaration, which lets us execute a callback for element
 * definitions that has access to the complete contentspec of the element
 */
static
int elementDeclClose(PROLOG_STATE *state,
	      int tok,
	      const char *ptr,
	      const char *end,
	      const ENCODING *enc)
{
  ...
  case XML_TOK_DECL_CLOSE:
    setTopLevel(state);
    return XML_ROLE_ELEMENT_DECL_CLOSE;
  }
  return common(state, tok);
}
...
{
#ifdef XML_DTD
    /* parameter entities are only allowed at the "toplevel" in the internal
     * subset; if we reach this state and documentEntity indicates that we
     * are in the internal subset, then this is a syntax error.  If we are in
     * the external subset, this is OK.
     */
