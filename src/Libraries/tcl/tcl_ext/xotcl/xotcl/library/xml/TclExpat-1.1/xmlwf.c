...
static void characterData(void *userData, const XML_Char *s, size_t len)
{
  FILE *fp = userData;
  for (; len > 0; --len, ++s) {
    switch (*s) {
...
static void defaultCharacterData(XML_Parser parser, const XML_Char *s, size_t len)
{
...
static
void metaCharacterData(XML_Parser parser, const XML_Char *s, size_t len)
{
  FILE *fp = XML_GetUserData(parser);
