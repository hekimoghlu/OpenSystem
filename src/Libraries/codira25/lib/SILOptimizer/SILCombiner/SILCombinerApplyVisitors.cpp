...
SILCombiner::propagateConcreteTypeOfInitExistential(FullApplySite Apply,
  ...
  //
  // If we create a new instruction thatâ€™s the same as the old one weâ€™ll
  // cause an infinite loop:
  // NewWMI will be added to the Builderâ€™s tracker list.
  // SILCombine, in turn, uses the tracker list to populate the worklist
  // As such, if we donâ€™t remove the witness method later on in the pass, we
  // are stuck:
