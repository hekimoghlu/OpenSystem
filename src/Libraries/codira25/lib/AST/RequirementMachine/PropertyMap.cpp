...
// rule ([P:A].[Q] => [P:A]). The term Ï„_0_0.[P:A], corresponding to the nested
// type 'A' of a generic parameter 'Ï„_0_0', might not have a property map entry
// of its own, if the only requirements on it are those implied by [P:A].
//
// In this case, a property map lookup for Ï„_0_0.[P:A] will find an entry for
// the term [P:A].
//
...
/// [superclass: Cache<Ï„_0_0> with <[U:C]>], then we actually need to
/// apply the substitution Ï„_0_0 := [P:A].[U:C] to the type 'Cache<Ï„_0_0>'.
///
...
/// [concrete: Array<Ï„_0_0> with <[U:C]>], then we actually need to
/// apply the substitution Ï„_0_0 := [P:A].[U:C] to the type 'Array<Ï„_0_0>'.
///
