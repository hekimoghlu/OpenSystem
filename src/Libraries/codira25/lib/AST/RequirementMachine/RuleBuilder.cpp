...
///
/// The substitutions are {Ï„_0_0 := X.Z}, and the Array : Equatable conformance
/// has a conditional requirement 'Ï„_0_0 : Equatable', so the following
...
void RuleBuilder::collectPackShapeRules(ArrayRef<GenericTypeParamType *> genericParams) {
  ...
  for (auto *genericParam : genericParams) {
    ...
    // Add the rule (Ï„_d_i.[shape] => [shape]).
    MutableTerm lhs;
