...
/// Computes the "meet" (LHS âˆ§ RHS) of two concrete type symbols (LHS and RHS
/// respectively), together with a set of transformations that turn LHS into
/// (LHS âˆ§ RHS) and RHS into (LHS âˆ§ RHS), respectively.
///
...
///
/// - If LHS == (LHS âˆ§ RHS), then lhsTransform will be None. Otherwise,
///   lhsTransform describes the transform from LHS to (LHS âˆ§ RHS).
///
/// - If RHS == (LHS âˆ§ RHS), then rhsTransform will be None. Otherwise,
///   rhsTransform describes the transform from LHS to (LHS âˆ§ RHS).
///
/// - If (LHS âˆ§ RHS) is distinct from both LHS and RHS, then both
  ...
  if (!isConflict) {
    ...
    if (lhsMeetRhs.RHS != rhsMeetLhs.RHS) {
      ABORT([&](auto &out) {
        ...
        out << "\n";
        out << "LHS âˆ§ RHS: " << lhsMeetRhs.RHS << "\n";
        out << "RHS âˆ§ LHS: " << rhsMeetLhs.RHS;
      });
    }

    // The meet operation should be idempotent.
    {
      // (LHS âˆ§ (LHS âˆ§ RHS)) == (LHS âˆ§ RHS)
      ...
      if (lhsMeetRhs.RHS != lhsMeetLhsMeetRhs.RHS) {
        ABORT([&](auto &out) {
          ...
          out << "\n";
          out << "LHS âˆ§ RHS: " << lhsMeetRhs.RHS << "\n";
          out << "LHS âˆ§ (LHS âˆ§ RHS): " << lhsMeetLhsMeetRhs.RHS;
        });
      }
    }

    {
      // (RHS âˆ§ (RHS âˆ§ LHS)) == (RHS âˆ§ LHS)
      ...
      if (lhsMeetRhs.RHS != rhsMeetRhsMeetRhs.RHS) {
        ABORT([&](auto &out) {
          ...
          out << "\n";
          out << "RHS âˆ§ LHS: " << rhsMeetLhs.RHS << "\n";
          out << "RHS âˆ§ (RHS âˆ§ LHS): " << rhsMeetRhsMeetRhs.RHS;
        });
      }
    }
  }
