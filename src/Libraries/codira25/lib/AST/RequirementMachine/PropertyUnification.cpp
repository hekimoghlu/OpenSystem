...
static void recordRelation(Term key,
  ...
  //
  //   U.(V => V.[p1]).[p2] âŠ— U.V.Relation([p1].[p2] => [p1]) âŠ— U.(V.[p1] => V).
  //
  RewritePath path;
...
void PropertyMap::unifyConcreteTypes(Term key,
  ...
  // Handle the case where (LHS âˆ§ RHS) is distinct from both LHS and RHS:
  // - First, record a new rule.
  // - Next, process the LHS -> (LHS âˆ§ RHS) difference.
  // - Finally, process the RHS -> (LHS âˆ§ RHS) difference.
  if (lhsDifferenceID && rhsDifferenceID) {
    ...
    // Recover a rewrite path from T to T.[LHS âˆ§ RHS].
    RewritePath path;
  ...
  // Handle the case where RHS == (LHS âˆ§ RHS) by processing LHS -> (LHS âˆ§ RHS).
  if (lhsDifferenceID) {
  ...
  // Handle the case where LHS == (LHS âˆ§ RHS) by processing RHS -> (LHS âˆ§ RHS).
  if (rhsDifferenceID) {
...
///
///   T.[concrete: Foo<Ï„_0_0, Ï„_0_1, String> with {X.Y, Z}] => T
///   T.[concrete: Foo<Int, Ï„_0_0, Ï„_0_1> with {A.B, W}] => T
///
