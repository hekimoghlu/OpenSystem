...
static bool decodeRegexCaptureTypes(ASTContext &ctx,
  ...
  // Encoding rules:
  // encode(ã€š`T`ã€›) ==> <version>, ã€š`T`ã€›, .end
  // ã€š`T` (atom)ã€› ==> .atom
  // ã€š`name: T` (atom)ã€› ==> .atom, `name`, '\0'
  // ã€š`[T]`ã€› ==> ã€š`T`ã€›, .formArray
  // ã€š`T?`ã€› ==> ã€š`T`ã€›, .formOptional
  // ã€š`(T0, T1, ...)` (top level)ã€› ==> ã€š`T0`ã€›, ã€š`T1`ã€›, ...
  // ã€š`(T0, T1, ...)`ã€› ==> .beginTuple, ã€š`T0`ã€›, ã€š`T1`ã€›, ..., .endTuple
  //
