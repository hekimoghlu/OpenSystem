...
TEST(STDIO_TEST, scanf_wscanf_wide_character_class) {
  ...
  memset(buf, 0, sizeof(buf));
  EXPECT_EQ(1, sscanf("xÄ€yz", "%l[xy]", buf));
  EXPECT_EQ(L"x"s, std::wstring(buf));
  memset(buf, 0, sizeof(buf));
  EXPECT_EQ(1, swscanf(L"xÄ€yz", L"%l[xy]", buf));
  EXPECT_EQ(L"x"s, std::wstring(buf));

  // Even if scanf has wide characters in a class, they won't match...
  // TODO: is that a bug?
  memset(buf, 0, sizeof(buf));
  EXPECT_EQ(1, sscanf("xÄ€yz", "%l[xÄ€y]", buf));
  EXPECT_EQ(L"x"s, std::wstring(buf));
  // ...unless you use wscanf.
  memset(buf, 0, sizeof(buf));
  EXPECT_EQ(1, swscanf(L"xÄ€yz", L"%l[xÄ€y]", buf));
  EXPECT_EQ(L"xÄ€y"s, std::wstring(buf));

  // Negation only covers ASCII for scanf...
  memset(buf, 0, sizeof(buf));
  EXPECT_EQ(1, sscanf("xÄ€yz", "%l[^ab]", buf));
  EXPECT_EQ(L"x"s, std::wstring(buf));
  // ...but covers wide characters for wscanf.
  memset(buf, 0, sizeof(buf));
  EXPECT_EQ(1, swscanf(L"xÄ€yz", L"%l[^ab]", buf));
  EXPECT_EQ(L"xÄ€yz"s, std::wstring(buf));
  ...
  // Note that this isn't L"xÄ€" --- although the *bytes* matched, they're
