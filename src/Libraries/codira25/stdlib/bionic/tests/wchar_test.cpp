...
TEST(wchar, wcwidth_non_spacing_special_cases) {
  ...
  // The following sequence is effectively "ì•½" without the leading ieung...
...
TEST(wchar, wcwidth_hangeul_compatibility_jamo) {
  ...
  // (Half of) the Korean "crying" emoticon "ã… ã… ".
  // Actually U+3160 "Hangeul Letter Yu" from Hangeul Compatibility Jamo.
  EXPECT_EQ(2, wcwidth(L'ã… '));
  // The two halves of the Korean internet shorthand "ã„±ã……" (short for ê°ì‚¬).
