...
// poly2_mod_phiN reduces |p| by Î¦(N).
...
//   t = s1 âŠ• a2
//   s3 = t âˆ§ (s2 âŠ• a1)
//   a3 = (a1 âŠ• a2) âˆ¨ (t âŠ• s2)
//
//   (s3, a3) = (s1, a1) - (s2, a2)
//   t = a1 âŠ• a2
//   s3 = (s1 âŠ• a2) âˆ§ (t âŠ• s2)
//   a3 = t âˆ¨ (s1 âŠ• s2)
//
...
// poly3_word_mul sets (|out_s|, |out_a|) to (|s1|, |a1|) Ã— (|s2|, |a2|).
...
// poly3_mul_const sets |p| to |p|Ã—m, where m = (ms, ma).
...
// poly3_fmadd sets |out| to |out| - |in|Ã—m, where m is (ms, ma).
...
// poly3_mod_phiN reduces |p| by Î¦(N).
static void poly3_mod_phiN(struct poly3 *p) {
  // In order to reduce by Î¦(N) we subtract by the value of the greatest
  // coefficient.
...
// |b| and writes 2Ã—|n| words to |out|. Each call uses 2*ceil(n/2) elements of
...
static void poly3_mul_aux(const struct poly3_span *out,
  ...
  // Calculate (a_1 + a_0) Ã— (b_1 + b_0) and write to scratch buffer.
...
// HRSS_poly3_mul sets |*out| to |x|Ã—|y| mod Î¦(N).
void HRSS_poly3_mul(struct poly3 *out, const struct poly3 *x,
  ...
  // |prod| needs to be reduced mod (ð‘¥^n - 1), which just involves adding the
...
// poly3_vec_fmsub subtracts (|ms|, |ma|) Ã— (|b_s|, |b_a|) from (|a_s|, |a_a|).
...
// poly3_invert_vec sets |*out| to |in|^-1, i.e. such that |out|Ã—|in| == 1 mod
// Î¦(N).
...
// HRSS_poly3_invert sets |*out| to |in|^-1, i.e. such that |out|Ã—|in| == 1 mod
// Î¦(N).
...
// and |b| and writes 2Ã—|n| words to |out|. Each call uses 2*ceil(n/2) elements
...
static void poly_mul_vec_aux(vec_t *out, vec_t *scratch, const vec_t *a,
  ...
  // Karatsuba. Karatsuba is a specific instance of the general Toomâ€“Cook
...
// poly_mul_vec sets |*out| to |x|Ã—|y| mod (ð‘¥^n - 1).
...
// poly_mul_novec sets |*out| to |x|Ã—|y| mod (ð‘¥^n - 1).
...
// poly_mul_x_minus_1 sets |p| to |p|Ã—(ð‘¥ - 1) mod (ð‘¥^n - 1).
static void poly_mul_x_minus_1(struct poly *p) {
  // Multiplying by (ð‘¥ - 1) means negating each coefficient and adding in
...
// poly_mod_phiN sets |p| to |p| mod Î¦(N).
...
// poly_invert_mod2 sets |*out| to |in^-1| (i.e. such that |*out|Ã—|in| = 1 mod
// Î¦(N)), all mod 2. This isn't useful in itself, but is part of doing inversion
// mod Q.
...
// poly_invert sets |*out| to |in^-1| (i.e. such that |*out|Ã—|in| = 1 mod Î¦(N)).
...
// all in {0, 1, Q-1, 65535} and |in| is mod Î¦(N). (Note that coefficients may
...
static void poly_lift(struct poly *out, const struct poly *a) {
  // We wish to calculate a/(ð‘¥-1) mod Î¦(N) over GF(3), where Î¦(N) is the
  // Nth cyclotomic polynomial, i.e. 1 + ð‘¥ + â€¦ + ð‘¥^700 (since N is prime).

  // 1/(ð‘¥-1) has a fairly basic structure that we can exploit to speed this up:
  //
  // R.<x> = PolynomialRing(GF(3)â€¦)
  ...
  // polynomial.
  //
  // Next define the overbar operator such that zÌ… = z[0] +
  // reverse(z[1:]). (Index zero of a polynomial here is the coefficient
  // of the constant term. So index one is the coefficient of ð‘¥ and so
  // on.)
  //
  // A less odd way to define this is to see that zÌ… negates the indexes,
  // so zÌ…[0] = z[-0], zÌ…[1] = z[-1] and so on.
  //
  // The use of zÌ… is that, when working mod (ð‘¥^701 - 1), vz[0] = <v,
  // zÌ…>, vz[1] = <v, ð‘¥zÌ…>, â€¦. (Where <a, b> is the inner product: the sum
  // of the point-wise products.) Although we calculated the inverse mod
  // Î¦(N), we can work mod (ð‘¥^N - 1) and reduce mod Î¦(N) at the end.
  // (That's because (ð‘¥^N - 1) is a multiple of Î¦(N).)
  //
  // When working mod (ð‘¥^N - 1), multiplication by ð‘¥ is a right-rotation
  // of the list of coefficients.
  //
  // Thus we can consider what the pattern of zÌ…, ð‘¥zÌ…, ð‘¥^2zÌ…, â€¦ looks like:
  //
  // def reverse(xs):
  //   suffix = list(xs[1:])
  //   suffix.reverse()
  ...
  //
  // (For a formula for zÌ…, see lemma two of appendix B.)
  //
  // After the first three elements have been taken care of, all then have
  // a repeating three-element cycle. The next value (ð‘¥^3zÌ…) involves
  ...
  // Reduce mod Î¦(N) by subtracting a multiple of out[700] from every
...
int HRSS_generate_key(
  ...
  // pg_phi1 is p (i.e. 3) Ã— g Ã— Î¦(1) (i.e. ð‘¥-1).
...
#pragma warning(pop)
  ...
  // Note that cf3 is not reduced mod Î¦(N). That reduction is deferred.
  ...
  // Î¦(N). In order to do that reduction one subtracts |c[N-1]| from every
  // coefficient. Therefore b(1) = -c[N-1]Ã—N. The value of |t|, above, then is
  // just recovering |c[N-1]|, and adding tÎ¦(N) is simply undoing the reduction.
  // Therefore b + tÎ¦(N) + lift(m) = c by construction and we don't need to
