...
#ifndef DALI_OPERATORS_VIDEO_READER_VIDEO_READER_OP_H_
#define DALI_OPERATORS_VIDEO_READER_VIDEO_READER_OP_H_
...
class VideoReader : public DataReader<GPUBackend, SequenceWrapper, SequenceWrapper, true> {
 ...
 protected:
  virtual void SetOutputShapeType(TensorList<GPUBackend> &output, Workspace &ws) {
    auto &curr_batch = prefetched_batch_tensors_[curr_batch_consumer_];
    output.Resize(curr_batch.shape(), curr_batch.type());
  }

  void PrepareAdditionalOutputs(Workspace &ws) {
    int output_index = 1;
    if (output_labels_) {
...
#endif  // DALI_OPERATORS_VIDEO_READER_VIDEO_READER_OP_H_
