...
BENCHMARK_DEFINE_F(OperatorBench, FlipGPU)(benchmark::State& st) {
  int batch_size = st.range(0);
  int H = st.range(1);
  int W = st.range(2);
  int C = st.range(3);
  // no change in performance for different values of below parameters:
  int horizontal = 1;
  int vertical = 1;
  int depthwise = 0;
  ...
  this->RunGPU<uint8_t>(
    ...
    OpSpec("Flip")
      ...
      .AddArg("depthwise", depthwise),
    batch_size, H, W, C);
}

BENCHMARK_REGISTER_F(OperatorBench, FlipGPU)->Iterations(1000)
