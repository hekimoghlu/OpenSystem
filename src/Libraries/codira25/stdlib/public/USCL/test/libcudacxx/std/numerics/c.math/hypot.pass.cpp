...
__host__ __device__ void test_hypot(T val)
{
  ...
  // If one of the arguments is Â±0, std::hypot(x, y) is equivalent to std::fabs called with the non-zero argument.
  ...
  {
    // If one of the arguments is Â±âˆž, std::hypot(x, y) returns +âˆž even if the other argument is NaN.
