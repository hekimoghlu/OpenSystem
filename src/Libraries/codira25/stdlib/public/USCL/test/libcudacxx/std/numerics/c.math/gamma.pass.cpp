...
__host__ __device__ void test_lgamma(T val)
{
  ...
  // If the argument is a negative integer, +âˆž is returned.
  assert(eq(cuda::std::lgamma(-2), cuda::std::numeric_limits<double>::infinity()));

  // If the argument is Â±0, +âˆž is returned.
  ...
  if constexpr (!cuda::std::is_integral_v<T>)
  {
    ...
    // If the argument is Â±âˆž, +âˆž is returned.
...
__host__ __device__ void test_tgamma(T val)
{
  ...
  if constexpr (!cuda::std::is_integral_v<T>)
  {
    ...
    // If the argument is -âˆž, NaN is returned.
