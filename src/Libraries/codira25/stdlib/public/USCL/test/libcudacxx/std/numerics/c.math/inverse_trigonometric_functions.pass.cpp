...
__host__ __device__ void test_atan2(T val)
{
  ...
  // If y is Â±0 and x is positive or +0, Â±0 is returned.
  ...
  if constexpr (!cuda::std::is_integral_v<T>)
  {
    ...
    // If y is Â±0 and x is negative or -0, Â±Ï€ is returned.
    assert(eq(cuda::std::atan2(-val, -val), -pi));
    assert(eq(cuda::std::atan2(-val, T(-2.0)), -pi));
    assert(eq(cuda::std::atan2(val, -val), pi));
    assert(eq(cuda::std::atan2(val, T(-2.0)), pi));

    // If y is Â±âˆž and x is finite, Â±Ï€/2 is returned.
    assert(eq(cuda::std::atan2(cuda::std::numeric_limits<T>::infinity(), T(2.0)), pi / T(2.0)));
    assert(eq(cuda::std::atan2(cuda::std::numeric_limits<T>::infinity(), T(-2.0)), pi / T(2.0)));
    assert(eq(cuda::std::atan2(-cuda::std::numeric_limits<T>::infinity(), T(2.0)), -pi / T(2.0)));
    assert(eq(cuda::std::atan2(-cuda::std::numeric_limits<T>::infinity(), T(-2.0)), -pi / T(2.0)));

    // If y is Â±âˆž and x is -âˆž, Â±3Ï€/4 is returned.
    assert(eq(cuda::std::atan2(cuda::std::numeric_limits<T>::infinity(), -cuda::std::numeric_limits<T>::infinity()),
              pi * T(0.75)));
    assert(eq(cuda::std::atan2(-cuda::std::numeric_limits<T>::infinity(), -cuda::std::numeric_limits<T>::infinity()),
              -pi * T(0.75)));

    // If y is Â±âˆž and x is +âˆž, Â±Ï€/4 is returned.
    assert(eq(cuda::std::atan2(cuda::std::numeric_limits<T>::infinity(), cuda::std::numeric_limits<T>::infinity()),
              pi / T(4.0)));
    assert(eq(cuda::std::atan2(-cuda::std::numeric_limits<T>::infinity(), cuda::std::numeric_limits<T>::infinity()),
              -pi / T(4.0)));

    // If x is Â±0 and y is negative, -Ï€/2 is returned.
    assert(eq(cuda::std::atan2(T(-2.0), val), -pi / T(2.0)));
    assert(eq(cuda::std::atan2(T(-2.0), -val), -pi / T(2.0)));

    // If x is Â±0 and y is positive, +Ï€/2 is returned.
    assert(eq(cuda::std::atan2(T(2.0), val), pi / T(2.0)));
    assert(eq(cuda::std::atan2(T(2.0), -val), pi / T(2.0)));

    // If x is -âˆž and y is finite and positive, +Ï€ is returned.
    assert(eq(cuda::std::atan2(T(2.0), -cuda::std::numeric_limits<T>::infinity()), pi));
    assert(eq(cuda::std::atan2(T(2.0), -cuda::std::numeric_limits<T>::infinity()), pi));

    // If x is -âˆž and y is finite and negative, -Ï€ is returned.
    assert(eq(cuda::std::atan2(T(-2.0), -cuda::std::numeric_limits<T>::infinity()), -pi));
    assert(eq(cuda::std::atan2(T(-2.0), -cuda::std::numeric_limits<T>::infinity()), -pi));

    // If x is +âˆž and y is finite and positive, +0 is returned.
    assert(eq(cuda::std::atan2(T(2.0), cuda::std::numeric_limits<T>::infinity()), val));
    assert(eq(cuda::std::atan2(T(2.0), cuda::std::numeric_limits<T>::infinity()), val));

    // If x is +âˆž and y is finite and negative, -0 is returned.
    assert(eq(cuda::std::atan2(T(-2.0), cuda::std::numeric_limits<T>::infinity()), -val));
    assert(eq(cuda::std::atan2(T(-2.0), cuda::std::numeric_limits<T>::infinity()), -val));
  }
