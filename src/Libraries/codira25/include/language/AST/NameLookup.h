...
private:
  ...
  ///    â€‚â€‚fn ii() {
  ///      â€‚â€‚fn F() {
  ///        â€‚â€‚ii() // OK! implicitly self.ii; BaseDC is the method
  ///        â€‚â€‚s()â€‚â€‚// OK! s() is defined in an outer type; BaseDC is the type
  ///        â€‚â€‚ss() // error: must write /Inner.ss() here since its static
  ///        â€‚â€‚i()  // error: there's no outer 'self.'
  ///      â€‚â€‚}
  ///    â€‚â€‚}
  /// \endcode
  ///
...
enum class DeclVisibilityKind {
  ...
  /// Declaration is a requirement â€“ in case the nominal decl does not supply
  /// a corresponding witness â€“ or an extension member of a protocol
