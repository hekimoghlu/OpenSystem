...
// â€œdependentâ€) must be differentiated with respect to only some input
// variables (the â€œindependentâ€). We say that variable `y` depends on `x` when
// the derivative of `y` with respect to `x` is not trivially null. We say that
// a variable is â€œvariedâ€ if it depends on at least one independent. Conversely
// we say that a variable is â€œusefulâ€ if at least one dependent depends on it.
// Finally, we say that a variable is â€œactiveâ€ if it is at the same time varied
...
// derivative `vÌ‡` at this place is certainly null. Conversely when variable `v`
// is not useful, then whatever the value of `vÌ‡`, this value does not matter
...
//
// Reference:
// Laurent HascoÃ«t. Automatic Differentiation by Program Transformation. 2007.
