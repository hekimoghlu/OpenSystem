...
TEST_F(LexerTest, CharactersContainTheEdgeContinuationByte) {
  ...
  // Ã€(0xC3 0x80), ã—€(0xE3 0x97 0x80), ðŸ€€(0xF0 0x9F 0x80 0x80),
  // Ã¿(0xC3 0xBF), ä¿¿(0xE4 0xBF 0xBF), ð¿(0xF0 0x90 0x90 0xBF)
  const char *Source = "Ã€ ã—€ ðŸ€€ Ã¿ ä¿¿ ð¿";
  ...
  L.lex(Tok);
  ASSERT_EQ(tok::identifier, Tok.getKind());
  ASSERT_EQ("ä¿¿", Tok.getText());

  L.lex(Tok);
  ASSERT_EQ(tok::identifier, Tok.getKind());
  ASSERT_EQ("ð¿", Tok.getText());
