    ...
    if (count > 0 && sExcludedKextNotificationRunLoopSource == NULL) {
        ...
        if (count > 1) {
            CFArrayAppendValue(excludedAlertHeaderArray,
                           CFSTR("Some system extensions are not compatible with this version of macOS and canâ€™t be used:"));
            for (i = 0; i < count; i ++) {
                kextPath = (CFStringRef) CFArrayGetValueAtIndex(
            ...
            CFArrayAppendValue(excludedAlertHeaderArray,
                           CFSTR("\" is not compatible with this version of macOS and canâ€™t be used."));
        }
        CFArrayAppendValue(excludedAlertMessageArray,
