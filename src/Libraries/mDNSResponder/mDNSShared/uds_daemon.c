...
mDNSlocal mStatus handle_regservice_request(request_state *const request)
{
    ...
    {
        LogMsg("ERROR: handle_regservice_request - Couldn't ConstructServiceName from, â€œ%#sâ€ â€œ%##sâ€ â€œ%##sâ€",
...
mDNSlocal mStatus handle_resolve_request(request_state *request)
{
    ...
    { LogMsg("ERROR: handle_resolve_request bad â€œ%sâ€ â€œ%sâ€ â€œ%sâ€", name, params.regtype, domain); return(mStatus_BadParamErr); }
...
mDNSexport void udsserver_info_dump_to_fd(int fd)
{
    ...
    else LogToFD(fd, "ProxyRecords: MISMATCH %d + %d = %d â‰  %d", ProxyA, ProxyD, ProxyA + ProxyD, m->ProxyRecords);
