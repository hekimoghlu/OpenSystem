    ...
    mDNSBool usableA, mDNSBool usableAAAA, mDNSBool reqDO)
{
    ...
    for (p = &m->DNSServers; (server = *p) != mDNSNULL; p = &server->next)
    {
        ...
        // (and if weâ€™re *not* resurrecting a record that was marked for deletion, itâ€™s a duplicate,
        // and the debugf message signifies that weâ€™re collapsing duplicate entries into one)
        *p = server->next;
