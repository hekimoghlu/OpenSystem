...
//  rr->resrec.rdata             = MUST be set by client, unless record type is CNAME or PTR and rr->HostTarget is set
    ...
    if (RRLocalOnly(rr))
    {
        ...
        // Hence, just bail out.
        // This comment is doesnâ€™t make any sense. -- SC
...
#define MAX_RESPONSE_RECORDS 100
    ...
    for (i = 0; i < totalrecords && ptr && ptr < end; i++)
    {
        ...
        else if (llintf && llintf->IgnoreIPv4LL && m->rec.r.resrec.rrtype == kDNSType_A)
        {
            ...
            // any IPv4 address they donâ€™t recognize, including RFC 3927 IPv4 link-local addresses.
            ...
            // that are deemed â€œsafeâ€ for IPv4 link-local communication;
...
mDNSexport void mDNS_DeregisterInterface(mDNS *const m, NetworkInterfaceInfo *set, InterfaceActivationSpeed activationSpeed)
{
    ...
    *p = (*p)->next;
    ...
    // so the call to AdjustAddressRecordSets*() wonâ€™t accidentally find it.
...
mDNSexport void mDNS_ConfigChanged(mDNS *const m)
{
    if (m->SPSState == 1)
    {
        ...
        {
            LogSPS("Renaming SPS from â€œ%#sâ€ to â€œ%#sâ€", name.c, newname.c);
...
// Even though this is called â€œSetupâ€ it is not called just once at startup.
// Itâ€™s actually called multiple times, every time thereâ€™s a configuration change.
