...
static void update_notification(void)
{
    ...
    if (!CFS_OQ)
    {
        ...
        // The strings CFS_OQ, CFS_CQ and the others below are the localization keys for the â€œLocalizable.stringsâ€ files,
        // and MUST NOT BE CHANGED, or localization substitution will be broken.
        // To change the text displayed to the user, edit the values in the appropriate â€œLocalizable.stringsâ€ file, not the keys here.
        ...
        // in the appropriate â€œLocalizable.stringsâ€ entries, not in the keys here (which then wonâ€™t match *any* entry in the localization files).
        ...
        CFS_OQ               = CFStringCreateWithCString(NULL, "â€œ",  kCFStringEncodingUTF8);	// DO NOT CHANGE THIS STRING
        CFS_CQ               = CFStringCreateWithCString(NULL, "â€",  kCFStringEncodingUTF8);	// DO NOT CHANGE THIS STRING
        ...
        CFS_LocalHostName    = CFStringCreateWithCString(NULL, "This computerâ€™s local hostname ",  kCFStringEncodingUTF8);
        CFS_LocalHostNameMsg = CFStringCreateWithCString(NULL, "To change the local hostname, "
                                                         "open System Preferences and click Sharing, then click â€œEditâ€ and type the name in the Local Hostname field.",  kCFStringEncodingUTF8);
