...
static std::optional<int64_t> parseUTCOffset(StringParsingBuffer<CharacterType>& buffer, bool parseSubMinutePrecision = true)
{
    ...
    //     ASCIISign TimeSpec
    //
    //  Maximum and minimum values are Â±23:59:59.999999999 = Â±86399999999999ns, which can be represented by int64_t / double's integer part.
...
bool isValidDuration(const Duration& duration)
{
    ...
    // 6. Let normalizedSeconds be days Ã— 86,400 + hours Ã— 3600 + minutes Ã— 60 + seconds + â„(ð”½(milliseconds)) Ã— 10^-3 + â„(ð”½(microseconds)) Ã— 10^-6 + â„(ð”½(nanoseconds)) Ã— 10^-9.
