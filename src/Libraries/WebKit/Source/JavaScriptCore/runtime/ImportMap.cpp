...
static void mergeModuleSpecifierMaps(ImportMap::SpecifierMap& oldMap, const ImportMap::SpecifierMap& newMap, const ImportMap::Reporter& reporter)
{
    ...
    // maps in place.
    // 2. For each specifier â†’ url of newMap:
    ...
    : m_imports(imports), m_scopesMap(scopesMap), m_integrity(integrity)
{
    ...
    // sorting normalized, with an entry a being less than an entry b if bâ€™s key
    // is code unit less than aâ€™s key.</spec>
...
void ImportMap::mergeExistingAndNewImportMaps(Ref<ImportMap>&& newImportMap, const ImportMap::Reporter& reporter)
{
    ...
    // 3. For each scopePrefix â†’ scopeImports of newImportMapScopes:
    for (auto& scope : newImportMapScopes) {
        ...
        // continue.
        //
        // 3.1.2. For each specifier â†’ url of scopeImports:
        //
    ...
    // 4. For each url â†’ integrity of newImportMap's integrity:
    ...
    // 5.1. For each specifier â†’ url of newImportMapImports:
