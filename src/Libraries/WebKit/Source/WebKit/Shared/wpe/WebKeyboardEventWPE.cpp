...
String WebKeyboardEvent::keyValueStringForWPEKeyval(unsigned keyval)
{
    switch (keyval) {
    // Modifier keys.
    case WPE_KEY_Alt_L:
...
String WebKeyboardEvent::keyCodeStringForWPEKeycode(unsigned keycode)
{
    switch (keycode) {
    case 0x0009:
        return "Escape"_s;
...
// more like what Firefox does, and generate these switch statements
// at build time.
String WebKeyboardEvent::keyIdentifierForWPEKeyval(unsigned keyval)
{
    switch (keyval) {
    case WPE_KEY_Menu:
    case WPE_KEY_Alt_L:
    ...
    return makeString("U+"_s, hex(wpe_keyval_to_unicode(keyval), 4));
}

int32_t WebKeyboardEvent::windowsKeyCodeForWPEKeyval(unsigned keyval)
{
    switch (keyval) {
    case WPE_KEY_Cancel:
    ...
    case WPE_KEY_F23:
    case WPE_KEY_F24:
        return VK_F1 + (keyval - WPE_KEY_F1);
...
String WebKeyboardEvent::singleCharacterStringForWPEKeyval(unsigned keyval)
{
    switch (keyval) {
    case WPE_KEY_ISO_Enter:
    case WPE_KEY_KP_Enter:
