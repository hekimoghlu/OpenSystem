...
{
    // The "compute the tick duration" algorithm (Â§17.4 Dispatching Actions).
    Seconds result;
...
void SimulatedInputDispatcher::transitionBetweenKeyFrames(const SimulatedInputKeyFrame& a, const SimulatedInputKeyFrame& b, AutomationCompletionHandler&& completionHandler)
{
    ...
    // The "dispatch tick actions" algorithm (Â§17.4 Dispatching Actions).
...
void SimulatedInputDispatcher::transitionInputSourceToState(SimulatedInputSource& inputSource, SimulatedInputSourceState& newState, AutomationCompletionHandler&& completionHandler)
{
    ...
    case SimulatedInputSourceType::Pen: {
        ...
        resolveLocation(valueOrDefault(a.location), b.location, b.origin.value_or(MouseMoveOrigin::Pointer), b.nodeHandle, [this, &a, &b, inputSource = inputSource.type, eventDispatchFinished = WTFMove(eventDispatchFinished)](std::optional<WebCore::IntPoint> location, std::optional<AutomationCommandError> error) mutable {
            ...
            b.location = location;
            // The "dispatch a pointer{Down,Up,Move} action" algorithms (Â§17.4 Dispatching Actions).
    ...
    case SimulatedInputSourceType::Keyboard: {
        ...
        // The "dispatch a key{Down,Up} action" algorithms (Â§17.4 Dispatching Actions).
    ...
    case SimulatedInputSourceType::Wheel:
        ...
        resolveLocation(valueOrDefault(a.location), b.location, b.origin.value_or(MouseMoveOrigin::Viewport), b.nodeHandle, [this, &a, &b, eventDispatchFinished = WTFMove(eventDispatchFinished)](std::optional<WebCore::IntPoint> location, std::optional<AutomationCommandError> error) mutable {
            ...
            if (!b.scrollDelta->isZero()) {
                ...
                // FIXME: This does not interpolate mouse scrolls per the "perform a scroll" algorithm (Â§15.4.4 Wheel actions).
...
void SimulatedInputDispatcher::run(std::optional<WebCore::FrameIdentifier> frameID, Vector<SimulatedInputKeyFrame>&& keyFrames, const HashMap<String, Ref<SimulatedInputSource>>& inputSources, AutomationCompletionHandler&& completionHandler)
{
    ...
    // The "dispatch actions" algorithm (Â§17.4 Dispatching Actions).
