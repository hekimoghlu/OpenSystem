...
#if ENABLE(FULLSCREEN_DISMISSAL_GESTURES)
    [_bannerLabel setText:[NSString stringWithFormat:WEB_UI_NSSTRING(@"â€%@â€ is in full screen.\nSwipe down to exit.", "Full Screen Warning Banner Content Text"), (NSString *)self.location]];
...
- (void)_showPhishingAlert
{
    ...
    NSString *alertMessage = [NSString stringWithFormat:WEB_UI_NSSTRING(@"Typing is not allowed in full screen websites. â€œ%@â€ may be showing a fake keyboard to trick you into disclosing personal or financial information.", "Full Screen Deceptive Website Warning Sheet Content Text"), (NSString *)self.location];
