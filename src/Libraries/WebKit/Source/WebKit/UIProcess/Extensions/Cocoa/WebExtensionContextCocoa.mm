...
void WebExtensionContext::resourceLoadDidCompleteWithError(WebPageProxyIdentifier pageID, const ResourceLoadInfo& loadInfo, const WebCore::ResourceResponse& response, const WebCore::ResourceError& error)
{
    ...
    // If a Fetch or XHR fails due to CORS, prompt the user for permission to the URL. This wonâ€™t help the failed request, but future requests might succeed if the user grants access.
...
NSString *WebExtensionContext::backgroundWebViewInspectionName()
{
    ...
    if (protectedExtension()->backgroundContentIsServiceWorker())
        m_backgroundWebViewInspectionName = WEB_UI_FORMAT_CFSTRING("%@ â€” Extension Service Worker", "Label for an inspectable Web Extension service worker", protectedExtension()->displayShortName().createCFString().get());
    else
        m_backgroundWebViewInspectionName = WEB_UI_FORMAT_CFSTRING("%@ â€” Extension Background Page", "Label for an inspectable Web Extension background page", protectedExtension()->displayShortName().createCFString().get());
