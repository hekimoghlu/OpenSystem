...
void WebInspectorUIProxy::updateInspectorWindowTitle() const
{
    ...
    if (level > 1) {
        NSString *debugTitle = [NSString stringWithFormat:WEB_UI_NSSTRING(@"Web Inspector [%d] â€” %@", "Web Inspector window title when inspecting Web Inspector"), level, (NSString *)m_urlString];
        [m_inspectorWindow setTitle:debugTitle];
    } else {
        NSString *title = [NSString stringWithFormat:WEB_UI_NSSTRING(@"Web Inspector â€” %@", "Web Inspector window title"), (NSString *)m_urlString];
