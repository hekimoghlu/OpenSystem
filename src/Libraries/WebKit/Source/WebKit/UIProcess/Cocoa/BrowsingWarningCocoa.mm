...
static NSMutableAttributedString *browsingDetailsText(const URL& url, SSBServiceLookupResult *result)
{
    if (result.isPhishing) {
        ...
        NSString *learnMore = WEB_UI_NSSTRING(@"Learn moreâ€¦", "Action from safe browsing warning");
    ...
    auto malwareOrUnwantedSoftwareDetails = [&] (NSString *description, NSString *statusStringToReplace, bool confirmMalware) {
        ...
        auto statusLink = adoptNS([[NSMutableAttributedString alloc] initWithString:WEB_UI_NSSTRING(@"the status of â€œ%site%â€", "Part of malware description")]);
