...
ExceptionOr<void> WebXRSession::updateRenderState(const XRRenderStateInit& newState)
{
    ...
    if (newState.layers) {
        /* If session was not created with "layers" enabled and newStateâ€™s layers contains more than 1 instance, throw a NotSupportedError and abort these steps.
         If sessionâ€™s pending render state is null, set it to a copy of activeState.
         If newStateâ€™s layers contains duplicate instances, throw a TypeError and abort these steps.
         For each layer in newStateâ€™s layers:

         If layer is an XRCompositionLayer and layerâ€™s session is different from session, throw a TypeError and abort these steps.
         If layer is an XRWebGLLayer and layerâ€™s session is different from session, throw a TypeError and abort these steps.
         Set sessionâ€™s pending render state's baseLayer to null.
         Set sessionâ€™s pending render state's layers to newStateâ€™s layers.
         */
...
void WebXRSession::applyPendingRenderState()
{
    ...
    // 1. Let activeState be sessionâ€™s active render state.
    // 2. Let newState be sessionâ€™s pending render state.
    // 3. Set sessionâ€™s pending render state to null.
    auto newState = WTFMove(m_pendingRenderState);
    ASSERT(newState);
    ASSERT(!m_pendingRenderState);

    // 4. Let oldBaseLayer be activeStateâ€™s baseLayer.
    // 5. Let oldLayers be activeStateâ€™s layers.
    // FIXME: those are only needed for step 6.2.

    // 6.1 Set activeState to newState.
    m_activeRenderState = newState;

    // 6.2 If oldBaseLayer is not equal to activeStateâ€™s baseLayer, oldLayers is not equal to activeStateâ€™s layers, or the dimensions of any of the layers have changed, update the viewports for session.
    // FIXME: implement this.

    // 6.3 If activeStateâ€™s inlineVerticalFieldOfView is less than sessionâ€™s minimum inline field of view set activeStateâ€™s inlineVerticalFieldOfView to sessionâ€™s minimum inline field of view.
    ...
    // 6.4 If activeStateâ€™s inlineVerticalFieldOfView is greater than sessionâ€™s maximum inline field of view set activeStateâ€™s inlineVerticalFieldOfView to sessionâ€™s maximum inline field of view.
    ...
    // 6.5 If activeStateâ€™s depthNear is less than sessionâ€™s minimum near clip plane set activeStateâ€™s depthNear to sessionâ€™s minimum near clip plane.
    if (m_activeRenderState->depthNear() < m_minimumNearClipPlane)
        m_activeRenderState->setDepthNear(m_minimumNearClipPlane);

    // 6.6 If activeStateâ€™s depthFar is greater than sessionâ€™s maximum far clip plane set activeStateâ€™s depthFar to sessionâ€™s maximum far clip plane.
    if (m_activeRenderState->depthFar() > m_maximumFarClipPlane)
        m_activeRenderState->setDepthFar(m_maximumFarClipPlane);

    // 6.7 Let baseLayer be activeStateâ€™s baseLayer.
    auto baseLayer = m_activeRenderState->baseLayer();

    // 6.8 Set activeStateâ€™s composition enabled and output canvas as follows:
...
void WebXRSession::onFrame(PlatformXR::FrameData&& frameData)
{
    ...
    queueTaskKeepingObjectAlive(*this, TaskSource::WebXR, [this, frameData = WTFMove(frameData)]() mutable {
        ...
        //  3.Set frameâ€™s time to frameTime.
        frame->setTime(static_cast<DOMHighResTimeStamp>(m_frameData.predictedDisplayTime));

        // 4. For each view in list of views, set viewâ€™s viewport modifiable flag to true.
        ...
        // 7.If sessionâ€™s pending render state is not null, apply the pending render state.
        if (m_pendingRenderState)
        ...
        if (frameShouldBeRendered() && m_frameData.shouldRender) {
            ...
            // 6.1.Set sessionâ€™s list of currently running animation frame callbacks to be sessionâ€™s list of animation frame callbacks.
            // 6.2.Set sessionâ€™s list of animation frame callbacks to the empty list.
            auto callbacks = m_callbacks;

            // 6.3.Set frameâ€™s active boolean to true.
            ...
            // 6.5.For each entry in sessionâ€™s list of currently running animation frame callbacks, in order:
            for (auto& callback : callbacks) {
                //  6.6.If the entryâ€™s cancelled boolean is true, continue to the next entry.
            ...
            // 6.9.Set sessionâ€™s list of currently running animation frame callbacks to the empty list.
            m_callbacks.removeAllMatching([](auto& callback) {
                return callback->isFiredOrCancelled();
            });

            // 6.10.Set frameâ€™s active boolean to false.
...
{
    // 1. If sessionâ€™s relevant global object is not the current global object, return false.
    RefPtr sessionDocument = downcast<Document>(scriptExecutionContext());
