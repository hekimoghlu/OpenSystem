...
void MediaSource::open()
{
    ...
    // â†³ If readyState is NOT set to "closed"
...
Ref<MediaTimePromise> MediaSource::waitForTarget(const SeekTarget& target)
{
    ...
    // â†³ If new playback position is not in any TimeRange of HTMLMediaElement.buffered
...
void MediaSource::monitorSourceBuffers()
{
    ...
    // â†³ If the HTMLMediaElement.readyState attribute equals HAVE_NOTHING:
    if (msp->mediaPlayerReadyState() == MediaPlayer::ReadyState::HaveNothing) {
        // 1. Abort these steps.
        return;
    }

    // â†³ If HTMLMediaElement.buffered does not contain a TimeRange for the current playback position:
    if (!hasCurrentTime()) {
    ...
    // â†³ If HTMLMediaElement.buffered contains a TimeRange that includes the current playback
    ...
    // â†³ If HTMLMediaElement.buffered contains a TimeRange that ends at the current playback position and does not have a range covering the time immediately after the current position:
...
void MediaSource::streamEndedWithError(std::optional<EndOfStreamError> error)
{
    ...
    // 3.
    if (!error) {
        // â†³ If error is not set, is null, or is an empty string
    ...
    if (error == EndOfStreamError::Network) {
        ...
        if (msp->mediaPlayerReadyState() == MediaPlayerReadyState::HaveNothing) {
            ...
            //  â†³ If the HTMLMediaElement.readyState attribute is greater than HAVE_NOTHING
...
ExceptionOr<Ref<SourceBuffer>> MediaSource::addSourceBuffer(const String& type)
{
    ...
    // â†³ Set the mode attribute on the new object to "sequence".
    // Otherwise:
    // â†³ Set the mode attribute on the new object to "segments".
