...
ExceptionOr<Ref<FetchResponse>> FetchResponse::create(ScriptExecutionContext& context, std::optional<FetchBodyWithType>&& bodyWithType, Init&& init)
{
    ...
    if (bodyWithType) {
        // 6.1 If responseâ€™s status is a null body status, then throw a TypeError.
        ...
        // 6.2 Set responseâ€™s body to bodyâ€™s body.
        body = WTFMove(bodyWithType->body);

        // 6.3 If bodyâ€™s type is non-null and responseâ€™s header list does not contain `Content-Type`, then append
        //     (`Content-Type`, bodyâ€™s type) to responseâ€™s header list.
