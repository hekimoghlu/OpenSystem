...
{
    // <'font-stretch'> = normal | <percentage [0,âˆž]> | ultra-condensed | extra-condensed | condensed | semi-condensed | semi-expanded | expanded | extra-expanded | ultra-expanded
...
{
    // <'font-size> = <absolute-size> | <relative-size> | <length-percentage [0,âˆž]> | math
...
{
    // <'line-height'> = normal | <number [0,âˆž]> | <length-percentage [0,âˆž]>
...
RefPtr<CSSValue> consumeFeatureTagValue(CSSParserTokenRange& range, const CSSParserContext& context)
{
    ...
    if (!range.atEnd() && range.peek().type() != CommaToken) {
        // Feature tag values could follow: <integer [0,âˆž]> | on | off
...
{
    // <'override-colors'> = [ <integer [0,âˆž]> <color> ]#
