...
template<typename Op> static std::optional<Child> simplifyForMinMax(Op& root, const SimplificationOptions& options)
{
    ...
    //   5.1. For each node child of rootâ€™s children:
...
std::optional<Child> simplify(Sum& root, const SimplificationOptions& options)
{
    ...
    // 8.1. For each of rootâ€™s children that are Sum nodes, replace them with their children.
    ...
    // 8.2. For each set of rootâ€™s children that are numeric values with identical units, remove those children and replace them with a single numeric value containing the sum of the removed nodes, and with the same unit. (E.g. combine numbers, combine percentages, combine px values, etc.)
...
std::optional<Child> simplify(Product& root, const SimplificationOptions& options)
{
    ...
    // 9.1. For each of rootâ€™s children that are Product nodes, replace them with their children.
    //
    //   -- and --
    //
    ...
    if (numericProduct) {
        ...
        // 9.3. If root contains only two children, one of which is a number (not a percentage or dimension) and the other of which is a Sum whose children are all numeric values, multiply all of the Sumâ€™s children by the number, then return the Sum.
    ...
    // 9.4. If root contains only numeric values and/or Invert nodes containing numeric values, and multiplying the types of all the children (noting that the type of an Invert node is the inverse of its childâ€™s type) results in a type that matches any of the types that a math function can resolve to, return the result of multiplying all the values of the children (noting that the value of an Invert node is the reciprocal of its childâ€™s value), expressed in the resultâ€™s canonical unit.
...
std::optional<Child> simplify(Negate& root, const SimplificationOptions&)
{
    ...
    return WTF::switchOn(root.a,
        [&]<Numeric T>(T& a) -> std::optional<Child> {
            // 6.1. If rootâ€™s child is a numeric value, return an equivalent numeric value, but with the value negated (0 - value).
            return makeChildWithValueBasedOn(0.0 - a.value, a);
        },
        [](IndirectNode<Negate>& a) -> std::optional<Child> {
            // 6.2. If rootâ€™s child is a Negate node, return the childâ€™s child.
            return { WTFMove(a->a) };
        },
...
std::optional<Child> simplify(Invert& root, const SimplificationOptions&)
{
    ...
    return WTF::switchOn(root.a,
        [&](Number& a) -> std::optional<Child> {
            // 7.1. If rootâ€™s child is a number (not a percentage or dimension) return the reciprocal of the childâ€™s value.
            return makeChild(Number { .value = (1.0 / a.value) });
        },
        [](IndirectNode<Invert>& a) -> std::optional<Child> {
            // 7.2. If rootâ€™s child is an Invert node, return the childâ€™s child.
            return { WTFMove(a->a) };
        },
        [](auto&) -> std::optional<Child> {
            return { };
