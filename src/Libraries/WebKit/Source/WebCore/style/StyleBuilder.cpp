...
void Builder::applyProperty(CSSPropertyID id, CSSValue& value, SelectorChecker::LinkMatchMask linkMatchMask, CascadeLevel cascadeLevel)
{
    ...
    if (!isUnset) {
        ...
        if (m_state.isCurrentPropertyInvalidAtComputedValueTime()) {
            ...
            // Otherwise: Either the propertyâ€™s inherited value or its initial value depending on whether the property
            // is inherited or not, respectively, as if the propertyâ€™s value had been specified as the unset keyword
