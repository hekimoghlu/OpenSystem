...
void AnimationTimelinesController::updateNamedTimelineMapForTimelineScope(const TimelineScope& scope, const Element& element)
{
    ...
    // This property declares the scope of the specified timeline names to extend across this elementâ€™s subtree. This allows a named timeline
    // (such as a named scroll progress timeline or named view progress timeline) to be referenced by elements outside the timeline-defining elementâ€™s
    // subtreeâ€”â€‹for example, by siblings, cousins, or ancestors.
    switch (scope.type) {
