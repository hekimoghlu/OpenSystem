...
void WebAnimation::setTimeline(RefPtr<AnimationTimeline>&& timeline)
{
    ...
    // 3. Let previous play state be animationâ€™s play state.
    auto previousPlayState = playState();

    // 4. Let previous current time be the animationâ€™s current time.
    ...
    // 10. If the start time of animation is resolved, make animationâ€™s hold time unresolved.
    if (m_startTime)
        m_holdTime = std::nullopt;
...
void WebAnimation::setPlaybackRate(double newPlaybackRate)
{
    ...
    if (m_timeline && m_timeline->isMonotonic() && previousTime) {
        ...
        // - the previous playback rate < 0 and the new playback rate â‰¥ 0, or
        // - the previous playback rate â‰¥ 0 and the new playback rate < 0,
        // Set animation's start time to the result of evaluating associated effect end âˆ’ start time for animation.
...
auto WebAnimation::playState() const -> PlayState
{
    ...
    // animation's effective playback rate > 0 and current time â‰¥ target effect end; or
    // animation's effective playback rate < 0 and current time â‰¤ 0,
    // â†’ finished
...
void WebAnimation::resetPendingTasks()
{
    ...
    // 6. Set the [[PromiseIsHandled]] internal slot of animationâ€™s current ready promise to true.
...
void WebAnimation::updateFinishedState(DidSeek didSeek, SynchronouslyNotify synchronouslyNotify)
{
    ...
    if (unconstrainedCurrentTime && m_startTime && !pending()) {
        ...
        if (m_playbackRate > 0 && *unconstrainedCurrentTime >= endTime) {
            ...
            // If animation playback rate â‰  0, and animation is associated with an active timeline,
...
ExceptionOr<void> WebAnimation::play(AutoRewind autoRewind)
{
    ...
    if ((playbackRate > 0 && enableSeek) && (!previousCurrentTime || *previousCurrentTime < 0_s || (*previousCurrentTime + previousCurrentTime->matchingEpsilon()) >= endTime)) {
        // If animationâ€™s effective playback rate > 0, enable seek is true and either animationâ€™s:
        //     - previous current time is unresolved, or
        //     - previous current time < zero, or
        //     - previous current time â‰¥ associated effect end,
        // Set the animationâ€™s hold time to zero.
        m_holdTime = 0_s;
    } else if ((playbackRate < 0 && enableSeek) && (!previousCurrentTime || *previousCurrentTime <= 0_s || *previousCurrentTime > endTime)) {
        // If animationâ€™s effective playback rate < 0, enable seek is true and either animationâ€™s:
        //     - previous current time is unresolved, or
        //     - previous current time â‰¤ zero, or
        ...
        // Otherwise,
        //     Set the animationâ€™s hold time to the animationâ€™s associated effect end.
        if (endTime.isInfinity())
            return Exception { ExceptionCode::InvalidStateError };
        m_holdTime = endTime;
    } else if (!playbackRate && !previousCurrentTime) {
        // If animationâ€™s effective playback rate = 0 and animationâ€™s current time is unresolved,
        // Set the animationâ€™s hold time to zero.
        m_holdTime = zeroTime();
    }
    ...
    // 8. If animationâ€™s hold time is resolved, let its start time be unresolved.
    if (m_holdTime)
        m_startTime = std::nullopt;
...
void WebAnimation::runPendingPlayTask()
{
    ...
    // 1. Assert that at least one of animationâ€™s start time or hold time is resolved.
    ...
    if (m_holdTime) {
        ...
        // 1. Let current time to match be the result of evaluating (ready time - start time) Ã— playback rate for animation.
...
ExceptionOr<void> WebAnimation::pause()
{
    ...
    // 4. If the animationâ€™s current time is unresolved and has finite timeline is false, perform the steps according
    // to the first matching condition below:
    if (!localTime && !hasFiniteTimeline) {
        if (m_playbackRate >= 0) {
            // If animation's playback rate is â‰¥ 0, let animation's hold time be zero.
            m_holdTime = zeroTime();
    ...
    // 5. If has finite timeline is true, and the animationâ€™s current time is unresolved,
    ...
    //     - the user agent has performed any processing necessary to suspend the playback of animationâ€™s associated effect, if any.
...
void WebAnimation::runPendingPauseTask()
{
    ...
    //    evaluating (ready time - start time) Ã— playback rate.
...
void WebAnimation::autoAlignStartTime()
{
    ...
    // 7. Set start time to start offset if effective playback rate â‰¥ 0, and end offset otherwise.
...
void WebAnimation::tick()
{
    ...
    //       descendant of the animationâ€™s associated effect including rendering the first frame of
    //       any keyframe effect or executing any custom effects associated with an animation effect.
    //     - the animation is associated with a timeline that is not inactive.
    //     - the animationâ€™s hold time or start time is resolved.
...
bool WebAnimation::computeRelevance()
{
    ...
    // Â§â€¯4.8.3.1 Calculating the active time, is not unresolved.
...
ExceptionOr<void> WebAnimation::commitStyles()
{
    ...
    // 2.3 Let inline style be the result of getting the CSS declaration block corresponding to targetâ€™s style attribute. If target does not have a style
    ...
    auto commitProperty = [&](AnimatableCSSProperty property) {
        ...
        // 4. Let effect value be the result of calculating the result of partialEffectStack for property using target's computed style (see Â§â€¯5.4.3 Calculating
...
std::optional<Seconds> WebAnimation::convertAnimationTimeToTimelineTime(Seconds animationTime) const
{
    ...
    if (!m_playbackRate || !m_startTime || animationTime.isInfinity())
        return std::nullopt;
    // 5. Return the result of calculating: time Ã— (1 / playback rate) + start time (where playback rate and start time are the playback rate and start time of animation, respectively).
