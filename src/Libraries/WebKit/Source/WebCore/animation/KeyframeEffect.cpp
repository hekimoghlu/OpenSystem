...
static inline void computeMissingKeyframeOffsets(Vector<KeyframeEffect::ParsedKeyframe>& keyframes)
{
    ...
    //    4. Set the computed keyframe offset of keyframe to offsetA + (offsetB âˆ’ offsetA) Ã— index / n.
...
static inline ExceptionOr<void> processPropertyIndexedKeyframes(JSGlobalObject& lexicalGlobalObject, Document& document, Strong<JSObject>&& keyframesInput, Vector<KeyframeEffect::ParsedKeyframe>& parsedKeyframes, Vector<String>& unusedEasings)
{
    ...
    for (auto& m : propertyIndexedKeyframe.propertiesAndValues) {
        ...
        // 2. If property name is â€œcompositeâ€, or â€œeasingâ€, or â€œoffsetâ€, skip the remaining steps in this loop and continue from the next member in property-indexed
        //    keyframe after m.
        ...
        for (auto& v : propertyValues) {
            ...
            // 2. Add the property-value pair, property name â†’ v, to k.
    ...
    // 5. Let offsets be a sequence of nullable double values assigned based on the type of the â€œoffsetâ€ member of the property-indexed keyframe as follows:
    //    - sequence<double?>, the value of â€œoffsetâ€ as-is.
    //    - double?, a sequence of length one with the value of â€œoffsetâ€ as its single item, i.e. Â« offset Â»,
    ...
    // 7. Let easings be a sequence of DOMString values assigned based on the type of the â€œeasingâ€ member of the property-indexed keyframe as follows:
    //    - sequence<DOMString>, the value of â€œeasingâ€ as-is.
    //    - DOMString, a sequence of length one with the value of â€œeasingâ€ as its single item, i.e. Â« easing Â»,
    ...
    // 8. If easings is an empty sequence, let it be a sequence of length one containing the single value â€œlinearâ€, i.e. Â« "linear" Â».
    if (easings.isEmpty())
    ...
    // 11. Assign each value in easings to a property named â€œeasingâ€ on the keyframe with the corresponding position in property keyframes until the end of property keyframes
    //     is reached.
    for (size_t i = 0; i < parsedKeyframes.size(); ++i)
        parsedKeyframes[i].easing = easings[i];

    // 12. If the â€œcompositeâ€ member of the property-indexed keyframe is not an empty sequence:
    ...
    if (!compositeModes.isEmpty()) {
        // 1. Let composite modes be a sequence of CompositeOperationOrAuto values assigned from the â€œcompositeâ€ member of property-indexed keyframe. If that member is a single
        //    CompositeOperationOrAuto value operation, let composite modes be a sequence of length one, with the value of the â€œcompositeâ€ as its single item.
...
ExceptionOr<void> KeyframeEffect::processKeyframes(JSGlobalObject& lexicalGlobalObject, Document& document, Strong<JSObject>&& keyframesInput)
{
    ...
    for (auto& keyframe : parsedKeyframes) {
        // Let the timing function of frame be the result of parsing the â€œeasingâ€ property on frame using the CSS syntax
        // defined for the easing property of the AnimationEffectTiming interface.
        // If parsing the â€œeasingâ€ property fails, throw a TypeError and abort this procedure.
