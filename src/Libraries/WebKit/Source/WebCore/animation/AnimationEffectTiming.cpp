...
void AnimationEffectTiming::updateComputedProperties(std::optional<WebAnimationTime> timelineDuration, double playbackRate)
{
    ...
    //     repeated duration = iteration duration Ã— iteration count
...
BasicEffectTiming AnimationEffectTiming::getBasicTiming(const ResolutionData& data) const
{
    ...
    auto phase = [this, data, localTime]() -> AnimationEffectPhase {
        ...
        auto atProgressTimelineBoundary = [&]() {
            ...
            // - the animationâ€™s playback rate is zero
            // return false
            if (!data.timelineDuration || data.timelineDuration->isZero())
                return false;
            if (!data.playbackRate)
                return false;
            // Let effective start time be the animationâ€™s start time if resolved, or zero otherwise.
            ...
            // - start time is resolved: (timeline time - start time) Ã— playback rate
            ...
            // Let effective timeline time be unlimited current time / animationâ€™s playback rate + effective start time
        ...
        // An animation effect is in the active phase if the animation effectâ€™s local time is not unresolved and it is not
...
ResolvedEffectTiming AnimationEffectTiming::resolve(const ResolutionData& data) const
{
    ...
    auto [transformedProgress, before] = [this, directedProgress, currentDirection, phase]() -> std::pair<std::optional<double>, TimingFunction::Before> {
        ...
        if (!iterationDuration.isZero()) {
            ...
            // 3. Return the result of evaluating the animation effectâ€™s timing function passing directed progress as the
