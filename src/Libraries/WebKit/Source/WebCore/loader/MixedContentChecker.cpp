...
bool MixedContentChecker::shouldUpgradeInsecureContent(LocalFrame& frame, IsUpgradable isUpgradable, const URL& url, FetchOptions::Destination destination, Initiator initiator)
{
    ...
    // 4.1.3 Â§â€¯4.3 Does settings prohibit mixed security contexts? returns "Does Not Restrict Mixed Security Contents" when applied to requestâ€™s client.
    if (!foundMixedContentInFrameTree(frame, url))
        return false;
...
{
    // 4.1.1 requestâ€™s URL is a potentially trustworthy URL.
    if (url.protocolIs("https"_s))
        return false;
    // 4.1.2 requestâ€™s URLâ€™s host is an IP address.
    if (URL::hostIsIPAddress(url.host()) && !shouldTreatAsPotentiallyTrustworthy(url))
        return false;
    // 4.1.4 requestâ€™s destination is not "image", "audio", or "video".
    if (!destinationIsImageAudioOrVideo(destination))
        return false;
    // 4.1.5 requestâ€™s destination is "image" and requestâ€™s initiator is "imageset".
