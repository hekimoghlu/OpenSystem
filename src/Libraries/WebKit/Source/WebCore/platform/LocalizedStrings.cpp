...
String contextMenuItemTagLookUpInDictionary(const String& selectedString)
{
    ...
    return WEB_UI_FORMAT_CFSTRING("Look Up â€œ%@â€", "Look Up context menu item with selected word", selectedCFString.get());
}
#endif
...
{
    auto selectedCFString = truncatedStringForMenuItem(selectedString).createCFString();
    return WEB_UI_FORMAT_CFSTRING("Translate â€œ%@â€", "Translate context menu item with selected word", selectedCFString.get());
}
#endif
...
String imageTitle(const String& filename, const IntSize& size)
{
    ...
    return WEB_UI_FORMAT_CFSTRING("%@ %@Ã—%@ pixels", "window title for a standalone image (uses multiplication symbol, not x)", filename.createCFString().get(), widthString.get(), heightString.get());
#elif PLATFORM(WIN)
    return WEB_UI_FORMAT_STRING("%s %dÃ—%d pixels", "window title for a standalone image (uses multiplication symbol, not x)", filename.wideCharacters().data(), size.width(), size.height());
#elif USE(GLIB)
    return WEB_UI_FORMAT_STRING("%s %dÃ—%d pixels", "window title for a standalone image (uses multiplication symbol, not x)", filename.utf8().data(), size.width(), size.height());
#else
    return makeStringByReplacingAll(WEB_UI_FORMAT_STRING("<filename> %dÃ—%d pixels", "window title for a standalone image (uses multiplication symbol, not x)", size.width(), size.height()), "<filename>"_s, filename);
#endif
}

String mediaElementLoadingStateText()
{
    return WEB_UI_STRING("Loadingâ€¦", "Media controller status message when the media is loading");
}
...
{
    return WEB_UI_FORMAT_CFSTRING("â€œ%@â€ would like to use Touch ID for â€œ%@â€.", "Allow the specified bundle to use Touch ID to sign in to the specified website on this device", bundleName.createCFString().get(), domain.createCFString().get());
}

String getAssertionTouchIDPromptTitle(const String& bundleName, const String& domain)
{
    return WEB_UI_FORMAT_CFSTRING("â€œ%@â€ would like to sign in to â€œ%@â€.", "Allow the specified bundle to sign in to the specified website", bundleName.createCFString().get(), domain.createCFString().get());
}
