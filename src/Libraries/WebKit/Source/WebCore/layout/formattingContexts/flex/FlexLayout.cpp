...
FlexLayout::FlexBaseAndHypotheticalMainSizeList FlexLayout::flexBaseAndHypotheticalMainSizeForFlexItems(const LogicalFlexItems& flexItems, bool isSizedUnderMinMaxConstraints) const
{
    ...
    for (auto& flexItem : flexItems) {
        ...
        auto computedFlexBase = [&]() -> LayoutUnit {
            ...
            // treating a value of content as max-content. If a cross size is needed to determine the main size (e.g. when the flex itemâ€™s main size
            // is in its block axis) and the flex itemâ€™s cross size is auto and not definite, in this calculation use fit-content as the flex itemâ€™s cross size.
