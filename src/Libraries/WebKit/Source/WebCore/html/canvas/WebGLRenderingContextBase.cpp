...
WebGLAny WebGLRenderingContextBase::getUniform(WebGLProgram& program, const WebGLUniformLocation& uniformLocation)
{
    ...
    case GraphicsContextGL::FLOAT: {
        ...
        m_context->getUniformfv(program.object(), location, valueSpan);
        if (length == 1)
            return value[0];
    ...
    case GraphicsContextGL::INT: {
        ...
        auto valueSpan = std::span { value }.first(length);
        m_context->getUniformiv(program.object(), location, valueSpan);
        if (length == 1)
            return value[0];
    ...
    case GraphicsContextGL::UNSIGNED_INT: {
        ...
        auto valueSpan = std::span { value }.first(length);
        m_context->getUniformuiv(program.object(), location, valueSpan);
        if (length == 1)
            return value[0];
...
void WebGLRenderingContextBase::makeXRCompatible(MakeXRCompatiblePromise&& promise)
{
    ...
    // 1. If the requesting documentâ€™s origin is not allowed to use the "xr-spatial-tracking"
    ...
    xrSystem.ensureImmersiveXRDeviceIsSelected([this, protectedThis = Ref { *this }, promise = WTFMove(promise), protectedXrSystem = Ref { xrSystem }]() mutable {
        ...
        // 4. Set contextâ€™s XR compatible boolean as follows:
        //    If contextâ€™s WebGL context lost flag is set
        //      Set contextâ€™s XR compatible boolean to false and reject promise with an InvalidStateError.
        if (isContextLost())
            return;
        ...
        // If contextâ€™s XR compatible boolean is true. Resolve promise.
        // If context was created on a compatible graphics adapter for the immersive XR device
        //  Set contextâ€™s XR compatible boolean to true and resolve promise.
...
void WebGLRenderingContextBase::vertexAttribfvImpl(ASCIILiteral functionName, GCGLuint index, Float32List&& list, GCGLsizei expectedSize)
{
    ...
    switch (expectedSize) {
    case 1:
        m_context->vertexAttrib1fv(index, data.first<1>());
        break;
    case 2:
        m_context->vertexAttrib2fv(index, data.first<2>());
        break;
    case 3:
        m_context->vertexAttrib3fv(index, data.first<3>());
        break;
    case 4:
        m_context->vertexAttrib4fv(index, data.first<4>());
        break;
    }
    VertexAttribValue& attribValue = m_vertexAttribValue[index];
