...
void VTTCueBox::applyCSSProperties()
{
    ...
    //   Let left be x-positionâ€‰vw and top be y-positionâ€‰vh.
    ...
    //   vw and height be auto. Otherwise, let width be auto and height be sizeâ€‰vh.
    // Use 'cqh' and 'cqw' rather than 'vh' and 'vw' here, as the video viewport
    // is not a true viewport, but it is a container, so they serve the same purpose.
...
int VTTCue::calculateComputedLinePosition() const
{
    ...
    // can happen when using the DOM APIâ€™s snapToLines and line attributes.)
    ...
    // the list of text tracks of a media element, return âˆ’1 and abort these steps.
    if (!track())
        return -1;
    ...
    // in the media elementâ€™s list of text tracks before track.
...
{
    // 1. Let region be cueâ€™s WebVTT cue region.
    ASSERT(region());

    // 2. If regionâ€™s WebVTT region scroll setting is up and region already has one child,
    // set regionâ€™s transition-property to top and transition-duration to 0.433s.

    // 3. Let offset be cueâ€™s computed position multiplied by regionâ€™s WebVTT region width
    ...
    // 4. Adjust offset using cueâ€™s computed position alignment as follows:

    // 5. If the computed position alignment is center alignment
    // Subtract half of regionâ€™s WebVTT region width from offset.
...
void VTTCue::calculateDisplayParameters()
{
    ...
    // 8. If the WebVTT cue writing direction is horizontal, then let width be sizeâ€‰vw and height
    // be auto. Otherwise, let width be auto and height be sizeâ€‰vh.
    ...
    // 25. Let left be x-positionâ€‰vw and top be y-positionâ€‰vh.
    ...
    while (!input.isAtEnd()) {
        ...
        case Line: {
            ...
            do {
                ...
                //    0 â‰¤ number â‰¤ 100, then jump to the step labeled next setting.
