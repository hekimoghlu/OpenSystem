...
void HTMLMediaElement::setSrcObject(MediaProvider&& mediaProvider)
{
    ...
    // srcObject: On setting, it must set the elementâ€™s assigned media provider object to the new
    // value, and then invoke the elementâ€™s media element load algorithm.
...
void HTMLMediaElement::selectMediaResource()
{
    ...
    // 1. Set the elementâ€™s networkState attribute to the NETWORK_NO_SOURCE value.
    m_networkState = NETWORK_NO_SOURCE;

    // 2. Set the elementâ€™s show poster flag to true.
    setShowPosterFlag(true);

    // 3. Set the media elementâ€™s delaying-the-load-event flag to true (this delays the load event).
    ...
    queueCancellableTaskKeepingObjectAlive(*this, TaskSource::MediaElement, m_resourceSelectionTaskCancellationGroup, [this, logSiteIdentifier]  {
        ...
        // 5. If the media elementâ€™s blocked-on-parser flag is false, then populate the list of pending text tracks.
        ...
        // 7. Set the media elementâ€™s networkState to NETWORK_LOADING.
        ...
        if (mode == Object) {
            ...
            // 6. Abort these steps. The element wonâ€™t attempt to load another resource until this
            //    algorithm is triggered again.
            return;
        }

        // â†³ If mode is attribute
        if (mode == Attribute) {
            m_loadState = LoadingFromSrcAttr;

            // 1. If the src attributeâ€™s value is the empty string, then end the synchronous section,
            ...
            //    specified by the src attributeâ€™s value relative to the media element when the src
            ...
            // 8. Abort these steps. The element wonâ€™t attempt to load another resource until this algorithm is triggered again.
            return;
        }

        // â†³ Otherwise (mode is children)
...
void HTMLMediaElement::setReadyState(MediaPlayer::ReadyState state)
{
    ...
    do {
        ...
        if (m_readyState == HAVE_FUTURE_DATA && oldState <= HAVE_CURRENT_DATA) {
            ...
            // If the elementâ€™s paused attribute is false, the user agent must queue a task to fire a simple event named playing at the element.
            if (!paused())
...
void HTMLMediaElement::updateBufferingState()
{
    ...
    //
    // The :buffering pseudo-class represents an element that is capable of being â€œplayedâ€ or â€œpausedâ€,
    ...
    // to be â€œplayingâ€ when it is â€œbufferingâ€. Whenever :buffering matches an element, :playing also
    // matches the element.)
...
void HTMLMediaElement::updateStalledState()
{
    ...
    // considered to be â€œplayingâ€ when it is â€œstalledâ€. Whenever :stalled matches an element, :playing
