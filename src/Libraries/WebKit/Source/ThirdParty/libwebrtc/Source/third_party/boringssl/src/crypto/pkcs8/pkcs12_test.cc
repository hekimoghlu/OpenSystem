...
static const char kUnicodePassword[] = "Hello, Ã¤Â¸â€“Ã§â€¢Å’";
...
TEST(PKCS12Test, TestUnicode) {
  // Generated with
  //   openssl pkcs12 -export -inkey ecdsa_p256_key.pem -in ecdsa_p256_cert.pem -password pass:"Hello, Ã¤Â¸â€“Ã§â€¢Å’"
...
TEST(PKCS12Test, RoundTrip) {
  ...
  // Test some Unicode.
  TestRoundTrip(kPassword, "Hello, Ã¤Â¸â€“Ã§â€¢Å’!",
                bssl::Span<const uint8_t>(kTestKey),
                bssl::Span<const uint8_t>(kTestCert),
                {bssl::Span<const uint8_t>(kTestCert2)}, 0, 0, 0, 0);
