...
// poly2_mod_phiN reduces |p| by ÃŽÂ¦(N).
...
//
//   (s3, a3) = (s1, a1) Ãƒâ€” (s2, a2)
//   a3 = a1 Ã¢Ë†Â§ a2
//   s3 = (s1 Ã¢Å â€¢ s2) Ã¢Ë†Â§ a3
//
//   (s3, a3) = (s1, a1) + (s2, a2)
//   t = s1 Ã¢Å â€¢ a2
//   s3 = t Ã¢Ë†Â§ (s2 Ã¢Å â€¢ a1)
//   a3 = (a1 Ã¢Å â€¢ a2) Ã¢Ë†Â¨ (t Ã¢Å â€¢ s2)
//
//   (s3, a3) = (s1, a1) - (s2, a2)
//   t = a1 Ã¢Å â€¢ a2
//   s3 = (s1 Ã¢Å â€¢ a2) Ã¢Ë†Â§ (t Ã¢Å â€¢ s2)
//   a3 = t Ã¢Ë†Â¨ (s1 Ã¢Å â€¢ s2)
//
...
// poly3_word_mul sets (|out_s|, |out_a|) to (|s1|, |a1|) Ãƒâ€” (|s2|, |a2|).
...
// poly3_mul_const sets |p| to |p|Ãƒâ€”m, where m = (ms, ma).
...
// poly3_fmadd sets |out| to |out| - |in|Ãƒâ€”m, where m is (ms, ma).
...
// poly3_mod_phiN reduces |p| by ÃŽÂ¦(N).
static void poly3_mod_phiN(struct poly3 *p) {
  // In order to reduce by ÃŽÂ¦(N) we subtract by the value of the greatest
  // coefficient.
...
// |b| and writes 2Ãƒâ€”|n| words to |out|. Each call uses 2*ceil(n/2) elements of
...
// HRSS_poly3_mul sets |*out| to |x|Ãƒâ€”|y| mod ÃŽÂ¦(N).
void HRSS_poly3_mul(struct poly3 *out, const struct poly3 *x,
  ...
  // |prod| needs to be reduced mod (Ã°Ââ€˜Â¥^n - 1), which just involves adding the
...
// poly3_vec_fmsub subtracts (|ms|, |ma|) Ãƒâ€” (|b_s|, |b_a|) from (|a_s|, |a_a|).
...
// poly3_invert_vec sets |*out| to |in|^-1, i.e. such that |out|Ãƒâ€”|in| == 1 mod
// ÃŽÂ¦(N).
...
// HRSS_poly3_invert sets |*out| to |in|^-1, i.e. such that |out|Ãƒâ€”|in| == 1 mod
// ÃŽÂ¦(N).
...
// and |b| and writes 2Ãƒâ€”|n| words to |out|. Each call uses 2*ceil(n/2) elements
...
static void poly_mul_vec_aux(vec_t *restrict out, vec_t *restrict scratch,
  ...
  // Karatsuba. Karatsuba is a specific instance of the general ToomÃ¢â‚¬â€œCook
...
// poly_mul_vec sets |*out| to |x|Ãƒâ€”|y| mod (Ã°Ââ€˜Â¥^n - 1).
...
// poly_mul_novec sets |*out| to |x|Ãƒâ€”|y| mod (Ã°Ââ€˜Â¥^n - 1).
...
// poly_mul_x_minus_1 sets |p| to |p|Ãƒâ€”(Ã°Ââ€˜Â¥ - 1) mod (Ã°Ââ€˜Â¥^n - 1).
static void poly_mul_x_minus_1(struct poly *p) {
  // Multiplying by (Ã°Ââ€˜Â¥ - 1) means negating each coefficient and adding in
...
// poly_mod_phiN sets |p| to |p| mod ÃŽÂ¦(N).
...
// poly_invert_mod2 sets |*out| to |in^-1| (i.e. such that |*out|Ãƒâ€”|in| = 1 mod
// ÃŽÂ¦(N)), all mod 2. This isn't useful in itself, but is part of doing inversion
// mod Q.
...
// poly_invert sets |*out| to |in^-1| (i.e. such that |*out|Ãƒâ€”|in| = 1 mod ÃŽÂ¦(N)).
...
// all in {0, 1, Q-1, 65535} and |in| is mod ÃŽÂ¦(N). (Note that coefficients may
...
static void poly_lift(struct poly *out, const struct poly *a) {
  // We wish to calculate a/(Ã°Ââ€˜Â¥-1) mod ÃŽÂ¦(N) over GF(3), where ÃŽÂ¦(N) is the
  // Nth cyclotomic polynomial, i.e. 1 + Ã°Ââ€˜Â¥ + Ã¢â‚¬Â¦ + Ã°Ââ€˜Â¥^700 (since N is prime).

  // 1/(Ã°Ââ€˜Â¥-1) has a fairly basic structure that we can exploit to speed this up:
  //
  // R.<x> = PolynomialRing(GF(3)Ã¢â‚¬Â¦)
  ...
  // polynomial.
  //
  // Next define the overbar operator such that zÃŒâ€¦ = z[0] +
  // reverse(z[1:]). (Index zero of a polynomial here is the coefficient
  // of the constant term. So index one is the coefficient of Ã°Ââ€˜Â¥ and so
  // on.)
  //
  // A less odd way to define this is to see that zÃŒâ€¦ negates the indexes,
  // so zÃŒâ€¦[0] = z[-0], zÃŒâ€¦[1] = z[-1] and so on.
  //
  // The use of zÃŒâ€¦ is that, when working mod (Ã°Ââ€˜Â¥^701 - 1), vz[0] = <v,
  // zÃŒâ€¦>, vz[1] = <v, Ã°Ââ€˜Â¥zÃŒâ€¦>, Ã¢â‚¬Â¦. (Where <a, b> is the inner product: the sum
  // of the point-wise products.) Although we calculated the inverse mod
  // ÃŽÂ¦(N), we can work mod (Ã°Ââ€˜Â¥^N - 1) and reduce mod ÃŽÂ¦(N) at the end.
  // (That's because (Ã°Ââ€˜Â¥^N - 1) is a multiple of ÃŽÂ¦(N).)
  //
  // When working mod (Ã°Ââ€˜Â¥^N - 1), multiplication by Ã°Ââ€˜Â¥ is a right-rotation
  // of the list of coefficients.
  //
  // Thus we can consider what the pattern of zÃŒâ€¦, Ã°Ââ€˜Â¥zÃŒâ€¦, Ã°Ââ€˜Â¥^2zÃŒâ€¦, Ã¢â‚¬Â¦ looks like:
  //
  // def reverse(xs):
  //   suffix = list(xs[1:])
  //   suffix.reverse()
  ...
  //
  // (For a formula for zÃŒâ€¦, see lemma two of appendix B.)
  //
  // After the first three elements have been taken care of, all then have
  // a repeating three-element cycle. The next value (Ã°Ââ€˜Â¥^3zÃŒâ€¦) involves
  ...
  // between Ã°Ââ€˜Â¥^3zÃŒâ€¦ and zÃŒâ€¦:
  //
  ...
  // Reduce mod ÃŽÂ¦(N) by subtracting a multiple of out[700] from every
...
int HRSS_generate_key(
  ...
  // pg_phi1 is p (i.e. 3) Ãƒâ€” g Ãƒâ€” ÃŽÂ¦(1) (i.e. Ã°Ââ€˜Â¥-1).
...
int HRSS_decap(uint8_t out_shared_key[HRSS_KEY_BYTES],
  ...
  // Note that cf3 is not reduced mod ÃŽÂ¦(N). That reduction is deferred.
  ...
  // ÃŽÂ¦(N). In order to do that reduction one subtracts |c[N-1]| from every
  // coefficient. Therefore b(1) = -c[N-1]Ãƒâ€”N. The value of |t|, above, then is
  // just recovering |c[N-1]|, and adding tÃŽÂ¦(N) is simply undoing the reduction.
  // Therefore b + tÃŽÂ¦(N) + lift(m) = c by construction and we don't need to
