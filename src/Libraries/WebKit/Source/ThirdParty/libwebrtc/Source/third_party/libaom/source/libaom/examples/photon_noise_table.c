...
// is, if one were to shoot on a 35mm camera (36Ã—24mm sensor) at the nominal
...
// entirety of a 36Ã—24mm (â€œ35mm formatâ€) sensor. If that assumption does not
// hold, then a â€œ35mm-equivalent ISO valueâ€ that can be passed to the tool can
// be obtained by multiplying the true ISO value by the ratio of 36Ã—24mm to the
// area that was actually used. For formats that approximately share the same
// aspect ratio, this is often expressed as the square of the â€œequivalence
// ratioâ€ which is the ratio of their diagonals. For example, APS-C (often
// ~24Ã—16mm) is said to have an equivalence ratio of 1.5 relative to the 35mm
// format, and therefore ISO 1000 on APS-C and ISO 1000Ã—1.5Â² = 2250 on 35mm
...
// size, that noise will tend to â€œaverage outâ€ to the same amount over a given
...
typedef struct {
  ...
  // â€œmid-toneâ€, as it would be e.g. 1800 cd/mÂ² for SMPTE ST 2084 (PQ).
  float mid_tone;
} transfer_function_t;
...
// Note: it is perhaps debatable whether â€œlinearâ€ for HLG should be scene light
// or display light. Here, it is implemented in terms of display light assuming
// a nominal peak display luminance of 1000 cd/mÂ², hence the system Î³ of 1.2. To
...
static void generate_photon_noise(const photon_noise_args_t *photon_noise_args,
  ...
  // In microns. Assumes a 35mm sensor (36mm Ã— 24mm).
