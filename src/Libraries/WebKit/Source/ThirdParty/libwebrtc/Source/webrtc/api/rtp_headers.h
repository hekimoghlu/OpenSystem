...
struct AbsoluteCaptureTime {
  ...
  //
  // Itâ€™s not always possible to do an NTP clock readout at the exact moment of
  ...
  // Estimated capture clock offset is the senderâ€™s estimate of the offset
  // between its own NTP clock and the capture systemâ€™s NTP clock. The sender is
  ...
  //
  // This field is encoded as a 64-bit twoâ€™s complement signed fixed-point
  // number with the high 32 bits for the seconds and low 32 bits for the
  // fractional part. Itâ€™s intended to make it easy for a receiver, that knows
  // how to estimate the sender systemâ€™s NTP clock, to also estimate the capture
  // systemâ€™s NTP clock:
  //
