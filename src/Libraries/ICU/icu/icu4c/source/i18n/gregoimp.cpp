...
double ClockMath::floorDivide(double numerator, int32_t denominator,
                          int32_t* remainder) {
    // For an integer n and representable âŒŠx/nâŒ‹, âŒŠRN(x/n)âŒ‹=âŒŠx/nâŒ‹, where RN is
    // rounding to nearest.
    double quotient = uprv_floor(numerator / denominator);
    if (remainder != nullptr) {
      // For doubles x and n, where n is an integer and âŒŠx+nâŒ‹ < 2Â³Â¹, the
      // expression `(int32_t) (x + n)` evaluated with rounding to nearest
      // differs from âŒŠx+nâŒ‹ if 0 < âŒˆxâŒ‰âˆ’x â‰ª x+n, as `x + n` is rounded up to
      // n+âŒˆxâŒ‰ = âŒŠx+nâŒ‹ + 1.  Rewriting it as âŒŠxâŒ‹+n makes the addition exact.
