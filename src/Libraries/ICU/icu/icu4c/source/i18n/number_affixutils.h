...
class U_I18N_API AffixUtils {
  ...
  public:
    ...
    /**
     * Executes the unescape state machine. Replaces the unquoted characters "-", "+", "%", "â€°", and
     * "Â¤" with the corresponding symbols provided by the {@link SymbolProvider}, and inserts the
     * result into the FormattedStringBuilder at the requested location.
     *
     * <p>Example input: "'-'Â¤x"; example output: "-$x"
     *
   ...
   * if you only need the length but not the string itself.
    ...
    /**
     * Checks whether the specified affix pattern has any unquoted currency symbols ("Â¤").
     *
