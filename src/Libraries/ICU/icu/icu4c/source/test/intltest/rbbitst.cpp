...
void RBBITest::checkUnicodeTestCase(const char *testFileName, int lineNumber,
    ...
    if (error) {
        ...
        for (int32_t i = 0; i < testString.length();) {
            ...
            i += U16_LENGTH(c);
            expected += expectedBreaks.count(i) == 1 ? u"Ã·" : u"Ã—";
            actual += actualBreaks.count(i) == 1 ? u"Ã·" : u"Ã—";
            expected += c;
            actual += c;
        }
        expected += expectedBreaks.count(testString.length()) == 1 ? u"Ã·" : u"Ã—";
        actual += actualBreaks.count(testString.length()) == 1 ? u"Ã·" : u"Ã—";
...
int32_t RBBILineMonkey::next(int32_t startPos) {
    ...
    for (;;) {
        ...
        if (fQU->contains(thisChar) && !fPi->contains(thisChar)) {
            setAppliedRule(pos, "LB 19 Ã— [QU-\\p{Pi}]");
            continue;
        }

        // [QU-\p{Pf}] Ã—
        if (fQU->contains(prevChar) && !fPf->contains(prevChar)) {
            setAppliedRule(pos, "LB 19 [QU-\\p{Pf}] Ã—");
            continue;
        }

        // LB 19a
        // [^\p{ea=F}\p{ea=W}\p{ea=H}] Ã— QU
        if (!feaFWH->contains(prevChar) && fQU->contains(thisChar)) {
            setAppliedRule(pos, "LB 19a [^\\p{ea=F}\\p{ea=W}\\p{ea=H}] Ã— QU");
            continue;
        }
        // Ã— QU ( [^\p{ea=F}\p{ea=W}\p{ea=H}] | eot )
        if (fQU->contains(thisChar)) {
            if (nextPos < fText->length()) {
                UChar32 nextChar = fText->char32At(nextPos);
                if (!feaFWH->contains(nextChar)) {
                    setAppliedRule(pos, "LB 19a Ã— QU [^\\p{ea=F}\\p{ea=W}\\p{ea=H}]");
                    continue;
                }
            } else {
                setAppliedRule(pos, "LB 19 Ã— QU eot");
                continue;
            }
        }
        // QU Ã— [^\p{ea=F}\p{ea=W}\p{ea=H}]
        if (fQU->contains(prevChar) && !feaFWH->contains(thisChar)) {
            setAppliedRule(pos, "LB 19a QU Ã— [^\\p{ea=F}\\p{ea=W}\\p{ea=H}]");
            continue;
        }
        // ( sot | [^\p{ea=F}\p{ea=W}\p{ea=H}] ) QU Ã—
        if (fQU->contains(prevChar)) {
            if (prevPos == 0) {
                setAppliedRule(pos, "LB 19a sot QU Ã—");
                continue;
            }
            ...
            if (!feaFWH->contains(fText->char32At(breakObliviousPrevPosX2)) ||
                ...
                setAppliedRule(pos, "LB 19a [^\\p{ea=F}\\p{ea=W}\\p{ea=H}] QU Ã—");
                continue;
            }
        }
        ...
        // (sot | BK | CR | LF | NL | SP | ZW | CB | GL) ( HY | [\u2010] ) Ã— AL
        ...
        for (const auto& [left, right] : {
                 std::pair{fCL, fPO}, // 1. NU (SY | IS)* CL Ã— PO
                 std::pair{fCP, fPO}, // 2. NU (SY | IS)* CP Ã— PO
                 std::pair{fCL, fPR}, // 3. NU (SY | IS)* CL Ã— PR
                 std::pair{fCP, fPR}, // 4. NU (SY | IS)* CP Ã— PR
             }) {
        ...
        for (const auto& [left, right] : {
                 std::pair{fPO, fNU}, // 8. PO Ã— NU
                 std::pair{fPR, fNU}, // 10. PR Ã— NU
                 std::pair{fHY, fNU}, // 11. HY Ã— NU
                 std::pair{fIS, fNU}, // 12. IS Ã— NU
             }) {
            if (left->contains(prevChar) && right->contains(thisChar)) {
                continueToNextPosition = true;
                break;
        ...
        if (fAP->contains(prevChar) &&
            (fAK->contains(thisChar) || thisChar == U'â—Œ' || fAS->contains(thisChar))) {
            setAppliedRule(pos, "LB 28a.1  AP x (AK | â—Œ | AS)");
            continue;
        }

        if ((fAK->contains(prevChar) || prevChar == U'â—Œ' || fAS->contains(prevChar)) &&
            (fVF->contains(thisChar) || fVI->contains(thisChar))) {
            setAppliedRule(pos, "LB 28a.2  (AK | â—Œ | AS) x (VF | VI)");
            continue;
        }

        if ((fAK->contains(prevCharX2) || prevCharX2 == U'â—Œ' || fAS->contains(prevCharX2)) &&
            fVI->contains(prevChar) &&
            (fAK->contains(thisChar) || thisChar == U'â—Œ')) {
            setAppliedRule(pos, "LB 28a.3  (AK | â—Œ | AS) VI x (AK | â—Œ)");
            continue;
        }
        ...
        if (nextPos < fText->length()) {
            ...
            if ((fAK->contains(prevChar) || prevChar == U'â—Œ' || fAS->contains(prevChar)) &&
                ...
                setAppliedRule(pos, "LB 28a.4  (AK | â—Œ | AS) x (AK | â—Œ | AS) VF");
                continue;
            }
        }
...
void RBBITest::RunMonkey(BreakIterator *bi, RBBIMonkeyKind &mk, const char *name, std::string engineState,
    ...
    while (loopCount < numIterations || numIterations == -1) {
        if (numIterations == -1 && loopCount % 10 == 0) {
            ...
            constexpr std::array<std::string_view, 5> monkeys{"ðŸ™ˆ", "ðŸ™‰", "ðŸ™Š", "ðŸµ", "ðŸ’"};
            fprintf(stderr, "%s",
                    monkeys[RandomNumberGenerator(randomNumberGenerator)() % monkeys.size()].data());
            if (loopCount % 1'000'000 == 0) {
                fprintf(stderr, "\nTested %" PRId64 " million random strings with %d errorsâ€¦\n",
        ...
        if (exportFile != nullptr) {
            for (i = 0; i < testText.length();) {
                fprintf(exportFile, expectedBreaks[i] ? "Ã· " : "Ã— ");
                char32_t const c = testText.char32At(i);
                fprintf(exportFile, "%04X ", static_cast<uint32_t>(c));
                i += U16_LENGTH(c);
            }
            fprintf(exportFile, expectedBreaks[testText.length()] ? "Ã·  # ðŸ’\n" : "Ã—  # ðŸ’\n");
        }
...
void RBBITest::TestTraceCreateBreakEngine() {
    ...
    UnicodeString text(
        u"test "
        u"æ¸¬è©¦ " // Hani
        u"ážŸáž¶áž€áž›áŸ’áž”áž„ " // Khmr
        u"àº—àº»àº”àºªàº­àºš " // Laoo
        u"á€…á€™á€ºá€¸á€žá€•á€ºá€™á€¾á€¯ " // Mymr
        u"à¸—à¸”à¸ªà¸­à¸š " // Thai
        u"test "
    );
    brkitr->setText(text);
...
void RBBITest::TestExternalBreakEngineWithFakeYue() {
    UErrorCode status = U_ZERO_ERROR;
    UnicodeString text(u"a bc defä¸€å…©å¹´å‰ä½¢çœŸä¿‚å””é¾æ„ç•€æˆ‘å½±ç›¸i jk lmn");
...
void RBBITest::TestExternalBreakEngineWithFakeTaiLe() {
    UErrorCode status = U_ZERO_ERROR;
    UnicodeString text(
        u"a bc defá¥›á¥«á¥’á¥°á¥–á¥­á¥°á¥žá¥á¥°á¥™á¥¥á¥¢á¥›á¥«á¥’á¥°á¥‘á¥©á¥¢á¥²á¥”á¥£á¥á¥´á¥“á¥¬á¥–á¥©á¥¢á¥²á¥›á¥£á¥á¥±á¥™á¥á¥±á¥™á¥¤á¥±á¥“á¥£á¥’á¥›á¥£á¥°á¥“á¥§"
        u"á¥°á¥˜á¥©á¥°á¥—á¥ªá¥’á¥´á¥›á¥£á¥°á¥˜á¥¬á¥°á¥á¥£á¥±á¥˜á¥’á¥±á¥”á¥£á¥›á¥´á¥˜á¥«á¥¢i jk lmn");
