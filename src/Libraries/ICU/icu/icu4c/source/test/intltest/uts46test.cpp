...
void UTS46Test::TestInvalidPunycodeDigits() {
    ...
    UnicodeString result;
    {
        ...
        assertEquals("normal result", u"á”¼á”´", result);
    }
    {
        IDNAInfo info;
...
void UTS46Test::TestACELabelEdgeCases() {
...

    {
        ...
        // If not CheckHyphens, the label must not begin with â€œxn--â€.
        IDNAInfo info;
...
void UTS46Test::TestDefaultNontransitional() {
    ...
    UnicodeString result;
    IDNAInfo info;
    forZero->labelToUnicode(u"Fâ“¤ÃŸáºž", result, info, errorCode);
    assertEquals("forZero.toUnicode(Fâ“¤ÃŸáºž)", u"fussss", result);
    forZero->labelToASCII(u"Fâ“¤ÃŸáºž", result, info, errorCode);
    assertEquals("forZero.toASCII(Fâ“¤ÃŸáºž)", u"fussss", result);
    forDefault->labelToUnicode(u"Fâ“¤ÃŸáºž", result, info, errorCode);
    assertEquals("forDefault.toUnicode(Fâ“¤ÃŸáºž)", u"fuÃŸÃŸ", result);
    forDefault->labelToASCII(u"Fâ“¤ÃŸáºž", result, info, errorCode);
    assertEquals("forDefault.toASCII(Fâ“¤ÃŸáºž)", u"xn--fu-hiaa", result);
}
