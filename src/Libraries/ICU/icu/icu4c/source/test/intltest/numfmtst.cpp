...
void NumberFormatTest::TestLocalizedPatternSymbolCoverage() {
    ...
    dfs.setSymbol(DecimalFormatSymbols::kGroupingSeparatorSymbol, u'â–');
    dfs.setSymbol(DecimalFormatSymbols::kDecimalSeparatorSymbol, u'â˜');
    dfs.setSymbol(DecimalFormatSymbols::kPatternSeparatorSymbol, u'â™');
    dfs.setSymbol(DecimalFormatSymbols::kDigitSymbol, u'â–°');
    dfs.setSymbol(DecimalFormatSymbols::kZeroDigitSymbol, u'à»');
    dfs.setSymbol(DecimalFormatSymbols::kSignificantDigitSymbol, u'â•');
    dfs.setSymbol(DecimalFormatSymbols::kPlusSignSymbol, u'â€ ');
    dfs.setSymbol(DecimalFormatSymbols::kMinusSignSymbol, u'â€¡');
    dfs.setSymbol(DecimalFormatSymbols::kPercentSymbol, u'âœ');
    dfs.setSymbol(DecimalFormatSymbols::kPerMillSymbol, u'â€±');
    dfs.setSymbol(DecimalFormatSymbols::kExponentialSymbol, u"â‘â‘"); // tests multi-char sequence
    dfs.setSymbol(DecimalFormatSymbols::kPadEscapeSymbol, u'â‚');

    {
        UnicodeString standardPattern(u"#,##0.05+%;#,##0.05-%");
        UnicodeString localizedPattern(u"â–°â–â–°â–°à»â˜à»à»•â€ âœâ™â–°â–â–°â–°à»â˜à»à»•â€¡âœ");
    ...
    {
        UnicodeString standardPattern(u"* @@@E0â€°");
        UnicodeString localizedPattern(u"â‚ â•â•â•â‘â‘à»â€±");
...
// rdar://
    ...
    assertEquals("MXV.getName(LONG_NAME) for pl",
                 UnicodeString(u"meksykaÅ„ska jednostka inwestycyjna", -1),
...
void NumberFormatTest::TestCurrencyVariants(){
    ...
    struct TestCase {
        ...
        {"en-US", u"GEL", u"GEL", u"â‚¾", u"GEL", u"GEL", U_USING_DEFAULT_WARNING}, // narrow: fallback to root
        {"ka-GE", u"GEL", u"â‚¾", u"â‚¾", u"â‚¾", u"â‚¾", U_USING_DEFAULT_WARNING}, // narrow: fallback to ka
        {"ka", u"GEL", u"â‚¾", u"â‚¾", u"â‚¾", u"â‚¾", U_USING_DEFAULT_WARNING}, // narrow fallback to short
        {"zh-TW", u"TWD", u"$", u"$", u"NT$", u"$", U_USING_DEFAULT_WARNING}, // narrow: fallback to short
        {"ccp", u"TRY", u"TRY", u"â‚º", u"TRY", u"TL", U_USING_DEFAULT_WARNING}, // everything fallback to root
    };
...
NumberFormatTest::TestParseCurrencyInUCurr() {
    const char* DATA[] = {
        ...
        "Malaysian ringgit1.00",
#if APPLE_ICU_CHANGES
// rdar://102181560 (Malaysian Ringgit plural form should be â€œringgitâ€ instead of â€œringgitsâ€)
#else
...
void NumberFormatTest::Test13035_MultiCodePointPaddingInPattern() {
    ...
    DecimalFormat df(u"a*'à®¨à®¿'###0b", status);
    ...
    //assertEquals("Multi-codepoint padding should not be split", u"aà®¨à®¿à®¨à®¿12b", result);
...
void NumberFormatTest::Test11035_FormatCurrencyAmount() {
    ...
    const char16_t* expected = u"12,345$67Â â€‹";
...
void NumberFormatTest::Test11839() {
    ...
    dfs->setSymbol(DecimalFormatSymbols::kMinusSignSymbol, u"aâˆ¸");
    dfs->setSymbol(DecimalFormatSymbols::kPlusSignSymbol, u"bâˆ”"); //  âˆ”  U+2214 DOT PLUS
    DecimalFormat df(u"0.00+;0.00-", dfs.orphan(), errorCode);
    UnicodeString result;
    df.format(-1.234, result, errorCode);
    assertEquals("Locale-specific minus sign should be used", u"1.23aâˆ¸", result);
    df.format(1.234, result.remove(), errorCode);
    assertEquals("Locale-specific plus sign should be used", u"1.23bâˆ”", result);
    // Test round-trip with parse
    expect2(df, -456, u"456.00aâˆ¸");
    expect2(df, 456, u"456.00bâˆ”");
}
...
void NumberFormatTest::Test11626_CustomizeCurrencyPluralInfo() {
    ...
    assertEquals("Plural other", u"99 Ð°Ð¼ÐµÑ€Ð¸Ñ‡ÐºÐ¸Ñ… Ð´Ð¾Ð»Ð°Ñ€Ð°", df.format(99, result.remove(), errorCode));
...
void NumberFormatTest::Test11025_CurrencyPadding() {
    UErrorCode status = U_ZERO_ERROR;
    UnicodeString pattern(u"Â¤Â¤ **####0.00");
...
void NumberFormatTest::Test12753_PatternDecimalPoint() {
 ...
 void NumberFormatTest::Test11647_PatternCurrencySymbols() {
    ...
    df.applyPattern(u"Â¤Â¤Â¤Â¤#", status);
    if (!assertSuccess("", status)) return;
    UnicodeString actual;
...
void NumberFormatTest::Test11640_TripleCurrencySymbol() {
    ...
    UnicodeString actual;
    DecimalFormat dFormat(u"Â¤Â¤Â¤ 0", status);
    if (U_FAILURE(status)) {
        dataerrln("Failure creating DecimalFormat %s", u_errorName(status));
        return;
    }
...
void NumberFormatTest::Test20348_CurrencyPrefixOverride() {
    ...
    assertEquals("Initial pattern",
        u"Â¤#,##0.00", fmt->toPattern(result.remove()));
    assertEquals("Initial prefix",
        u"Â¤", fmt->getPositivePrefix(result.remove()));
    assertEquals("Initial suffix",
        u"-Â¤", fmt->getNegativePrefix(result.remove()));
...
void NumberFormatTest::Test20956_MonetarySymbolGetters() {
    ...
    assertEquals("OUTPUT", u"12Â 345.12Â kr", sb);
}
...
// rdar://
    ...
    }
    {
        ...
        if (status.errIfFailureAndReset()) { return; }
        assertEquals("iso_code", u"XXXÂ 1.10",
            nf->format(1.1, result.remove(), status));
 #if APPLE_ICU_CHANGES
// rdar://
        assertEquals("currency", u"", nf->getCurrency()); // rdar://51985640
...
void NumberFormatTest::Test20499_CurrencyVisibleDigitsPlural() {
    ...
    const char16_t* expected = u"24,00 lei romÃ¢neÈ™ti";
...
void NumberFormatTest::Test20961_CurrencyPluralPattern() {
    IcuTestErrorCode status(*this, "Test20961_CurrencyPluralPattern");
    {
        ...
        assertEquals("Currency pattern", u"#,##0.00 Â¤Â¤Â¤", result);
    }
}
...
void NumberFormatTest::Test13733_StrictAndLenient() {
    ...
    } cases[] = { {u"CA$ 12", u"Â¤ 0", 12, 12},
                  {u"CA$12", u"Â¤0", 12, 12},
                  {u"CAD 12", u"Â¤Â¤ 0", 12, 12},
                  {u"12 CAD", u"0 Â¤Â¤", 12, 12},
                  {u"12 Canadian dollars", u"0 Â¤Â¤Â¤", 12, 12},
                  {u"$12 ", u"Â¤Â¤Â¤Â¤0", 12, 12},
                  {u"12$", u"0Â¤Â¤Â¤Â¤", 12, 12},
                  {u"CA$ 12", u"Â¤0", 0, 12},
                  {u"CA$ 12", u"0 Â¤Â¤", 0, 12},
                  {u"CA$ 12", u"0 Â¤Â¤Â¤", 0, 12},
                  {u"CA$ 12", u"Â¤Â¤Â¤Â¤0", 0, 12},
                  {u"CA$ 12", u"0Â¤Â¤Â¤Â¤", 0, 12},
                  {u"CA$12", u"Â¤ 0", 0, 12},
                  {u"CA$12", u"Â¤Â¤ 0", 0, 12},
                  {u"CA$12", u"0 Â¤Â¤", 0, 12},
                  {u"CA$12", u"0 Â¤Â¤Â¤", 0, 12},
                  {u"CA$12", u"0Â¤Â¤Â¤Â¤", 0, 12},
                  {u"CAD 12", u"Â¤0", 0, 12},
                  {u"CAD 12", u"0 Â¤Â¤", 0, 12},
                  {u"CAD 12", u"0 Â¤Â¤Â¤", 0, 12},
                  {u"CAD 12", u"Â¤Â¤Â¤Â¤0", 0, 12},
                  {u"CAD 12", u"0Â¤Â¤Â¤Â¤", 0, 12},
                  {u"12 CAD", u"Â¤ 0", 0, 12},
                  {u"12 CAD", u"Â¤0", 0, 12},
                  {u"12 CAD", u"Â¤Â¤ 0", 0, 12},
                  {u"12 CAD", u"Â¤Â¤Â¤Â¤0", 0, 12},
                  {u"12 CAD", u"0Â¤Â¤Â¤Â¤", 0, 12},
                  {u"12 Canadian dollars", u"Â¤ 0", 0, 12},
                  {u"12 Canadian dollars", u"Â¤0", 0, 12},
                  {u"12 Canadian dollars", u"Â¤Â¤ 0", 0, 12},
                  {u"12 Canadian dollars", u"Â¤Â¤Â¤Â¤0", 0, 12},
                  {u"12 Canadian dollars", u"0Â¤Â¤Â¤Â¤", 0, 12},
                  {u"$12 ", u"Â¤ 0", 0, 12},
                  {u"$12 ", u"Â¤Â¤ 0", 0, 12},
                  {u"$12 ", u"0 Â¤Â¤", 0, 12},
                  {u"$12 ", u"0 Â¤Â¤Â¤", 0, 12},
                  {u"$12 ", u"0Â¤Â¤Â¤Â¤", 0, 12},
                  {u"12$", u"Â¤ 0", 0, 12},
                  {u"12$", u"Â¤0", 0, 12},
                  {u"12$", u"Â¤Â¤ 0", 0, 12},
                  {u"12$", u"0 Â¤Â¤", 0, 12},
                  {u"12$", u"0 Â¤Â¤Â¤", 0, 12},
                  {u"12$", u"Â¤Â¤Â¤Â¤0", 0, 12} };
    for (const auto& cas : cases) {
...
void NumberFormatTest::Test10997_FormatCurrency() {
    ...
    assertEquals("minFrac 4 should be respected in different currency", u"â‚¬123.4500", str2);
}
...
void NumberFormatTest::Test21556_CurrencyAsDecimal() {
    ...
    {
        DecimalFormat df(u"a0Â¤00b", status);
        if (status.errDataIfFailureAndReset()) {
            return;
        }
        df.setCurrency(u"EUR", status);
        ...
        assertEquals("Basic test: format", u"a3â‚¬14b", result);
        UnicodeString pattern;
        assertEquals("Basic test: toPattern", u"a0Â¤00b", df.toPattern(pattern));
...

    {
        ...
        DecimalFormat* df = dynamic_cast<DecimalFormat*>(nf.getAlias());
        df->applyPattern(u"a0Â¤00b", status);
        UnicodeString result;
        FieldPosition fp(UNUM_CURRENCY_FIELD);
        df->format(3.141, result, fp);
        assertEquals("Via applyPattern: format", u"a3Â£14b", result);
        UnicodeString pattern;
        assertEquals("Via applyPattern: toPattern", u"a0Â¤00b", df->toPattern(pattern));
