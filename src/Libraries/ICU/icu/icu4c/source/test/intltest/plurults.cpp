...
void PluralRulesTest::testGetOrAddSamplesFromString() {
    UErrorCode status = U_ZERO_ERROR;
    UnicodeString description(u"testkeyword: e != 0 @decimal 2.0c6~4.0c6, â€¦");
...
void PluralRulesTest::testGetOrAddSamplesFromStringCompactNotation() {
    UErrorCode status = U_ZERO_ERROR;
    UnicodeString description(u"testkeyword: e != 0 @decimal 2.0~4.0, â€¦");
    LocalPointer<PluralRules> rules(PluralRules::createRules(description, status));
    if (U_FAILURE(status)) {
        errln("Couldn't create plural rules from a string, with error = %s", u_errorName(status));
        return;
...
void PluralRulesTest::testSamplesWithExponent() {
    ...
    UnicodeString description(
        ...
        u" @integer 1000000, 2e6, 3e6, 4e6, 5e6, 6e6, 7e6, â€¦ @decimal 2.1e6, 3.1e6, 4.1e6, 5.1e6, 6.1e6, 7.1e6, â€¦; "
        u"other:  @integer 2~17, 100, 1000, 10000, 100000, 2e5, 3e5, 4e5, 5e5, 6e5, 7e5, â€¦"
        u" @decimal 2.0~3.5, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0, 2.1e5, 3.1e5, 4.1e5, 5.1e5, 6.1e5, 7.1e5, â€¦"
    );
    ...
    checkNewSamples(description, test, u"many", u"@integer 1000000, 2e6, 3e6, 4e6, 5e6, 6e6, 7e6, â€¦", DecimalQuantity::fromExponentString(u"1000000", status));
    checkNewSamples(description, test, u"other", u"@integer 2~17, 100, 1000, 10000, 100000, 2e5, 3e5, 4e5, 5e5, 6e5, 7e5, â€¦", DecimalQuantity::fromExponentString(u"2", status));
    ...
    UnicodeString description2(
        ...
        u"many: e = 0 and i != 0 and i % 1000000 = 0 and v = 0 or e != 0..5"
        u" @decimal 2.1e6, 3.1e6, 4.1e6, 5.1e6, 6.1e6, 7.1e6, â€¦; "
        u"other:  @decimal 2.0~3.5, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0, 2.1e5, 3.1e5, 4.1e5, 5.1e5, 6.1e5, 7.1e5, â€¦"
    );
    ...
    checkNewSamples(description2, test2, u"many", u"@decimal 2.1e6, 3.1e6, 4.1e6, 5.1e6, 6.1e6, 7.1e6, â€¦", DecimalQuantity::fromExponentString(u"2.1c6", status));
    checkNewSamples(description2, test2, u"other", u"@decimal 2.0~3.5, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0, 2.1e5, 3.1e5, 4.1e5, 5.1e5, 6.1e5, 7.1e5, â€¦", DecimalQuantity::fromExponentString(u"2.0", status));
}
...
void PluralRulesTest::testSamplesWithCompactNotation() {
    ...
    UnicodeString description(
        ...
        u" @integer 1000000, 2c6, 3c6, 4c6, 5c6, 6c6, 7c6, â€¦ @decimal 2.1c6, 3.1c6, 4.1c6, 5.1c6, 6.1c6, 7.1c6, â€¦; "
        u"other:  @integer 2~17, 100, 1000, 10000, 100000, 2c5, 3c5, 4c5, 5c5, 6c5, 7c5, â€¦"
        u" @decimal 2.0~3.5, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0, 2.1c5, 3.1c5, 4.1c5, 5.1c5, 6.1c5, 7.1c5, â€¦"
    );
    LocalPointer<PluralRules> test(PluralRules::createRules(description, status));
    if (U_FAILURE(status)) {
        errln("Couldn't create plural rules from a string using exponent notation, with error = %s", u_errorName(status));
    ...
    checkNewSamples(description, test, u"many", u"@integer 1000000, 2c6, 3c6, 4c6, 5c6, 6c6, 7c6, â€¦", DecimalQuantity::fromExponentString(u"1000000", status));
    checkNewSamples(description, test, u"other", u"@integer 2~17, 100, 1000, 10000, 100000, 2c5, 3c5, 4c5, 5c5, 6c5, 7c5, â€¦", DecimalQuantity::fromExponentString(u"2", status));
    ...
    UnicodeString description2(
        ...
        u"many: c = 0 and i != 0 and i % 1000000 = 0 and v = 0 or c != 0..5"
        u" @decimal 2.1c6, 3.1c6, 4.1c6, 5.1c6, 6.1c6, 7.1c6, â€¦; "
        u"other:  @decimal 2.0~3.5, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0, 2.1c5, 3.1c5, 4.1c5, 5.1c5, 6.1c5, 7.1c5, â€¦"
    );
    LocalPointer<PluralRules> test2(PluralRules::createRules(description2, status));
    if (U_FAILURE(status)) {
        errln("Couldn't create plural rules from a string using exponent notation, with error = %s", u_errorName(status));
    ...
    checkNewSamples(description2, test2, u"many", u"@decimal 2.1c6, 3.1c6, 4.1c6, 5.1c6, 6.1c6, 7.1c6, â€¦", DecimalQuantity::fromExponentString(u"2.1c6", status));
    checkNewSamples(description2, test2, u"other", u"@decimal 2.0~3.5, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0, 2.1c5, 3.1c5, 4.1c5, 5.1c5, 6.1c5, 7.1c5, â€¦", DecimalQuantity::fromExponentString(u"2.0", status));
}
...
PluralRulesTest::testScientificPluralKeyword() {
    ...
    LocalPointer<PluralRules> rules(PluralRules::createRules(
        ...
        u"  @decimal 2.0~3.5, 10.0, 100.0, 1000.0, 10000.0, 100000.0, 1000000.0, â€¦", errorCode));
