...
void IntlTestSpoof::testSpoofAPI() {
    ...
    TEST_SETUP
        ...
        const UnicodeString behrooz(u"Ø¨Ù‡Ø±ÙˆØ²");
        const UnicodeString update(u"Ø¨Ù‡â€ŒØ±ÙˆØ²");
...
void IntlTestSpoof::testBidiSkeleton() {
    TEST_SETUP
    CHECK_BIDI_SKELETON(u"A1<×©×‚", UBIDI_LTR, u"Al<×©\u0307");
    CHECK_BIDI_SKELETON(u"Î‘×©Öº>1", UBIDI_LTR, u"Al<×©\u0307");
    CHECK_BIDI_SKELETON(u"A1<×©×‚", UBIDI_RTL, u"×©\u0307>Al");
    CHECK_BIDI_SKELETON(u"Î‘×©Öº>1", UBIDI_RTL, u"l<×©\u0307A");
    TEST_TEARDOWN;
}
...
void IntlTestSpoof::testAreBidiConfusable() {
    TEST_SETUP
        ...
        // The following string has RLMs around the 2â€“, flipping it; it uses an
        // EN DASH instead of the HYPHEN-MINUS above.
        const UnicodeString j2Dash(u"J\u200F2\u2013\u200F");
        TEST_ASSERT(j2Dash == u"Jâ€2â€“â€");
...
void IntlTestSpoof::testBug13314_MixedNumbers() {
    ...
    TEST_ASSERT_SUCCESS(status);
    int32_t failedChecks = uspoof_areConfusableUnicodeString(sc.getAlias(), u"ï¦œ", u"åˆ—", &status);
    TEST_ASSERT_SUCCESS(status);
    assertEquals("The CJK strings should be confusable", USPOOF_SINGLE_SCRIPT_CONFUSABLE, failedChecks);
    failedChecks = uspoof_check2UnicodeString(sc.getAlias(), u"3Èœ", nullptr, &status);
    TEST_ASSERT_SUCCESS(status);
