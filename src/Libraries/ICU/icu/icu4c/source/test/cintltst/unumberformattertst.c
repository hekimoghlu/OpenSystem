...
// rdar://143383524 ([Numerals]?: BL: CrystalE22E190a: Suggesting to keep Western Arabic numerals (1,2,3...) as system default for Bangla instead of native numerals)
    ...
    if (assertSuccess("Formatting end-to-end 1", &ec)) {
        assertUEquals("Should produce a result in Bangla digits", u"à§§à§¦,à§¦à§¦,à§¦à§¦à§­", str);
    }
...
static void TestSimpleNumberFormatterFull(void) {
    ...
    if (assertSuccess("Formatting end-to-end 2", &ec)) {
        assertUEquals("Should produce a result with Swiss symbols", u"4â€™321", str);
    }
    ...
    if (assertSuccess("Formatting end-to-end 3", &ec)) {
        assertUEquals("Should produce a result with mutated number", u"+0â€™007.600", str);
    }
...
static void TestNegativeDegrees(void) {
    ...
    TestCase testCases[] = {
        { u"measure-unit/temperature-celsius unit-width-short",               0,  u"0Â°C" },
        { u"measure-unit/temperature-celsius unit-width-short usage/default", 0,  u"32Â°F" },
        { u"measure-unit/temperature-celsius unit-width-short usage/weather", 0,  u"32Â°F" },

        { u"measure-unit/temperature-celsius unit-width-short",               -1, u"-1Â°C" },
        { u"measure-unit/temperature-celsius unit-width-short usage/default", -1, u"30Â°F" },
        { u"measure-unit/temperature-celsius unit-width-short usage/weather", -1, u"30Â°F" }
    };
