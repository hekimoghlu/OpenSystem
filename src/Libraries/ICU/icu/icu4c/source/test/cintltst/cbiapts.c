...
static const UChar koTest[] = { /* 00 */ 0xBAA8, 0xB4E0, 0x0020, 0xC778, 0xB958, 0x0020, 0xAD6C, 0xC131, 0xC6D0, 0xC758,
                                ...
                                /* 30 */ 0x7B97, 0x673A, 0x53EA, 0x662F, 0x5904, 0x7406, 0x6570, 0x5B57, 0x3002, 0 }; // åŸºæœ¬ä¸Šï¼Œè®¡ç®—æœºåªæ˜¯å¤„ç†æ•°å­—ã€‚
...
//                                                "Wenn ich schon hÃ¶re zu Guttenberg kommt evtl. zurÃ¼ck."
...
//                                                "TrÃ¨s bonne prise de parole de M. Junod, municipal Ã  la culture de Lausanne."
...
"4â€‘inch phone.\n
3ì›” 30ì¼\n
ä»Žä½ å¼€å§‹ä½¿ç”¨ iPhone 6s çš„é‚£ä¸€åˆ»èµ·ï¼Œä½ å°±ä¼šæ„Ÿè§‰åˆ°å®ƒæ˜¯å¦‚æ­¤ä¸åŒã€‚\n""
*/
...
static const RBTokResult expTokMiscCFST[] = { // 8 tokens
    ...
    { {  14,  2}, 0x1110     }, // 3ì›”
    { {  17,  3}, 0x1110     }, // 30ì¼
    { {  21,  6}, 0x40000000 }, // ä»Žä½ å¼€å§‹ä½¿ç”¨
    { {  28,  6}, 0x04       }, // iPhone
    { {  35,  2}, 0x10       }, // 6s
    { {  38, 19}, 0x40000121 }, // çš„é‚£ä¸€åˆ»èµ·ï¼Œä½ å°±ä¼šæ„Ÿè§‰åˆ°å®ƒæ˜¯å¦‚æ­¤ä¸åŒã€‚
};
...
static void* dataBufFromFile(const char* relative_path, long* dataBufSizeP) {
    ...
    // where the intltest binary is, so we can use a relative path.
...
static void TestTokenizerThaiHang(void) {
    ...
    if (assertSuccess("Failed to create tokenizer", &err)) {
        ubrk_setText(tok, u"à¹„à¸—à¸¢.à¹„à¸—à¸¢", -1, &err);
