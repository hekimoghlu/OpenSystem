...
// - warning: format â€˜%Sâ€™ expects argument of type â€˜wchar_t *â€™, but argument 4 has type â€˜char *â€™
// - warning: unknown conversion type character â€˜bâ€™ in format
//
...
test_trunc_string_mbyte(void)
{
    ...
    buf = alloc(40);
    s = vim_strsave((char_u *)"Ã„ text tha just fits");
    trunc_string(s, buf, 20, 40);
    assert(STRCMP(buf, "Ã„ text tha just fits") == 0);
    vim_free(buf);
    vim_free(s);

    buf = alloc(40);
    s = vim_strsave((char_u *)"a text Ã„Ã–ÃœÃ¤ nott fits");
    trunc_string(s, buf, 20, 40);
    assert(STRCMP(buf, "a text Ã„...nott fits") == 0);
    vim_free(buf);
    vim_free(s);

    buf = alloc(40);
    s = vim_strsave((char_u *)"a text that not fitsÃ„");
    trunc_string(s, buf, 20, 40);
    assert(STRCMP(buf, "a text t...not fitsÃ„") == 0);
    vim_free(buf);
    vim_free(s);
}
...
test_vim_snprintf(void)
{
	...
	n = vim_snprintf(buf, bsize, "%s", "æ¼¢èªž");
	assert(n == 6);
	assert(bsize == 0 || STRNCMP(buf, "æ¼¢èªž", bsize_int) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize_int)] == '\0');

	n = vim_snprintf(buf, bsize, "%8s", "æ¼¢èªž");
	assert(n == 8);
	assert(bsize == 0 || STRNCMP(buf, "  æ¼¢èªž", bsize_int) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize_int)] == '\0');
	...
	n = vim_snprintf(buf, bsize, "%.3s", "æ¼¢èªž");
	assert(n == 3);
	assert(bsize == 0 || STRNCMP(buf, "æ¼¢", bsize_int) == 0);
	assert(bsize == 0 || buf[MIN(n, bsize_int)] == '\0');
