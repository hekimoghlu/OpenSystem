...
int
generic_draw_header(struct statistic *s, int x, int y, int anchor)
{
	if (s->header) {
...
void
generic_draw_aligned(struct statistic *s, int x)
{
	int y = 0;
	...
	for (i = 0; i < cells->length && y < maxy; ++i, ++y) {
		...
		if (ERR == mvwaddnstr(s->window, y, localx, cells->array[i].string, len)) {
			int erry, errx;
			...
			top_log("error info: s->header %s y %d x %d n %d string: %s\n"
					...
					s->header, y, localx + x, len, cells->array[i].string, errx, erry);
		}
	}
}

void
generic_draw(struct statistic *s, int x)
{
	int y = 0;
	size_t i;
	struct generic_cells *cells;
	...
	for (i = 0; i < cells->length && y < maxy; ++i, ++y) {
		generic_draw_extended(
				s, x, y, GENERIC_DRAW_LEFT, cells->array[i].string, cells->array[i].length);
	}
}
...
void
generic_draw_extended(
		struct statistic *s, int xoffset, int yoffset, int anchor, const char *string, int slen)
{
...
void
generic_draw_centered(struct statistic *s, int x)
{
	struct generic_cells *cells;
	size_t i;
	int y = 0;
	...
	for (i = 0; i < cells->length; ++i) {
		generic_draw_extended(
				s, x, y, GENERIC_DRAW_CENTERED, cells->array[i].string, cells->array[i].length);
		++y;
	}
}

void
generic_draw_right(struct statistic *s, int x)
{
	struct generic_cells *cells;
	size_t i;
	int y = 0;
	...
	for (i = 0; i < cells->length; ++i) {
		generic_draw_extended(
				s, x, y, GENERIC_DRAW_RIGHT, cells->array[i].string, cells->array[i].length);
		++y;
	}
}

bool
generic_resize_cells(struct statistic *s, struct statistic_size *size)
{
...
bool
generic_move_cells(struct statistic *s, int x, int y)
{
...
void
generic_get_request_size(struct statistic *s)
{
	struct generic_cells *cells;
...
static struct generic_cells *
alloc_generic_cells(void)
{
	size_t i, length = 10;
	struct generic_cells *cells;
...
static void
free_generic_cells(struct generic_cells *cells)
{
	size_t i;
...
static void
generic_cell_destructor(struct statistic *s, void *ptr)
{
...
bool
generic_insert_cell(struct statistic *s, const char *sample)
{
	...
	if (cells->length >= cells->length_allocated) {
		...
		size_t i;
		cells->array = realloc(cells->array, sizeof(*(cells->array)) * newlength);
	...
	if (cells->array[offset].string && cells->array[offset].allocated_length >= sample_z_length) {
...
void
generic_reset_insertion(struct statistic *s)
{
	struct generic_cells *cells;
...
void
generic_get_minimum_size(struct statistic *s)
{
	struct generic_cells *cells;
