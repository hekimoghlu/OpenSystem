...
static bool
csw_insert_cell(struct statistic *s, const void *sample)
{
	const libtop_psamp_t *psamp = sample;
	char buf[GENERIC_INT_SIZE];

	if (top_uinteger_format_result(
				buf, sizeof(buf), psamp->csw.now, psamp->csw.previous, psamp->csw.began)) {
		return true;
	}
...
struct statistic *
top_csw_create(WINDOW *parent, const char *name)
{
	return create_statistic(STATISTIC_CSW, parent, NULL, &callbacks, name);
}
