...
libtop_p_disks_sample(void)
{
	...
	while ((drive = IOIteratorNext(drive_list)) != 0) {
		...
		if (IORegistryEntryCreateCFProperties(
					drive, (CFMutableDictionaryRef *)&properties, kCFAllocatorDefault, 0)) {
			libtop_print(libtop_user_data, "Error in IORegistryEntryCreateCFProperties()");
			retval = -1;
...
libtop_p_proc_table_read(boolean_t reg)
{
	...
	if (kr != KERN_SUCCESS) {
		libtop_print(libtop_user_data, "Error in host_processor_sets(): %s", mach_error_string(kr));
		return -1;
	}
	...
	for (i = 0; i < pcnt; i++) {
		...
		if (kr != KERN_SUCCESS) {
			libtop_print(libtop_user_data, "Error in host_processor_set_priv(): %s",
		...
		kr = mach_vm_deallocate(
				mach_task_self(), (mach_vm_address_t)(uintptr_t)tasks, tcnt * sizeof(*tasks));
		kr = mach_port_deallocate(mach_task_self(), pset);
		kr = mach_port_deallocate(mach_task_self(), psets[i]);
	}

	kr = mach_vm_deallocate(
			mach_task_self(), (mach_vm_address_t)(uintptr_t)psets, pcnt * sizeof(*psets));
...
static int __attribute__((noinline)) kinfo_for_pid(struct kinfo_proc *kinfo, pid_t pid)
{
	size_t miblen = 4, len;
	int mib[miblen];
	int res;
...
#define TASK_POLICY_STATE_COUNT                                                                    \
	((mach_msg_type_number_t)(sizeof(struct task_policy_state) / sizeof(integer_t)))
...
libtop_pinfo_update_jetsam_info(task_t task, libtop_pinfo_t *pinfo)
{
	...
	ssize_t size = memorystatus_control(
			MEMORYSTATUS_CMD_GET_PRIORITY_LIST, pinfo->psamp.pid, 0, &entry, sizeof(entry));
	...
	*state = LIBTOP_STATE_MAX;
	...
	for (i = 0; i < tcnt; i++) {
		...
		if ((info.flags & (TH_FLAGS_IDLE | TH_FLAGS_GLOBAL_FORCED_IDLE)) == 0) {
			pinfo->psamp.total_timens
					+= TIME_VALUE_TO_NS(&info.user_time) + TIME_VALUE_TO_NS(&info.system_time);
		}
	...
	kr = mach_vm_deallocate(
			mach_task_self(), (mach_vm_address_t)(uintptr_t)threads, tcnt * sizeof(*threads));
	return kr;
}
...
libtop_update_vm_regions(task_read_t task, libtop_pinfo_t *pinfo)
{
	...
	fprintf(stderr,
			"range\tsize\tprivate\tshared\trc\toid\tmode\tvprvt\trprvt\trshrd\tempty\taliased\n");
#endif
	...
	for (addr = 0;; addr += size) {
...
#ifdef TOP_DEBUG_VM
		fprintf(stderr, "%016llx-%016llx\t%lld\t%d\t%d\t%d\t%d\t%s", addr, addr + size, size,
				info.private_pages_resident, info.shared_pages_resident, info.ref_count,
				info.obj_id, share_mode_to_string(info.share_mode));
#endif
		...
		case SM_SHARED:
			...
			oinfo = libtop_p_oinfo_insert(info.obj_id, info.share_mode, info.shared_pages_resident,
...
libtop_p_task_update(task_t task, boolean_t reg)
{
	...
	pinfo->psamp.total_timens = TIME_VALUE_TO_NS(&ti.user_time) + TIME_VALUE_TO_NS(&ti.system_time);
...
libtop_p_pinsert(libtop_pinfo_t *pinfo)
{
	...
	rb_insert(&libtop_ptree, pinfo, libtop_p_pinfo_pid_comp, libtop_pinfo_t, pnode);
}
...
libtop_p_oinfo_insert(int obj_id, int share_type, int resident_page_count, int ref_count, int size,
		libtop_pinfo_t *pinfo)
{
...
libtop_p_wq_update(libtop_pinfo_t *pinfo)
{
	...
	result = proc_pidinfo(pinfo->psamp.pid, PROC_PIDWORKQUEUEINFO,
			/*arg is UNUSED with this flavor*/ 0ULL, &wqinfo, PROC_PIDWORKQUEUEINFO_SIZE);
	if (!result)
		return -1; /*error*/
...
libtop_i64_test(void)
{
	...
	while (1) {
		fprintf(stderr, "test: i %d value %d i64 value %" PRIu64 " matches? %" PRIu64 "\n", i,
				value, libtop_i64_value(&i64), accum);
		fprintf(stderr, "+ %d\n", incr);
		value += incr;
		accum += incr;
