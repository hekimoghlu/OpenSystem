...
#ifndef lf_hfs_journal_h
#define lf_hfs_journal_h
...
typedef struct journal_header {
    ...
    uint32_t       blhdr_size;    // size in bytes of each block_list_header in the journal
    uint32_t       checksum;
...
typedef struct journal {
    pthread_mutex_t     jlock;             // protects the struct journal data
    pthread_mutex_t     flock;             // serializes flushing of journal
    pthread_rwlock_t    trim_lock;         // protects the async_trim field, below
    
    struct vnode       *jdev;              // vnode of the device where the journal lives
    off_t               jdev_offset;       // byte offset to the start of the journal
    uint32_t            jdev_blknum;       // Physical block number of the journal
    ...
    int32_t             flags;
    uint32_t            tbuffer_size;      // default transaction buffer size
    ConditionalFlag_S   flushing;
    ConditionalFlag_S   asyncIO;
    ConditionalFlag_S   writing_header;
    ...
    pthread_mutex_t     old_start_lock;    // protects the old_start
...
/*
 * Call journal_release() to release all buffers held by the journal.
 * This is used incase of live-files unmount, since the media is no longer
...
int   journal_modify_block_end(journal *jnl, GenericLFBuf *psGenBuf, void (*func)(GenericLFBuf *bp, void *arg), void *arg);
int   journal_kill_block(journal *jnl, GenericLFBuf *bp);
