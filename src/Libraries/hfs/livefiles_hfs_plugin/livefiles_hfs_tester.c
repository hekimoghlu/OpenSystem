...
static int HFSTest_ScanDir(UVFSFileNode RootNode )
{
    ...
    //Create new file with size 0
    iErr = CreateNewFile(RootNode, &TestFile1, "FðŸ¤ª2",0);
    printf("Create TestFile in TestFolder err [%d]\n", iErr);
    if (iErr) goto exit;

    UVFSFileAttributes sOutAttrs;
    iErr = HFS_fsOps.fsops_getattr(TestFile1, &sOutAttrs);
    printf("fsops_getattr FðŸ¤ª2 err [%d]\n",iErr);
    if (iErr) goto exit;
    ...
    // Remove File
    iErr =  RemoveFile(RootNode,"FðŸ¤ª2");
    printf("Remove File FðŸ¤ª2 from TestFolder err [%d]\n", iErr);
    if (iErr) goto exit;
    // Remove Folders
