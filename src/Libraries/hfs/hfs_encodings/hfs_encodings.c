...
void hfs_converterdone(void)
{
	if (encodinglst_lck_grp_attr)
		lck_grp_attr_free(encodinglst_lck_grp_attr);
	if (encodinglst_lck_grp)
		lck_grp_free(encodinglst_lck_grp);
	if (encodinglst_lck_attr)
		lck_attr_free(encodinglst_lck_attr);
}
...
hfs_addconverter(int id, u_int32_t encoding, hfs_to_unicode_func_t get_unicode, unicode_to_hfs_func_t get_hfsname)
{
	...
	if (id)
		OSKextRetainKextWithLoadTag(id);
 ...
 */
int unicode_to_mac_roman(UniChar *uni_str, u_int32_t unicodeChars, Str31 hfs_str)
{
	u_int8_t	*p;
	const UniChar	*u;
	UniChar		c;
