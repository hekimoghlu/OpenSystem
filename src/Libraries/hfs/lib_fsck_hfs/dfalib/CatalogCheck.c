...
printSymLinkName(SGlobPtr GPtr, UInt32 fid)
{
	...
	if (GetFileNamePathByID(GPtr, fid, pathname, &path_len, filename, &fname_len, &status) == 0) {
		fsckPrintFormat(GPtr->context, E_BadSymLinkName, pathname);
	}
	return;
}
...
#define	HFS_JOURNAL_INFO	".journal_info_block"
	...
	if (gCIS.dirCount != gCIS.dirThreads) {
		...
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "\t%s: dirCount = %u, dirThread = %u\n", __FUNCTION__, gCIS.dirCount, gCIS.dirThreads);
		}
	}
	...
	if (hfsplus && (gCIS.fileCount != gCIS.fileThreads)) {
		...
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "\t%s: fileCount = %u, fileThread = %u\n", __FUNCTION__, gCIS.fileCount, gCIS.fileThreads);
		}
	}
	...
	if (!hfsplus && (gCIS.fileThreads != gCIS.filesWithThreads)) {
		...
		gScavGlobals->CBTStat |= S_Orphan;
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "\t%s: fileThreads = %u, filesWithThread = %u\n", __FUNCTION__, gCIS.fileThreads, gCIS.filesWithThreads);
		}
	}
...
#if DEBUG_XATTR
	fsck_print(ctx, LOG_TYPE_INFO, "%s: Record folderID=%d for prime modulus calculations\n", __FUNCTION__, dir->folderID);
#endif
...
CheckFile(const HFSPlusCatalogKey * key, const HFSPlusCatalogFile * file)
{
...
#if DEBUG_XATTR
	fsck_print(ctx, LOG_TYPE_INFO, "%s: Record fileID=%d for prime modulus calculations\n", __FUNCTION__, file->fileID); 
#endif
	...
	if (islink == 0) {
		if (file->flags & kHFSHasLinkChainMask &&
			...
			RepairOrderPtr p;
			fsckPrintFormat(gScavGlobals->context, E_LinkChainNonLink, file->fileID);
		...
		if (((file->bsdInfo.fileMode & S_IFMT) == S_IFREG) &&
			...
			RepairOrderPtr p;
			char badstr[16];
			fsckPrintFormat(gScavGlobals->context, E_FileLinkCountError, file->fileID);
			snprintf(badstr, sizeof(badstr), "%u", file->bsdInfo.special.linkCount);
			fsckPrintFormat(gScavGlobals->context, E_BadValue, "1", badstr);
		 ...
		 */
		if ((state.cur_debug_level & d_check_slink) != 0) {
			if (((file->bsdInfo.fileMode & S_IFMT) == S_IFLNK) ||
				...
				if (((file->bsdInfo.fileMode & S_IFMT) != S_IFLNK) ||
				    file->userInfo.fdType != kSymLinkFileType ||
				    file->userInfo.fdCreator != kSymLinkCreator) {
					fsckPrintFormat(gScavGlobals->context, E_BadSymLink, file->fileID);
					// Should find a way to print out the path, no?
				}
				if (file->dataFork.logicalSize > PATH_MAX) {
					fsckPrintFormat(gScavGlobals->context, E_BadSymLinkLength, file->fileID, (unsigned int)file->dataFork.logicalSize, (unsigned int)PATH_MAX);
					...
					if (dataBuffer == NULL) {
                        if (state.debug)
                            fsck_print(ctx, LOG_TYPE_INFO, "Unable to allocate %llu bytes for reading symlink", file->dataFork.logicalSize);
					} else {
						...
						if (dataLen != file->dataFork.logicalSize) {
							fsckPrintFormat(gScavGlobals->context, E_BadSymLinkLength, file->fileID, (unsigned int)dataLen, (unsigned int)file->dataFork.logicalSize);
							printSymLinkName(gScavGlobals, file->fileID);
							if (state.debug)
								fsck_print(ctx, LOG_TYPE_INFO, "Symlink for file id %u has bad data length\n", file->fileID);
						}
						free(dataBuffer);
					}
				}
	...
	if (islink == 1 && file->dataFork.totalBlocks != 0) {
		fsckPrintFormat(gScavGlobals->context, E_LinkHasData, file->fileID);
...
CheckFile_HFS(const HFSCatalogKey * key, const HFSCatalogFile * file)
{
	...
	if (file->dataPhysicalSize > ((UInt64)blocks * (UInt64)gScavGlobals->calculatedVCB->vcbBlockSize)) {
		...
		fsckPrintFormat(gScavGlobals->context, E_PEOF, idstr);
		return (noErr);		/* we don't fix this, ignore the error */
	}
	if (file->dataLogicalSize > file->dataPhysicalSize) {
		snprintf (idstr, sizeof(idstr), "id=%u", fileID);
		fsckPrintFormat(gScavGlobals->context, E_LEOF, idstr);
                return (noErr);		/* we don't fix this, ignore the error */
	}
...
CheckCatalogName(u_int16_t charCount, const u_int16_t *uniChars, u_int32_t parentID, Boolean thread)
{
    ...
    if ( charCount < 3 && *uniChars == 0x2E )
    {
        ...
        {
		fsckPrintFormat(gScavGlobals->context, E_IllegalName);
            if ( fsck_get_verbosity_level() >= kDebugLog ) {
               	fsck_print(ctx, LOG_TYPE_INFO, "\tillegal name is 0x" );
            ...
            *myPtr++ = newName.ustr.length; // copy in length of new name and bump past it
            ...
            if ( fsck_get_verbosity_level() >= kDebugLog ) {
               	fsck_print(ctx, LOG_TYPE_INFO, "\treplacement name is 0x" );
...
RecordBadAllocation(UInt32 parID, unsigned char * filename, UInt32 forkType, UInt32 oldBlkCnt, UInt32 newBlkCnt)
{
	...
	if (forkType == kEAData) {
		...
		fsckPrintFormat(gScavGlobals->context, E_PEOAttr, filename, real_filename);
		free(real_filename);
	} else {
        fsckPrintFormat(gScavGlobals->context, E_PEOF, filename);
	}
	sprintf(goodstr, "%d", newBlkCnt);
	sprintf(badstr, "%d", oldBlkCnt);
    fsckPrintFormat(gScavGlobals->context, E_BadValue, goodstr, badstr);
...
RecordTruncation(UInt32 parID, unsigned char * filename, UInt32 forkType, UInt64 oldSize, UInt64 newSize)
{
	...
	if (forkType == kEAData) {
		...
		fsckPrintFormat(gScavGlobals->context, E_LEOAttr, filename, real_filename);
		free(real_filename);
	} else {
		fsckPrintFormat(gScavGlobals->context, E_LEOF, filename);
	}
	sprintf(oldSizeStr, "%qd", oldSize);
	sprintf(newSizeStr, "%qd", newSize);
	fsckPrintFormat(gScavGlobals->context, E_BadValue, newSizeStr, oldSizeStr);
...
CaptureMissingThread(UInt32 threadID, const HFSPlusCatalogKey *nextKey)
{
	...
	fsckPrintFormat(gScavGlobals->context, E_NoThd, threadID);
