		...
		uint32_t incorrect, uint32_t correct)
{
	...
	fsckPrintFormat(gptr->context, isdir? E_DirLinkBadFlags : E_FileLinkBadFlags, link_id);
	snprintf(str1, sizeof(str1), "0x%x", correct);
	snprintf(str2, sizeof(str2), "0x%x", incorrect);
	fsckPrintFormat(gptr->context, E_BadValue, str1, str2);
		...
		uint32_t incorrect, uint32_t correct, Boolean check_duplicates)
{
	...
	if ((check_duplicates != 0) && 
		...
		fsckPrintFormat(gptr->context, isdir? E_DirInodeBadFlags : E_FileInodeBadFlags, inode_id);
		snprintf(str1, sizeof(str1), "0x%x", correct);
		snprintf(str2, sizeof(str2), "0x%x", incorrect);
		fsckPrintFormat(gptr->context, E_BadValue, str1, str2);
	}
		...
		uint32_t incorrect, uint32_t correct)
{
	...
	fsckPrintFormat(gptr->context, isdir? E_DirInodeBadParent : E_FileInodeBadParent, inode_id);
...
{
	fsckPrintFormat(gptr->context, E_DirInodeBadName, inode_id);
	fsckPrintFormat(gptr->context, E_BadValue, correct, incorrect);
			...
			uint8_t incorrect, uint8_t correct, int check_duplicates)
{
	...
	if ((check_duplicates != 0) && 
		...
		fsckPrintFormat(gptr->context, E_DirHardLinkOwnerFlags, file_id);
		snprintf(str1, sizeof(str1), "0x%x", correct);
		snprintf(str2, sizeof(str2), "0x%x", incorrect);
		fsckPrintFormat(gptr->context, E_BadValue, str1, str2);
	}
...
int record_link_badfinderinfo(SGlobPtr gptr, uint32_t file_id, Boolean isdir)
{
	...
	if (IsDuplicateRepairOrder(gptr, p) == 1) {
		...
		fsckPrintFormat(gptr->context, p->type, file_id);
	}
		...
		uint32_t incorrect, uint32_t correct)
{
	...
	if (IsDuplicateRepairOrder(gptr, p) == 1) {
		...
		fsckPrintFormat(gptr->context, E_DirLinkAncestorFlags, dir_id);
		snprintf(str1, sizeof(str1), "0x%x", correct);
		snprintf(str2, sizeof(str2), "0x%x", incorrect);
		fsckPrintFormat(gptr->context, E_BadValue, str1, str2);
	}
	...
	if (isdir) {
		...
		if (retval == 0) {
			...
			if (rec->recordType != kHFSPlusAttrInlineData) {
				if (fsck_get_verbosity_level() >= kDebugLog) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tfirst link EA is not inline for dirinode=%u (found=0x%x)\n", inode_id, rec->recordType);
				}
				retval = ENOENT;
				goto out;
			}
			...
			if (rec->attrData[rec->attrSize-1] != '\0') {
				if (fsck_get_verbosity_level() >= kDebugLog) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tfirst link EA attrData is not NULL terminated for dirinode=%u\n", inode_id);
				}
				retval = ENOENT;
				goto out;
			}
			...
			for (i = 0; i < rec->attrSize-1; i++) {
				if (isdigit(rec->attrData[i]) == 0) {
					if (fsck_get_verbosity_level() >= kDebugLog) {
						fsck_print(ctx, LOG_TYPE_INFO, "\tfirst link EA attrData contains non-digit 0x%x for dirinode=%u\n", rec->attrData[i], inode_id);
					}
					retval = ENOENT;
				goto out;
				}
            ...
            if (link_id > UINT32_MAX) {
                if (fsck_get_verbosity_level() >= kDebugLog) {
                    fsck_print(ctx, LOG_TYPE_INFO, "\tfirst link ID=%lu is > UINT32_MAX for dirinode=%u\n", link_id, inode_id);
                }
                *first_link_id = 0;
                retval = ENOENT;
                goto out;
			...
			*first_link_id = (uint32_t)link_id;
			if (*first_link_id < kHFSFirstUserCatalogNodeID) {
				if (fsck_get_verbosity_level() >= kDebugLog) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tfirst link ID=%u is < 16 for dirinode=%u\n", *first_link_id, inode_id);
				}
				*first_link_id = 0;
				retval = ENOENT;
			goto out;
		...
		*first_link_id = 0;
		if ((inode_rec != NULL) && 
			...
			*first_link_id = inode_rec->hfsPlusFile.hl_firstLinkID;
			if (*first_link_id < kHFSFirstUserCatalogNodeID) {
				if (fsck_get_verbosity_level() >= kDebugLog) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tfirst link ID=%u is < 16 for fileinode=%u\n", *first_link_id, inode_id);
				}
				*first_link_id = 0;
				retval = ENOENT;
				goto out;
		...
		CatalogRecord *rec, CatalogKey *key, Boolean isdir)
{
	...
	if (isdir) {
        ...
        if (ref_num > UINT32_MAX) {
            if (fsck_get_verbosity_level() >= kDebugLog) {
                fsck_print(ctx, LOG_TYPE_INFO, "\tlink reference num=%lu is > UINT32_MAX for inode=%u\n", ref_num, inode_id);
            }
            retval = 1;
            goto out;
        }
	...
	if (linkCount == 0) {
		...
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tlinkCount=0 for dirinode=%u\n", inode_id);
		}
		retval = 1;
		goto out;
	}
	...
	if (retval) {
		record_link_badchain(gptr, isdir);
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tError getting first link ID for inode=%u\n", inode_id);
		}
		goto out;
	}
	...
	while (cur_link_id != 0) {
		...
		if (retval) {
			record_link_badchain(gptr, isdir);
			if (fsck_get_verbosity_level() >= kDebugLog) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tError getting link=%u for inode=%u\n", cur_link_id, inode_id);
			}
			goto out;
		}
		...
		if (linkrec.recordType != kHFSPlusFileRecord) {
			record_link_badchain(gptr, isdir);
			if (fsck_get_verbosity_level() >= kDebugLog) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tIncorrect record type for link=%u for inode=%u (expected=2, found=%u)\n", cur_link_id, inode_id, linkrec.recordType);
			}
			retval = 1;
			goto out;
		}
		...
		if ((linkrec.hfsPlusFile.flags & kHFSHasLinkChainMask) == 0) {
			...
			if (fsck_get_verbosity_level() >= kDebugLog) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tIncorrect flag for link=%u for inode=%u (found=0x%x)\n", cur_link_id, inode_id, linkrec.hfsPlusFile.flags);
			}
			retval = 1;
			goto out;
		}
		...
		if (isdir) {
			...
			if ((linkrec.hfsPlusFile.userInfo.fdType != kHFSAliasType) ||
				...
				if (fsck_get_verbosity_level() >= kDebugLog) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tdirlink: fdType = 0x%08lx, fdCreator = 0x%08lx\n",
			...
			if (linkrec.hfsPlusFile.hl_linkReference != inode_id) {
				record_link_badchain(gptr, isdir);
				if (fsck_get_verbosity_level() >= kDebugLog) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tIncorrect dirinode ID for dirlink=%u (expected=%u, found=%u)\n", cur_link_id, inode_id, linkrec.hfsPlusFile.hl_linkReference);
				}
				retval = 1;
				goto out;
			}
			...
			if ((linkrec.hfsPlusFile.userInfo.fdType != kHardLinkFileType) ||
				...
				record_link_badfinderinfo(gptr, linkrec.hfsPlusFile.fileID, isdir);
				if (fsck_get_verbosity_level() >= kDebugLog) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tfilelink: fdType = 0x%08lx, fdCreator = 0x%08lx\n",
						(unsigned long)linkrec.hfsPlusFile.userInfo.fdType, 
						(unsigned long)linkrec.hfsPlusFile.userInfo.fdCreator);
				}
			}
			...
			if (linkrec.hfsPlusFile.hl_linkReference != link_ref_num) {
				record_link_badchain(gptr, isdir);
				if (fsck_get_verbosity_level() >= kDebugLog) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tIncorrect link reference number for filelink=%u (expected=%u, found=%u)\n", cur_link_id, inode_id, linkrec.hfsPlusFile.hl_linkReference);
				}
				retval = 1;
				goto out;
			}
		...
		if (prev_link_id != linkrec.hfsPlusFile.hl_prevLinkID) {
			record_link_badchain(gptr, isdir);
			if (fsck_get_verbosity_level() >= kDebugLog) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tIncorrect prevLinkID for link=%u for inode=%u (expected=%u, found=%u)\n", cur_link_id, inode_id, prev_link_id, linkrec.hfsPlusFile.hl_prevLinkID);
			}
			retval = 1;
			goto out;
		}
		...
		while (cur) {
			if (cur->link_id == cur_link_id) {
				if (fsck_get_verbosity_level() >= kDebugLog) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tDuplicate link=%u found in list for inode=%u\n", cur_link_id, inode_id);
				}
				record_link_badchain(gptr, isdir);
				retval = 1;
				goto out;
	...
	if (linkCount != count) {
		record_link_badchain(gptr, isdir);
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tIncorrect linkCount for inode=%u (expected=%u, found=%u)\n", inode_id, count, linkCount);
		}
		retval = 1;
		goto out;
	}
...
static void check_dirlink_ancestors(SGlobPtr gptr, uint32_t dir_id)
{
	...
	if ((dir_id != kHFSRootFolderID) && (dir_id != gptr->dirlink_priv_dir_id)) {
		fsckPrintFormat(gptr->context, E_BadParentHierarchy, dir_id);
		gptr->CBTStat |= S_Orphan;
	}
		...
		HFSPlusCatalogFile *rec, HFSPlusCatalogKey *key, Boolean isdir)
{
	...
	if (fsck_get_verbosity_level() >= kDebugLog)
		fsck_print(ctx, LOG_TYPE_INFO, "link_check:  adding <%u, %u>\n", rec->hl_linkReference, rec->fileID);
#endif
...
static void print_dfs(struct dfs_stack *dfs)
{
	...
	for (i = 0; i < dfs->depth; i++) {
		fsck_print(ctx, LOG_TYPE_INFO, "(%u,%u) ", dfs->idptr[i].inode_id, dfs->idptr[i].catalog_id);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "\n");
}
...
#define DFS_PEEK(dfsid) \
	...
	if (visited.list == NULL) {
		visited.size = 0;
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tcheck_loops: Allocation failed for visited list\n");
		}
	}
	visited.offset = 0;
	...
	while (dfs.depth > 1) {
		...
		if (child.inode_id) {
			...
			if (retval) {
				fsckPrintFormat(gptr->context, E_DirLoop);
				if (fsck_get_verbosity_level() >= kDebugLog) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tDetected when adding (%u,%u) to following traversal stack -\n", child.inode_id, child.catalog_id);
					print_dfs(&dfs);
				}
				gptr->CatStat |= S_LinkErrNoRepair;
				retval = E_DirLoop;
	...
	if (dfs.depth >= max_alloc_depth) {
		fsckPrintFormat(gptr->context, E_DirHardLinkNesting);
		if (fsck_get_verbosity_level() >= kDebugLog) {
			print_dfs(&dfs);
		}
		gptr->CatStat |= S_LinkErrNoRepair;
...
int dirhardlink_check(SGlobPtr gptr) 
{
	...
	fsckPrintFormat(gptr->context, hfsMultiLinkDirCheck);

	if (fsck_get_verbosity_level() >= kDebugLog) {
		fsck_print(ctx, LOG_TYPE_INFO, "\tprivdir_valence=%u, calc_dirlinks=%u, calc_dirinode=%u\n", gptr->dirlink_priv_dir_valence, gptr->calculated_dirlinks, gptr->calculated_dirinodes);
	}
	...
	if (gptr->dirlink_priv_dir_id == 0) {
		fsckPrintFormat(gptr->context, E_MissingPrivDir);
		gptr->CatStat |= S_LinkErrNoRepair;
	}
	...
	if ((gptr->CatStat & S_DirHardLinkChain) == 0) {
		...
		if (retval) {
			...
			if (fsck_get_verbosity_level() >= kDebugLog) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tdirlink prime buckets do not match\n");
			}
			retval = 0;
		}
	}
