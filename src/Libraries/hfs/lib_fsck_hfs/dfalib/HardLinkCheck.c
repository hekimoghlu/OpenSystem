...
CheckHardLinkList(SGlobPtr gp, UInt32 inodeID, struct HardLinkList *list, int calc_link_count, UInt32 firstID)
{
	...
	if (list == NULL) {
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tCheckHardLinkList: list=NULL for inodeID = %u\n", inodeID);
		}
		return ENOMEM;
	}
	/*
...
HardLinkCheckBegin(SGlobPtr gp, void** cookie)
{
	...
	if (info == NULL) {
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "hardLinkCheckBegin:  malloc(%zu) failed\n", sizeof(struct HardLinkInfo));
		}
		return 1;
	}
	...
	if (info->fileBucket == NULL) {
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "HardLinkCheckBegin: prime bucket allocation failed\n");
		}
	}
...
CaptureHardLink(void *cookie, const HFSPlusCatalogFile *file)
{
	...
	if ((info->priv_dir_itime && file->createDate != info->priv_dir_itime) &&
		...
		if (state.debug)
			fsck_print(ctx, LOG_TYPE_INFO, "Hard Link catalog entry %u has bad time %u (should be %u, or at least %u)\n",
		...
		if (p == NULL) {
			if (state.debug)
				fsck_print(ctx, LOG_TYPE_INFO, "Unable to allocate hard link time repair order!");
			return;
		}

		fsckPrintFormat(info->globals->context, E_BadHardLinkDate);
		snprintf(str1, sizeof(str1), "%u", correct);
		snprintf(str2, sizeof(str2), "%u", file->createDate);
		fsckPrintFormat(info->globals->context, E_BadValue, str1, str2);
...
RepairHardLinkChains(SGlobPtr gp, Boolean isdir)
{
	...
	if (metadirid == 0) {
		if (fsck_get_verbosity_level() >= kDebugLog) {
			if (isdir) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tPrivate directory for dirlinks not found.  Stopping repairs.\n");
			} else {
				fsck_print(ctx, LOG_TYPE_INFO, "\tPrivate directory for filelinks not found.  Stopping repairs.\n");
			}
		} 
		result = ENOENT;
		goto done;
	...
	if (linkInfo == NULL) {
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "RepairHardLinkChains:  calloc(%d, %zu) failed\n", slots, sizeof(struct IndirectLinkInfo));
		}
		result = ENOMEM;
		goto done;
	}
	...
	for (result = BTSearchRecord(fcb, &iterator, kInvalidMRUCacheKey, &btrec, &reclen, &iterator);
		...
		if (isdir) {
            ...
            if (ref_num < 0 || ref_num > UINT32_MAX) {
                result = EINVAL;
                if (fsck_get_verbosity_level() >= kDebugLog) {
                    fsck_print(ctx, LOG_TYPE_INFO, "\tLink reference num=%ld is invalid for inode=%u result=%d\n", ref_num, inodeID, result);
                }
                break;
            }
		...
		if (li) {
			...
			if (result != 0) {
				if (fsck_get_verbosity_level() >= kDebugLog)
					fsck_print(ctx, LOG_TYPE_INFO, "\tError getting first link ID for inode = %u (result=%d)\n", inodeID, result);
			}
...
CheckHardLinks(void *cookie)
{
	...
	gp = info->globals;
	fsckPrintFormat(gp->context, hfsHardLinkCheck);
	...
	if (catBucket == NULL) {
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "CheckHardLinks:  calloc(1, %zu) failed\n", sizeof(PrimeBuckets));
		}
		result = ENOMEM;
		goto exit;
	}
	...
	if ((result == 0) && (info->fileBucket != NULL)) {
		...
		if (result) {
			...
			if (fsck_get_verbosity_level() >= kDebugLog) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tfilelink prime buckets do not match\n");
			}
			goto exit;
		}
	}
	...
	if (filelink_entry_count) {
		 ...
		 */
		if (fsck_get_verbosity_level() >= kDebugLog) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tCheckHardLinks: found %u pre-Leopard file inodes.\n", filelink_entry_count);
		}
...
RecordOrphanLink(SGlobPtr gp, Boolean isdir, UInt32 linkID)
{
	...
	fsckPrintFormat(gp->context, isdir ? E_OrphanDirLink : E_OrphanFileLink, linkID);
...
RecordOrphanInode(SGlobPtr gp, Boolean isdir, UInt32 inodeID)
{
	...
	fsckPrintFormat(gp->context, isdir ? E_OrphanDirInode : E_OrphanFileInode, inodeID);
...
{
	fsckPrintFormat(gp->context, E_UnlinkedFile, filename);
...
RecordBadHardLinkChainFirst(SGlobPtr gp, UInt32 fileID, UInt32 is, UInt32 shouldbe)
{
	...
	fsckPrintFormat(gp->context, E_InvalidLinkChainFirst, fileID);
	sprintf(goodstr, "%u", shouldbe);
	sprintf(badstr, "%u", is);
	fsckPrintFormat(gp->context, E_BadValue, goodstr, badstr);
...
RecordBadHardLinkPrev(SGlobPtr gp, UInt32 fileID, UInt32 is, UInt32 shouldbe)
{
	...
	fsckPrintFormat(gp->context, E_InvalidLinkChainPrev, fileID);
	sprintf(goodstr, "%u", shouldbe);
	sprintf(badstr, "%u", is);
	fsckPrintFormat(gp->context, E_BadValue, goodstr, badstr);
...
RecordBadHardLinkNext(SGlobPtr gp, UInt32 fileID, UInt32 is, UInt32 shouldbe)
{
	...
	fsckPrintFormat(gp->context, E_InvalidLinkChainNext, fileID);
...
RecordBadLinkCount(SGlobPtr gp, UInt32 inodeID, UInt32 is, UInt32 shouldbe) 
{
	RepairOrderPtr p;
	char goodstr[16], badstr[16];
	fsckPrintFormat(gp->context, E_InvalidLinkCount, inodeID);
...
hash_insert(UInt32 linkID, int totalSlots, int slotsUsed, struct IndirectLinkInfo *linkInfo)
{
	...
	if ((linkInfo[i].flags & LINKINFO_INIT) == 0) {
		if (linkInfo[i].list) {
			fsck_print(ctx, LOG_TYPE_INFO, "hash: overwriting data! (old:%u, new:%u)\n", linkInfo[i].linkID, linkID);
			exit(13);
		}
		linkInfo[i].flags |= LINKINFO_INIT;
		linkInfo[i].linkID = linkID;
		...
		fsck_print(ctx, LOG_TYPE_INFO, "hash: duplicate insert! (%d)\n", linkID);
		exit(13);
	} else {
		fsck_print(ctx, LOG_TYPE_INFO, "hash table full (%d entries) \n", slotsUsed);
		exit(14);
	}
}
