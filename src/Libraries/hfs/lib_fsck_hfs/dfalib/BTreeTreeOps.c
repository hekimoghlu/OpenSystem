...
//          Panic("\pSearchTree: curNodeNum is zero!");
			if (state.debug) fsck_print(ctx, LOG_TYPE_STDERR, "%s(%d):  curNodeNum is 0\n", __FUNCTION__, __LINE__);
        ...
        if (((BTNodeDescriptor*)nodeRec.buffer)->height != level)
        {
				if (state.debug)
				{
					fsck_print(ctx, LOG_TYPE_STDERR, "%s(line %d):  height %d != level %d\n", __FUNCTION__, __LINE__, ((BTNodeDescriptor*)nodeRec.buffer)->height, level);
					fsck_print(ctx, LOG_TYPE_STDERR, "    curNodeNum = %u\n", curNodeNum);
        ...
        if (level == 1)
        {
            ...
            {
				if (state.debug) fsck_print(ctx, LOG_TYPE_STDERR, "%s(%d):  wrong kind of node\n", __FUNCTION__, __LINE__);
                err = fsBTInvalidNodeErr;
                goto ReleaseAndExit;           
            }
        }
        else
        {
            ...
            {
				if (state.debug) fsck_print(ctx, LOG_TYPE_STDERR, "%s(%d):  other wrong kind of node\n", __FUNCTION__, __LINE__);
                err = fsBTInvalidNodeErr;
                goto ReleaseAndExit;
            }
        }
								 ...
								 UInt16						*recsRotatedPtr )
{
	...
	*recsRotatedPtr		= index - moveIndex;
	...
	if ( leftNodePtr->numRecords == leftInsertIndex )
	{
		...
		if ( !didItFit )
		{
			if (state.debug) fsck_print(ctx, LOG_TYPE_INFO, "RotateRight: InsertKeyRecord (left) returned false!\n");
			err = fsBTBadRotateErr;
			goto ErrorExit;
		}
	...
	while ( index > moveIndex )
	{
		if ( index == leftInsertIndex )	// insert new record in right node
		{
			...
			if ( !didItFit )
			{
				if (state.debug) fsck_print(ctx, LOG_TYPE_INFO, "RotateRight: InsertKeyRecord (right) returned false!\n");
				err = fsBTBadRotateErr;
				goto ErrorExit;
			}
		...
		else
		{
			...
			if ( !didItFit )
			{
				if (state.debug) fsck_print(ctx, LOG_TYPE_INFO, "RotateRight: RotateRecordRight returned false!\n");
				err = fsBTBadRotateErr;
				goto ErrorExit;
			}
		}
									...
									Boolean printKeys ) 
{
	...
	if ( printKeys )
	{
		fsck_print(ctx, LOG_TYPE_INFO, "%s - left and right keys:\n", __FUNCTION__ );
...
static void PrintNodeDescriptor( NodeDescPtr  thePtr )
{
	fsck_print(ctx, LOG_TYPE_INFO, "    fLink %d bLink %d kind %d height %d numRecords %d \n",
...
static void PrintKey( UInt8 *  myPtr, int mySize )
{
	...
	{
		fsck_print(ctx, LOG_TYPE_INFO, "%02X", *(myPtr + i) );
	}
	fsck_print(ctx, LOG_TYPE_INFO, "\n" );
} /* PrintKey */
