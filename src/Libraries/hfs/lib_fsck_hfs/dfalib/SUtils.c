...
RepairOrderPtr AllocMinorRepairOrder( SGlobPtr GPtr, size_t n )						/* #extra bytes needed */
{
    ...
    else if ( fsck_get_verbosity_level() >= kDebugLog )
       	fsck_print(ctx, LOG_TYPE_INFO, "\t%s - AllocateClearMemory failed to allocate %d bytes \n", __FUNCTION__, n);
...
void	InitializeVolumeObject( SGlobPtr GPtr )
{
	...
	if ( err == noErr ) {
		...
		if ( myMDBPtr->drSigWord == kHFSPlusSigWord || myMDBPtr->drSigWord == kHFSXSigWord) {
			...
			if ( err == noErr ) {
				...
				if ( fsck_get_verbosity_level() >= kDebugLog ) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tInvalid primary volume header - error %d \n", err );
				}
			}
		}
			...
			if ( fsck_get_verbosity_level() >= kDebugLog ) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tBlock %d is not an MDB or Volume Header \n", MDB_BlkN );
			}
		}
		...
		if ( fsck_get_verbosity_level() >= kDebugLog ) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tcould not get volume block %d, err %d \n", MDB_BlkN, err );
		}
	}
	...
	if ( err == noErr ) {
		...
		if ( myMDBPtr->drSigWord == kHFSPlusSigWord || myMDBPtr->drSigWord == kHFSXSigWord ) {
			...
			if ( err == noErr ) {
				...
				if ( fsck_get_verbosity_level() >= kDebugLog ) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tInvalid alternate volume header - error %d \n", err );
				}
			}
		}
		else if ( myMDBPtr->drSigWord == kHFSSigWord ) {
			...
			if ( fsck_get_verbosity_level() >= kDebugLog ) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tBlock %qd is not an MDB or Volume Header \n", myVOPtr->totalDeviceSectors - 2 );
			}
		}
		...
		if ( fsck_get_verbosity_level() >= kDebugLog ) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tcould not get alternate volume header at %qd, err %d \n",
	...
	if ( (myVOPtr->flags & kVO_PriMDBOK) != 0 && 
		...
		if ( err == noErr ) {
			...
			if ( fsck_get_verbosity_level() >= kDebugLog ) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tcould not get primary MDB at block %qd, err %d \n", myVOPtr->primaryMDB, err );
			}
		}
	}
...
void PrintVolumeObject( void )
{
	...
	if ( myVOPtr->volumeType == kHFSVolumeType )
		fsck_print(ctx, LOG_TYPE_INFO, "\tvolume type is HFS \n" );
	else if ( myVOPtr->volumeType == kEmbededHFSPlusVolumeType )
		fsck_print(ctx, LOG_TYPE_INFO, "\tvolume type is embedded HFS+ \n" );
	else if ( myVOPtr->volumeType == kPureHFSPlusVolumeType )
		fsck_print(ctx, LOG_TYPE_INFO, "\tvolume type is pure HFS+ \n" );
	else
		fsck_print(ctx, LOG_TYPE_INFO, "\tunknown volume type \n" );
	
	fsck_print(ctx, LOG_TYPE_INFO, "\tprimary MDB is at block %qd 0x%02qx \n", myVOPtr->primaryMDB, myVOPtr->primaryMDB );
	fsck_print(ctx, LOG_TYPE_INFO, "\talternate MDB is at block %qd 0x%02qx \n", myVOPtr->alternateMDB, myVOPtr->alternateMDB );
	fsck_print(ctx, LOG_TYPE_INFO, "\tprimary VHB is at block %qd 0x%02qx \n", myVOPtr->primaryVHB, myVOPtr->primaryVHB );
	fsck_print(ctx, LOG_TYPE_INFO, "\talternate VHB is at block %qd 0x%02qx \n", myVOPtr->alternateVHB, myVOPtr->alternateVHB );
	fsck_print(ctx, LOG_TYPE_INFO, "\tsector size = %d 0x%02x \n", myVOPtr->sectorSize, myVOPtr->sectorSize );
	fsck_print(ctx, LOG_TYPE_INFO, "\tVolumeObject flags = 0x%02X \n", myVOPtr->flags );
	fsck_print(ctx, LOG_TYPE_INFO, "\ttotal sectors for volume = %qd 0x%02qx \n",
			myVOPtr->totalDeviceSectors, myVOPtr->totalDeviceSectors );
	fsck_print(ctx, LOG_TYPE_INFO, "\ttotal sectors for embedded volume = %qd 0x%02qx \n",
	...
	if ( err == noErr ) {
		...
		if ( myVHPtr->signature == kHFSPlusSigWord ) {
			...
			if ( err == noErr ) {
				...
				if ( fsck_get_verbosity_level() >= kDebugLog ) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tInvalid embedded alternate volume header at block %qd - error %d \n", myAlternateBlockNum, err );
				}
			}
		}
		else {
			if ( fsck_get_verbosity_level() >= kDebugLog ) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tBlock number %qd is not embedded alternate volume header \n", myAlternateBlockNum );
			}
		}
		(void) ReleaseVolumeBlock( GPtr->calculatedVCB, &myBlockDescriptor, kReleaseBlock );
	}
		...
		if ( fsck_get_verbosity_level() >= kDebugLog ) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tcould not get embedded alternate volume header at %qd, err %d \n",
					myAlternateBlockNum, err );
		}
	}
	...
	if ( err == noErr ) {
		...
		if ( myVHPtr->signature == kHFSPlusSigWord ) {
			...
			if ( err == noErr ) {
				...
				if ( fsck_get_verbosity_level() >= kDebugLog ) {
					fsck_print(ctx, LOG_TYPE_INFO, "\tInvalid embedded primary volume header at block %qd - error %d \n", myPrimaryBlockNum, err );
				}
			}
		}
		else {
			if ( fsck_get_verbosity_level() >= kDebugLog ) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tBlock number %qd is not embedded primary volume header \n", myPrimaryBlockNum );
			}
		}
		(void) ReleaseVolumeBlock( GPtr->calculatedVCB, &myBlockDescriptor, kReleaseBlock );
	}
		...
		if ( fsck_get_verbosity_level() >= kDebugLog ) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tcould not get embedded primary volume header at %qd, err %d \n",
					myPrimaryBlockNum, err );
		}
	}
										...
										HFSPlusVolumeHeader * theAltVHPtr )
{
	...
	if ( thePriVHPtr->catalogFile.totalBlocks != theAltVHPtr->catalogFile.totalBlocks ) {
		...
		if ( thePriVHPtr->catalogFile.extents[0].startBlock == theAltVHPtr->catalogFile.extents[0].startBlock ) {
			...
			if ( thePriVHPtr->catalogFile.totalBlocks > theAltVHPtr->catalogFile.totalBlocks )
				...
				useAlternate = 1;
			if ( fsck_get_verbosity_level() >= kDebugLog ) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tvolume headers disagree on catalog file total blocks - primary %d alternate %d \n",
	...
	if ( thePriVHPtr->extentsFile.totalBlocks != theAltVHPtr->extentsFile.totalBlocks ) {
		...
		if ( thePriVHPtr->extentsFile.extents[0].startBlock == theAltVHPtr->extentsFile.extents[0].startBlock ) {
			...
			if ( thePriVHPtr->extentsFile.totalBlocks > theAltVHPtr->extentsFile.totalBlocks )
				...
				useAlternate = 1;
			if ( fsck_get_verbosity_level() >= kDebugLog ) {
				fsck_print(ctx, LOG_TYPE_INFO, "\tvolume headers disagree on extents file total blocks - primary %d alternate %d \n",
	...
	if ( usePrimary == 1 && useAlternate == 1 ) {
		...
		if ( fsck_get_verbosity_level() >= kDebugLog ) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tvolume headers disagree but there is confusion on which to use \n" );
		}
		return; 
	}
...
DumpData(const void *data, size_t len, char *label)
{
	...
	while (cp < end) {
		...
		if (allzeroes) {
			...
			if (allzeroes == 1) {
				fsck_print(ctx, LOG_TYPE_STDERR, ". . .\n");
				allzeroes = 2;
			}
			if (allzeroes) {
				cp += WIDTH;
		...
		allzeroes = 1;
		if (label == NULL) {
			fsck_print(ctx, LOG_TYPE_STDERR, "%04x:  ", (int)(cp - base));
		}
		else {
			fsck_print(ctx, LOG_TYPE_STDERR, "%s %04x:  ", label, (int)(cp - base));
		}
		for (i = 0, tmp = cp; tmp < tend; tmp++) {
			fsck_print(ctx, LOG_TYPE_STDERR, "%02x", *tmp);
			if (++i % 2 == 0)
				fsck_print(ctx, LOG_TYPE_STDERR, " ");
			if (*tmp)
				allzeroes = 0;
		...
		fsck_print(ctx, LOG_TYPE_STDERR, "    |");
		for (tmp = cp; tmp < tend; tmp++) {
			fsck_print(ctx, LOG_TYPE_STDERR, "%c", isalnum(*tmp) ? *tmp : '.');
		}
		for (i = 0; i < gap; i++) {
			fsck_print(ctx, LOG_TYPE_STDERR, " ");
		}
...
Boolean VolumeObjectIsValid(SGlobPtr gptr)
{
	...
	if (myVOPtr->volumeType == kUnknownVolumeType) {
        fsck_print(ctx, LOG_TYPE_WARNING, "volumeType is %d\n", kUnknownVolumeType);
		goto done;
	} 
	...
	if (retval == false) {
		...
		if (err != noErr) {
			fsck_print(ctx, LOG_TYPE_STDERR, "%s: Cannot GetVolumeBlock: %d\n", __FUNCTION__, err);
		} else {
...
void CheckEmbeddedVolInfoInMDBs( SGlobPtr GPtr )
{
	...
	if ( err != noErr ) {
		if ( fsck_get_verbosity_level() >= kDebugLog ) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tcould not get primary MDB \n" );
		}
		goto ExitThisRoutine;
	}
	...
	if ( err != noErr ) {
		if ( fsck_get_verbosity_level() >= kDebugLog ) {
			fsck_print(ctx, LOG_TYPE_INFO, "\tcould not get alternate MDB \n" );
		}
		goto ExitThisRoutine;
	}
	...
	if ( primaryIsDamaged || alternateIsDamaged ) {
		...
		if ( primaryIsDamaged ) {
			...
			if ( fsck_get_verbosity_level() >= kDebugLog )
				fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid primary wrapper MDB \n");
		}
		else {
			myVOPtr->flags &= ~kVO_AltMDBOK; // mark the alternate MDB as damaged
			if ( fsck_get_verbosity_level() >= kDebugLog )
				fsck_print(ctx, LOG_TYPE_INFO, "\tinvalid alternate wrapper MDB \n");
		}
	}
...
void PrintName( int theCount, const UInt8 *theNamePtr, Boolean isUnicodeString )
{
    ...
    for ( i = 0; i < myCount; i++ ) 
       	fsck_print(ctx, LOG_TYPE_INFO, "%02X ", *(theNamePtr + i) );
...
void print_prime_buckets(PrimeBuckets *cur) 
{
	...
	fsck_print(ctx, LOG_TYPE_INFO, "n32 = { ");
	for (i=0; i<32; i++) {
		fsck_print(ctx, LOG_TYPE_INFO, "%d,", cur->n32[i]);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "}\n");

	fsck_print(ctx, LOG_TYPE_INFO, "n27 = { ");
	for (i=0; i<27; i++) {
		fsck_print(ctx, LOG_TYPE_INFO, "%d,", cur->n27[i]);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "}\n");

	fsck_print(ctx, LOG_TYPE_INFO, "n25 = { ");
	for (i=0; i<25; i++) {
		fsck_print(ctx, LOG_TYPE_INFO, "%d,", cur->n25[i]);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "}\n");

	fsck_print(ctx, LOG_TYPE_INFO, "n7 = { ");
	for (i=0; i<7; i++) {
		fsck_print(ctx, LOG_TYPE_INFO, "%d,", cur->n7[i]);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "}\n");
	
	fsck_print(ctx, LOG_TYPE_INFO, "n11 = { ");
	for (i=0; i<11; i++) {
		fsck_print(ctx, LOG_TYPE_INFO, "%d,", cur->n11[i]);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "}\n");

	fsck_print(ctx, LOG_TYPE_INFO, "n13 = { ");
	for (i=0; i<13; i++) {
		fsck_print(ctx, LOG_TYPE_INFO, "%d,", cur->n13[i]);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "}\n");

	fsck_print(ctx, LOG_TYPE_INFO, "n17 = { ");
	for (i=0; i<17; i++) {
		fsck_print(ctx, LOG_TYPE_INFO, "%d,", cur->n17[i]);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "}\n");

	fsck_print(ctx, LOG_TYPE_INFO, "n19 = { ");
	for (i=0; i<19; i++) {
		fsck_print(ctx, LOG_TYPE_INFO, "%d,", cur->n19[i]);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "}\n");

	fsck_print(ctx, LOG_TYPE_INFO, "n23 = { ");
	for (i=0; i<23; i++) {
		fsck_print(ctx, LOG_TYPE_INFO, "%d,", cur->n23[i]);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "}\n");

	fsck_print(ctx, LOG_TYPE_INFO, "n29 = { ");
	for (i=0; i<29; i++) {
		fsck_print(ctx, LOG_TYPE_INFO, "%d,", cur->n29[i]);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "}\n");

	fsck_print(ctx, LOG_TYPE_INFO, "n31 = { ");
	for (i=0; i<31; i++) {
		fsck_print(ctx, LOG_TYPE_INFO, "%d,", cur->n31[i]);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "}\n");
}
