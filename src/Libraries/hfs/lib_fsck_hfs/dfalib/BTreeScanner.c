...
static int ReadMultipleNodes( BTScanState *theScanStatePtr )
{
	...
	if ( myErr == noErr )
	{
		...
		{
			uint8_t *raw, *cached;
			fsck_print(ctx, LOG_TYPE_STDERR, "CacheRead and CacheRawRead returned different values\n");
			fsck_print(ctx, LOG_TYPE_STDERR, "\tmyPhyOffset = %llu, myContiguousBytes = %u\n", myPhyOffset, myContiguousBytes);
			size_t i = 0;
			for (i = 0; i < myContiguousBytes; i++)
			{
				...
				if (cached != raw)
				{
					fsck_print(ctx, LOG_TYPE_STDERR, "\tOffset %zu:  cached value = 0x%02x, raw value = 0x%02x\n", i, cached, raw);
					break;
				}
			}
			extern void dumpCache(void*);
			dumpCache(myBTreeCBPtr->fcbPtr->fcbVolume->vcbBlockCache);
			abort();
		}
		free(tempBuffer);
#endif
