...
int main(int argc, char *argv[]) {
	...
	fsck_print(ctx, LOG_TYPE_INFO, "\tFile:\t\tDecompData.h\n");
	fsck_print(ctx, LOG_TYPE_INFO, "\tContains:\tData tables for use in FixDecomps (CatalogCheck.c)\n");
	fsck_print(ctx, LOG_TYPE_INFO, "\tNote:\t\tThis file is generated automatically by running DecompMakeData\n");
	fsck_print(ctx, LOG_TYPE_INFO, "*/\n");
	fsck_print(ctx, LOG_TYPE_INFO, "#include \"DecompDataEnums.h\"\n\n");
	...
	for (classAndReplPtr = uCharClassAndRepl; classAndReplPtr->uChar != 0; classAndReplPtr++) {
		...
		if (shiftUChar >= kShiftUniCharLimit) {
			fsck_print(ctx, LOG_TYPE_INFO, "Exceeded uChar range for 0x%04X\n", classAndReplPtr->uChar);
			return 1;
		}
		entryIndex = shiftUChar >> kLoFieldBitSize;
		if (rangesIndex[entryIndex] == -1) {
			if (rangeCount >= kMaxRangeCount) {
				fsck_print(ctx, LOG_TYPE_INFO, "Exceeded max range count with 0x%04X\n", classAndReplPtr->uChar);
				return 1;
			}
		...
		if (classAndReplPtr->action != 0) {
			...
			if (replDataCount + matchAndReplacementCount >= kMaxReplaceDataCount) {
				fsck_print(ctx, LOG_TYPE_INFO, "Exceeded max replacement data count with 0x%04X\n", classAndReplPtr->uChar);
				return 1;
			}
	...
	fsck_print(ctx, LOG_TYPE_INFO, "static const int8_t classAndReplIndex[kHiFieldEntryCount] = {\n");
	for (entryIndex = 0; entryIndex < kHiFieldEntryCount; entryIndex++) {
		char *	formatPtr = (entryIndex + 1 < kHiFieldEntryCount)? "%2d,\t": "%2d\t";
		if (entryIndex % kIndexValuesPerLine == 0)			// beginning of line,
			fsck_print(ctx, LOG_TYPE_INFO, "\t");								//  print tab
		fsck_print(ctx, LOG_TYPE_INFO, formatPtr, rangesIndex[entryIndex]);		// print values
		if ((entryIndex + 1) % kIndexValuesPerLine == 0)		// end of line, print starting UniChar value
			fsck_print(ctx, LOG_TYPE_INFO, "// uChar 0x%04X-\n", (u_int16_t)(((entryIndex + 1 - kIndexValuesPerLine) << kLoFieldBitSize) - kShiftUniCharOffset) );
	}
	fsck_print(ctx, LOG_TYPE_INFO, "};\n\n");
	
	// print filled in class ranges
	fsck_print(ctx, LOG_TYPE_INFO, "static const u_int8_t combClassRanges[][kLoFieldEntryCount] = {\n", kLoFieldEntryCount);
	for (rangeIndex = 0; rangeIndex < rangeCount; rangeIndex++) {
		fsck_print(ctx, LOG_TYPE_INFO, "\t{\t");
		for (entryIndex = 0; entryIndex < kLoFieldEntryCount; entryIndex++) {
			char *	formatPtr = (entryIndex + 1 < kLoFieldEntryCount)? "%3d,": "%3d";
			fsck_print(ctx, LOG_TYPE_INFO, formatPtr, classRanges[rangeIndex][entryIndex]);	// print values
		}
	...
	fsck_print(ctx, LOG_TYPE_INFO, "static const u_int8_t replaceRanges[][kLoFieldEntryCount] = {\n", kLoFieldEntryCount);
	for (rangeIndex = 0; rangeIndex < rangeCount; rangeIndex++) {
		fsck_print(ctx, LOG_TYPE_INFO, "\t{\t");
		for (entryIndex = 0; entryIndex < kLoFieldEntryCount; entryIndex++) {
			char *	formatPtr = (entryIndex + 1 < kLoFieldEntryCount)? "%3d,": "%3d";
			fsck_print(ctx, LOG_TYPE_INFO, formatPtr, replRanges[rangeIndex][entryIndex]);	// print values
		}
		fsck_print(ctx, LOG_TYPE_INFO, "\t},\t// uChar 0x%04X-\n", rangesKey[rangeIndex]);
	}
	fsck_print(ctx, LOG_TYPE_INFO, "};\n\n");
	...
	fsck_print(ctx, LOG_TYPE_INFO, "static const u_int16_t replaceData[] = {\n");
	for (entryIndex = 0; entryIndex < replDataCount; entryIndex++) {
		...
		if (entryIndex % kReplDataValuesPerLine == 0)			// beginning of line,
			fsck_print(ctx, LOG_TYPE_INFO, "\t");										//  print tab
		fsck_print(ctx, LOG_TYPE_INFO, formatPtr, replacementData[entryIndex]);			// print values
		if ((entryIndex + 1) % kReplDataValuesPerLine == 0 || entryIndex + 1 == replDataCount)	// end of line,
			fsck_print(ctx, LOG_TYPE_INFO, "// index %d-\n", entryIndex & ~(kReplDataValuesPerLine-1) );	// print starting index value
	}
	fsck_print(ctx, LOG_TYPE_INFO, "};\n\n");

	// print summary info
	fsck_print(ctx, LOG_TYPE_INFO, "// combClassData:\n");
	fsck_print(ctx, LOG_TYPE_INFO, "// trimmed index: kHiFieldEntryCount(= %d) bytes\n", kHiFieldEntryCount);
	fsck_print(ctx, LOG_TYPE_INFO, "// ranges: 2 * %d ranges * kLoFieldEntryCount(= %d) bytes = %d\n", rangeCount, kLoFieldEntryCount, 2*rangeCount*kLoFieldEntryCount);
	fsck_print(ctx, LOG_TYPE_INFO, "// replData: %d entries * 2 = %d\n", replDataCount, 2*replDataCount);
	fsck_print(ctx, LOG_TYPE_INFO, "// total: %d\n\n", kHiFieldEntryCount + 2*rangeCount*kLoFieldEntryCount + 2*replDataCount);
