							...
							void *						stopRefCon )
{
	...
	DebugLog("IOFWIsochChannel<%p>::init - packetSize = %d, doIRM = %d\n", this, packetSize, doIRM );
...
IOReturn IOFWIsochChannel::checkMemoryInRange( IOMemoryDescriptor * memory )
{
	...
	if( status == kIOReturnSuccess )
	{
		...
		DebugLog("IOFWIsochChannel<%p>::checkMemoryInRange - dma_command = %p, mapper = %p\n", this, dma_command, mapper);
...
IOReturn IOFWIsochChannel::allocateChannelBegin( 	IOFWSpeed		inSpeed,
												...
												UInt32 *		outChannel )
{
	// (IOFireWireFamily) IOFWIsochChannel<0xffffff8054bea140>::allocateChannelBegin - inSpeed = 0, fDoIRM = 0, inAllowedChans = 0x0000000000000001, fChannel=64, fBandwidth=0
	DebugLog("IOFWIsochChannel<%p>::allocateChannelBegin - inSpeed = %d, fDoIRM = %d, inAllowedChans = 0x%016llx, fChannel=%d, fBandwidth=%d\n", this, inSpeed, fDoIRM, inAllowedChans, (uint32_t)fChannel, (uint32_t)fBandwidth );
	...
	if( fDoIRM )
	{
		...
		do
		{
			...
			if( status == kIOReturnSuccess )
			{
				...
				DebugLog("IOFWIsochChannel<%p>::allocateChannelBegin - oldIRM set to %08x %08x %08x\n", this, OSSwapBigToHostInt32(oldIRM[0]), OSSwapBigToHostInt32(oldIRM[1]), OSSwapBigToHostInt32(oldIRM[2]) ) ;
			}
