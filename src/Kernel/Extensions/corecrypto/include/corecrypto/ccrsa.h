 ...
 *
 * â€¢ If nlen = 1024 bits (i.e., 128 bytes), and the output length of the approved hash function output block is 512 bits (i.e., 64 bytes), then the length (in bytes) of the salt (sLen) shall satisfy 0 â‰¤ sLen â‰¤ hLen â€“ 2,
 * â€¢ Otherwise, the length (in bytes) of the salt (sLen) shall satisfy 0 â‰¤ sLen â‰¤ hLen, where hLen is the length of the hash function output block (in bytes).
 *
 *
 * â€¢ CAVS test vectors are not very useful in the case of RSA-PSS, because they only validate the exponentiation part of the signature. See: http://csrc.nist.gov/groups/STM/cavp/documents/components/RSA2SP1VS.pdf
 *
