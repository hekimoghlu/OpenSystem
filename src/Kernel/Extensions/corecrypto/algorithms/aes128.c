...
/*****************************************************************************/
#include <stdint.h>
#include <string.h> // CBC mode, for memset memcpy
#include "aes128.h"
...
#define Nk 4
// Key length in bytes [128 bit], also a block length
#define KEYLEN 16
...
static void KeyExpansion(struct _pdcmode_aes128_ctx *ctx, const uint8_t* key)
{
    uint32_t i, j, k;
...
static void AddRoundKey(const struct _pdcmode_aes128_ctx *ctx, uint8_t round)
{
    uint8_t i,j;
    for(i=0;i<4;++i)
    {
...
static void Cipher(const struct _pdcmode_aes128_ctx *ctx)
{
    uint8_t round = 0;
...
static void InvCipher(const struct _pdcmode_aes128_ctx *ctx)
{
    uint8_t round=0;
...
static void XorWithIv(const uint8_t *iv, uint8_t* buf)
{
    for (int i = 0; i < KEYLEN; i++)
    {
        buf[i] ^= iv[i];
...
void AES128_CBC_encrypt(const struct _pdcmode_aes128_ctx *ctx, struct pdccbc_iv* iv, unsigned long nblocks, const void *in, void *out)
{
    ...
    {
        BlockCopy(output, input);
        XorWithIv(iv_ptr, output);
        state = (state_t*)output;
        Cipher(ctx);
        memcpy(iv_ptr, output, 16);
        input += KEYLEN;
        output += KEYLEN;
    }
