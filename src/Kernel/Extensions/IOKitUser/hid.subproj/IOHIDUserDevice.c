                                     ...
                                     dispatch_queue_t queue)
{
    ...
    snprintf(label, sizeof(label), "%s.IOHIDUserDeviceRef", dispatch_queue_get_label (queue) ? dispatch_queue_get_label (queue) : "");
...
bool __IOHIDUserDeviceSetupAnalytics(IOHIDUserDeviceRef device)
{
    ...
    if (!device->queue.usageAnalytics) {
        IOHIDUDLog("Unable to create queue analytics");
    }
