...
platform_is_amp(void)
{
	...
	if (verbosity_enabled && !reported_is_amp) {
		T_LOG("Ã°Å¸â€ºÂ°Ã¯Â¸Â Platform has %d perflevels (%s)", platform_nperflevels(), is_amp ? "AMP" : "SMP");
...
platform_is_virtual_machine(void)
{
	...
	if (vmm_present && verbosity_enabled) {
		T_LOG("Ã°Å¸â€ºÂ°Ã¯Â¸Â Platform is a virtual machine!");
	}
...
platform_sched_policy(void)
{
	...
	if (verbosity_enabled) {
		T_LOG("Ã°Å¸â€ºÂ°Ã¯Â¸Â Platform is running the \"%s\" scheduler policy", sched_policy_name);
	}
...
platform_num_clusters(void)
{
	if (num_clusters == 0) {
		...
		if (verbosity_enabled) {
			T_LOG("Ã°Å¸â€ºÂ°Ã¯Â¸Â Platform has %u CPU clusters", num_clusters);
		}
	}
	return num_clusters;
}
...
wait_for_quiescence(int argc, char *const argv[], double idle_threshold, int timeout_seconds)
{
	...
	for (int i = 0; i < argc; i++) {
		if (strcmp(argv[i], "--no-quiesce") == 0) {
			T_LOG("Ã°Å¸â€¢â€™ Skipping quiescence due to \"--no-quiesce\"");
			return true;
		}
	}
	...
	if (verbosity_enabled) {
		T_LOG("Ã°Å¸â€¢â€™ Waiting up to %d second(s) for the system to quiesce above %.2f%% idle...",
	...
	while (waited_seconds < timeout_seconds) {
		...
		if (waited_seconds >= 1) {
			...
			if (total_delta > 0.0) {
				...
				if (verbosity_enabled) {
					T_LOG("Ã°Å¸â€¢â€™ Not yet quiesced (%.2f%% idle)", idle_ratio * 100.0);
				}
			}
		}
	...
	if (verbosity_enabled) {
		if (quiesced) {
			T_LOG("Ã°Å¸â€¢â€™ System quiesced to %.2f%% idle within %d second(s)", idle_ratio * 100.0, waited_seconds);
		} else {
			T_LOG("Ã°Å¸â€¢â€™ Failed to quiesce within %.2f%% idle after %d second(s)", idle_threshold * 100.0, waited_seconds);
		}
	}
	return quiesced;
}
...
{
	T_LOG("Ã°Å¸ÂÅ½Ã¯Â¸Â Restoring CLPC state...");
...
execute_clpcctrl(char *clpcctrl_args[], bool read_value)
{
	...
	pid_t pid = dt_launch_tool_pipe(full_clpcctrl_args, false, NULL,
		...
		T_LOG("Ã°Å¸ÂÅ½Ã¯Â¸Â [clpcctrl] %s", data);
		if (read_value) {
	    ...
	    ^bool (char *data, __unused size_t data_size, __unused dt_pipe_data_handler_context_t *context) {
		T_LOG("Ã°Å¸ÂÅ½Ã¯Â¸Â [clpcctrl] Error msg: %s", data);
		return false;
	},
...
check_recommended_core_mask(uint64_t *core_mask)
{
	...
	if (verbosity_enabled) {
		...
		T_LOG("Ã°Å¸â€œË† kern.sched_recommended_cores: %016llx, expecting %016llx if all are recommended",
...
static char *begin_notification = "Ã°Å¸â€“Å Ã¯Â¸Â_trace_begun...";
static char *end_notification = "Ã°Å¸â€“Å Ã¯Â¸Â_trace_ended...";
static char *trigger_end_notification = "Ã°Å¸â€“Å Ã¯Â¸Â_stopping_trace...";
...
begin_collect_trace_fmt(int argc, char *const argv[], char *fmt, ...)
{
	...
	T_QUIET; T_ASSERT_EQ(geteuid(), 0, "Ã°Å¸â€“Å Ã¯Â¸Â Tracing requires the test to be run as root user");
	...
	pid_t trace_pid = dt_launch_tool_pipe(trace_args, false, NULL,
	    ^bool (char *data, __unused size_t data_size, __unused dt_pipe_data_handler_context_t *context) {
		T_LOG("Ã°Å¸â€“Å Ã¯Â¸Â [trace] %s", data);
		return false;
	},
	    ^bool (char *data, __unused size_t data_size, __unused dt_pipe_data_handler_context_t *context) {
		T_LOG("Ã°Å¸â€“Å Ã¯Â¸Â [trace] Error msg: %s", data);
		return false;
	},
	    BUFFER_PATTERN_LINE, NULL);

	T_LOG("Ã°Å¸â€“Å Ã¯Â¸Â Starting trace collection for \"%s\" trace[%u]", handle->trace_filename, trace_pid);
...
end_collect_trace(trace_handle_t handle)
{
	...
	T_LOG("Ã°Å¸â€“Å Ã¯Â¸Â Now waiting on trace to finish up...");
	int signal_num;
	int exit_status;
...
save_collected_trace(trace_handle_t handle)
{
	...
	T_LOG("Ã°Å¸â€“Å Ã¯Â¸Â \"%s\" marked for upload", compressed_path);
	...
	pid_t aa_pid = dt_launch_tool_pipe(compress_args, false, NULL,
		...
		T_LOG("Ã°Å¸â€“Å Ã¯Â¸Â [aa] %s", data);
		return false;
	},
	    ^bool (char *data, __unused size_t data_size, __unused dt_pipe_data_handler_context_t *context) {
		T_LOG("Ã°Å¸â€“Å Ã¯Â¸Â [aa] Error/debug msg: %s", data);
		return false;
	},
	    BUFFER_PATTERN_LINE, NULL);
	T_QUIET; T_EXPECT_TRUE(aa_pid, "Ã°Å¸â€“Å Ã¯Â¸Â [aa] pid %d", aa_pid);
	int exit_status = 0;
	...
	T_LOG("Ã°Å¸â€“Å Ã¯Â¸Â Finished saving trace (%s), which is available compressed at \"%s\"",
...
discard_collected_trace(trace_handle_t handle)
{
	...
	if (trace_state->status == ENDED) {
		T_LOG("Ã°Å¸â€“Å Ã¯Â¸Â Deleted recorded trace file at \"%s\"", trace_state->abs_filename);
	}
