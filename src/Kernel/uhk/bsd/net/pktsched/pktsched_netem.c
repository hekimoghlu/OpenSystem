...
nlc_enqueue(struct netem *ne, classq_pkt_t *p, bool *pdrop)
{
	...
	NETEM_LOG(LOG_DEBUG, "Ã¢â€Å’ begin p %p len %u, now %llu", p->cp_mbuf,
	...
	NETEM_MTX_UNLOCK(ne);
	NETEM_LOG(LOG_DEBUG, "Ã¢â€â€ %p end", p->cp_mbuf);
	return ret;
    ...
    bool *more)
{
	...
	NETEM_LOG(LOG_DEBUG, "Ã¢â€Å’ begin");
    ...
    netem_output_func_t *output_func, uint32_t output_max_batch_size)
{
	...
	NETEM_LOG(LOG_INFO, "Ã¢â€Å’ begin %s", name);
	...
	NETEM_LOG(LOG_INFO, "Ã¢â€â€ ret %d", ret);
	return ret;
}
