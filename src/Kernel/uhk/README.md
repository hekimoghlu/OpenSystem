# What is Universal Hybrid Kernel (UHK)?

UHK kernel is part of the Darwin operating system for use in macOS and iOS operating systems. UHK is an acronym for X is Not Unix.
UHK is a hybrid kernel combining the Mach kernel developed at Carnegie Mellon University with components from FreeBSD and a C++ API for writing drivers called IOKit.
UHK runs on x86_64 and ARM64 for both single processor and multi-processor configurations.
...
The UHK make system can build kernel based on `KERNEL_CONFIGS` & `ARCH_CONFIGS` variables as arguments.
Here is the syntax:
...
```text
make install_kernels DSTROOT=/tmp/UHK-dst
```
...
The UHK build system can optionally output color-formatted build output. To enable this, you can either
set the `UHK_LOGCOLORS` environment variable to `y`, or you can pass `LOGCOLORS=y` to the make command.

### Customize the UHK Version

The UHK version is derived from the SDK or KDK by reading the `CFBundleVersion`
...
See doc/UHK_version.md for more details.
...
To test the UHK kernel, you need to build a kernelcache that links the kexts and
...
## Installing New Header Files from UHK

UHK installs header files at the following locations -
...
create Makefile similar to `UHK/bsd/sys/Makefile`.
...
i. `EXPORT_MI_LIST` : Exports header file to all of UHK (bsd/, osfmk/, etc.)
...
Private Interfaces. These are visible within UHK and
...
2. `KERNEL_PRIVATE` : If defined, enclosed code is available to all of UHK
...
within the UHK/bsd module.
4. `MACH_KERNEL_PRIVATE`: If defined, enclosed code is visible exclusively
within the UHK/osfmk module.
5. `UHK_KERNEL_PRIVATE`: If defined, enclosed code is visible exclusively
within UHK.
6. `KERNEL` :  If defined, enclosed code is available within UHK and kernel
...
* `*_UHK.h` headers contain components of the VM subsystem only for use by other UHK code.
...
`UHK` contributes to the modules defined in CoreOSModuleMaps by installing
module map files that are sourced from `usr/include/module.modulemap` and
`usr/local/include/module.modulemap`. The naming convention for the `UHK`
...
`UHK` offers the following mechanisms for conditionally compiling code:
...
It is recommended that you avoid compiling based on the target platform. `UHK`
