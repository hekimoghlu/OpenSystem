...
static void nss_cms_decoder_work_data(SecCmsDecoderRef p7dcx, const unsigned char* data, size_t len, Boolean final)
{
     ...
     */
    if (((data == NULL || len == 0) && !final) || len > UINT_MAX) {
        goto loser;
    }
    ...
    if (cinfo->ciphcx != NULL) {
         ...
         */
        rv = SecCmsCipherContextDecrypt(
            cinfo->ciphcx, buf, &outlen, buflen, data, (unsigned int)len, final);
        if (rv != SECSuccess) {
            p7dcx->error = PORT_GetError();
            goto loser;
        }
 ...
 */
OSStatus SecCmsDecoderCreate(SecCmsContentCallback cb,
                             void* cb_arg,
...
OSStatus SecCmsMessageDecode(const SecAsn1Item* encodedMessage,
                             SecCmsContentCallback cb,
                             void* cb_arg,
                             PK11PasswordFunc pwfn,
                             void* pwfn_arg,
                             ...
                             SecCmsMessageRef* outMessage)
{
    ...
    result = SecCmsDecoderCreate(
        cb, cb_arg, pwfn, pwfn_arg, decrypt_key_cb, decrypt_key_cb_arg, &decoder);
    if (result) {
        return result;
    }
