 ...
 * a hardware token...
 */
SecSymmetricKeyRef SecCmsUtilDecryptSymKeyRSA(SecPrivateKeyRef privkey, SecAsn1Item* encKey, SECOidTag bulkalgtag)
{
    /* that's easy */
                                                         ...
                                                         * KeyAgreeRecipientInfo.originator.OriginatorPublicKey */
{
    ...
    if (CFDataGetLength(key) < 0) {
        goto out;
    }
    size_t expectedEncKeyLength = CCCryptorGetOutputLength(ciphercc, (size_t)CFDataGetLength(key), true);
    encKey->Data = PORT_ArenaAlloc(poolp, expectedEncKeyLength);
    size_t bytes_output = 0;
    rv = CCCryptorUpdate(
        ciphercc, CFDataGetBytePtr(key), (size_t)CFDataGetLength(key), encKey->Data, expectedEncKeyLength, &bytes_output);
    if (rv) {
                                                         ...
                                                         * KeyAgreeRecipientInfo.originator.OriginatorPublicKey */
{
    ...
    outKey = (SecSymmetricKeyRef)CFDataCreate(NULL, cek.Data, (CFIndex)cek.Length);
