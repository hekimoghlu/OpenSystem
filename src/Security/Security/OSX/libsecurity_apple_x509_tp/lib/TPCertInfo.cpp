...
CSSM_TP_APPLE_EVIDENCE_INFO *TPCertGroup::buildCssmEvidenceInfo()
{
	...
	for(unsigned i=0; i<mNumCerts; i++) {
		...
		/*
		 * Always alloc StatusCodes array with one more element than the actual
		 * number of codes, so we can stash CrlReason at the end of the array.
		 */
		...
		if(issuerCert != NULL && !issuerCert->isSelfSigned() && (untrustedRoot == NULL)) {
			...
			if(possibleAnchorCert != NULL) {
				if(possibleAnchorCert->isSelfSigned()) {
					/*
					 * We found a better replacement issuer, so use it.
					 * note that we don't need to free the old issuerCert first as it
					 * comes from inCertGroup or gatheredCerts (not from dbList).
