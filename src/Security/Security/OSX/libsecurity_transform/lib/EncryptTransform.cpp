...
CFDataRef EncryptDecryptBase::remove_oaep_padding(CFDataRef encodedMessage)
{
    ...
    // function (2^61 âˆ’ 1 octets for SHA-1) then output â€˜â€˜decoding errorâ€™â€™ and stop."; we don't have
    // ready access to the input limits of the hash functions, and in the real world we won't be
    // seeing messages that long anyway.

    // (2) If emLen < 2hLen + 1, output â€˜â€˜decoding errorâ€™â€™ and stop.
    ...
    // (3) Let maskedSeed be the first hLen octets of EM and let maskedDB be the remaining emLenâˆ’hLen
    // octets.
    ...
    // (5) Let seed = maskedSeed âŠ• seedMask.
    ...
    // (6) Let dbMask = MGF (seed, emLen âˆ’ hLen).
    ...
    // (9) Separate DB into an octet string pHashâ€™ consisting of the first hLen octets of DB, a
    // (possibly empty) octet string PS consisting of consecutive zero octets following pHashâ€™,
    // and a message M as If there is no 01 octet to separate PS from M , output â€˜â€˜decoding errorâ€™â€™ and stop.
    ...
    if (CFEqual(pHash, pHashPrime)) {
        ...
        // (10) If pHashâ€™ does not equal pHash, output â€˜â€˜decoding errorâ€™â€™ and stop.
        goto out;
    }
...
CFDataRef EncryptDecryptBase::apply_oaep_padding(CFDataRef dataValue)
{
	...
	// (2^61 âˆ’ 1 octets for SHA-1) then output â€˜â€˜parameter string too longâ€™â€™ and stop.
	...
	// (2) If mLen > emLen âˆ’ 2hLen âˆ’ 1, output â€˜â€˜message too longâ€™â€™ and stop.
	hashAlgo = (CFStringRef)this->GetAttribute(kSecOAEPMGF1DigestAlgorithmAttributeName);
	if (hashAlgo == NULL) {
		hashAlgo = kSecDigestSHA1;
	}
	...
	// (3) Generate an octet string PS consisting of emLen âˆ’ mLen âˆ’ 2hLen âˆ’ 1 zero octets. The length of PS may be 0.
	...
	// (5) Concatenate pHash,PS, the message M, and other padding to form a data block DB as DB = pHashâˆ¥PSâˆ¥01âˆ¥M.
	...
	// (7) Let dbMask = MGF (seed, emLen âˆ’ hLen).
	...
	// (10) Let maskedSeed = seed âŠ• seedMask
	...
	// (11) Let EM = maskedSeedâˆ¥maskedDB (if we didn't have to pushback the NULL we could do this without physically concatanating)
