...
static OSStatus privKeyForPubKeyHashWrap(CFDictionaryRef context, SecKeyRef *privKey, CFTypeRef *laCtx)
{
    ...
    if (!isLocalAuthenticationAvailable()) {
        os_log_error(TL_LOG, "LocalAuthentication is not available");
        return errSecUnimplemented;
    }

    if (!isCryptoTokenKitAvailable()) {
        os_log_error(TL_LOG, "CryptoTokenKit is not available");
        return errSecUnimplemented;
    }
    ...
    LAContext *authContext = [[getLAContextClass() alloc] init];
    NSDictionary *parameters = @{};
    NSString *pin = (__bridge NSString *)getPin(context);
	if (pin) {
        parameters = @{ @"PIN": pin };
    }

    NSString *tokenID = (__bridge NSString *)getTokenId(context);
    TKClientToken *clientToken = [[getTKClientTokenClass() alloc] initWithTokenID:tokenID];
    TKClientTokenSession *tokenSession = [[getTKClientTokenSessionClass() alloc] initWithToken:clientToken LAContext:authContext parameters:parameters error:&err];
    if (tokenSession == nil) {
        os_log_error(TL_LOG, "Failed to connect to token: %{public}@", err);
        return errSecParam;
    }
    ...
    for (TKClientTokenAdvertisedItem *identityItem in tokenSession.identities) {
        SecIdentityRef identity = identityItem.identityRef;
