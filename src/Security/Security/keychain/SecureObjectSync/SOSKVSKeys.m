...
const CFStringRef kSOSKVSLastCleanupTimestampKey = CFSTR("tLastKVSKeyCleanup");
const CFStringRef kSOSKVSOTRConfigVersion = CFSTR("OTRConfigVersion");
const CFStringRef kSOSKVSWroteLastKeyParams = CFSTR("WroteLastKeyParams");
...
CFStringRef SOSMessageKeyCreateFromPeerToTransport(SOSMessage* transport, CFStringRef myName, CFStringRef peer_name){
    CFErrorRef error = NULL;

    CFStringRef circleName = [transport SOSTransportMessageGetCircleName];
    CFStringRef result = SOSMessageKeyCreateWithCircleNameAndPeerNames(circleName, peer_name, myName);
    CFReleaseSafe(error);
    return result;
}
...
CFStringRef SOSMessageKeyCreateFromTransportToPeer(SOSMessage* transport, CFStringRef myID, CFStringRef peer_name) {
    ...
    CFStringRef result = SOSMessageKeyCreateWithCircleNameAndPeerNames(circleName, myID, peer_name);
    CFReleaseSafe(error);
    return result;
}
