...
const uint8_t *ccder_decode_implicit_uint64(ccder_tag expected_tag, uint64_t *_Nonnull r, const uint8_t *_Nullable der, const uint8_t *_Nonnull der_end) {
    ...
    if (der && len && (*der & 0x80) != 0x80) {
        if (ccn_read_uint(ccn_nof_size(sizeof(*r)), (cc_unit*)r, len, der) >= 0)
            return der + len;
    }
    return NULL;
}
...
der_decode_manifest_and_objects_message(SOSMessageRef message,
    ...
    if (body_end != der_end) {
        SOSErrorCreate(kSOSErrorInvalidMessage, error, NULL, CFSTR("Trailing garbage at end of message"));
        return NULL;
    }
...
static const uint8_t *der_decode_v0_message_body(SOSMessageRef message, CFErrorRef *error, const uint8_t *der, const uint8_t *der_end) {
    ...
    if (der) switch (messageType) {
        ...
        default:
            SOSErrorCreate(kSOSErrorInvalidMessage, error, NULL, CFSTR("Invalid message type %llu"), messageType);
            break;
    }
    return der;
}
