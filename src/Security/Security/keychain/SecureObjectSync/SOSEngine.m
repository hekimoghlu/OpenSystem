...
static bool SOSEngineSaveCoders(SOSEngineRef engine, SOSTransactionRef txn, CFErrorRef *error) {
    ...
    if (engine->codersNeedSaving) {
        ...
        if (ok) {
            ...
            secinfo("coder", "saved coders: %@", engine->coders);
        } else {
...
static bool SOSEngineLoadCoders(SOSEngineRef engine, SOSTransactionRef txn, CFErrorRef *error) {
    ...
    secnotice("coder", "Will force peer registration: %{BOOL}d", needPeerRegistration);
...
static bool SOSEngineLoad(SOSEngineRef engine, SOSTransactionRef txn, CFErrorRef *error) {
    ...
    basicEngineState = SOSDataSourceCopyStateWithKey(engine->dataSource, kSOSEngineStatev2, kSOSEngineProtectionDomainClassD, txn, &localError);
    if (basicEngineState) {
        CFDataRef data = NULL;
        engineState = derStateToDictionaryCopy(basicEngineState, &localError);

        data = SOSDataSourceCopyStateWithKey(engine->dataSource, kSOSEngineManifestCache, kSOSEngineProtectionDomainClassD, txn, &localError);
        manifestCache = derStateToDictionaryCopy(data, &localError);
        CFReleaseNull(data);

        data = SOSDataSourceCopyStateWithKey(engine->dataSource, kSOSEnginePeerStates, kSOSEngineProtectionDomainClassD, txn, &localError);
        peerStateDict = derStateToDictionaryCopy(data, &localError);
        CFReleaseNull(data);
    } else {
        // Look for original V0 engine state next
        CFDataRef v0EngineStateData = SOSDataSourceCopyStateWithKey(engine->dataSource, kSOSEngineState, kSOSEngineProtectionDomainClassD, txn, &localError);
        if (v0EngineStateData) {
            engineState = derStateToDictionaryCopy(v0EngineStateData, &localError);
            if (engineState) {
    ...
    ok = engineState && SOSEngineSetStateWithDictionary(engine, engineState, &localError);

    ok &= SOSEngineSetManifestCacheWithDictionary(engine, manifestCache, &localError);

    ok &= peerStateDict && SOSEngineSetPeerStateWithDictionary(engine, peerStateDict, &localError);
...
static void SOSEngineSetNotifyPhaseBlock(SOSEngineRef engine) {
    SOSDataSourceAddNotifyPhaseBlock(engine->dataSource, ^(SOSDataSourceRef ds, SOSTransactionRef txn, SOSDataSourceTransactionPhase phase, SOSDataSourceTransactionSource source, CFArrayRef changes) {
        dispatch_sync(engine->queue, ^{
            IF_SOS_DISABLED_SERVER {
                secnotice("engine", "SOS disabled for this platform");
            } else {
                CFErrorRef localError = NULL;
...
static void SOSEngineReferenceBackupPeer(SOSEngineRef engine, CFStringRef peerID, CFSetRef viewNameSet, CFDataRef keyBag, CFMutableDictionaryRef newViewNameSet2ChangeTracker, CFMutableDictionaryRef  newPeerMap) {
    ...
    if (newEntry) {
        ...
        if (keyBag) {
            ...
            SOSChangeTrackerUpdatesChanges child = ^bool(SOSChangeTrackerRef ct, SOSEngineRef engine, SOSTransactionRef txn, SOSDataSourceTransactionSource source, SOSDataSourceTransactionPhase phase, CFArrayRef changes, CFErrorRef *error) {
                return SOSPeerDataSourceWillChange((SOSPeerRef)newEntry, SOSEngineGetDataSource(engine), source, changes, error);
            };
           
            SOSChangeTrackerRegisterChangeUpdate(ct, [child copy]);
        }
    }
}
...
static void SOSEngineReferenceSyncPeer(SOSEngineRef engine, CFStringRef peerID, CFSetRef viewNameSet, CFMutableDictionaryRef newViewNameSet2ChangeTracker, CFMutableDictionaryRef newPeerMap) {
    ...
    if (newEntry) {
        ...
        SOSChangeTrackerUpdatesManifests trackManifestCopy = trackManifest;
...
// Snags a read-only database connection
static bool SOSEngineDoReadOnlyTxnOnQueue(SOSEngineRef engine, CFErrorRef *error, void(^action)(SOSTransactionRef txn))
{
    return SOSDataSourceReadWithCommitQueue(engine->dataSource, error, ^(SOSTransactionRef txn) {
...
void SOSEngineDispose(SOSEngineRef engine) {
    IF_PLATFORM_DOES_NOT_SUPPORTS_SOS {
        return;
    }
...
void SOSEngineSetSyncCompleteListener(SOSEngineRef engine, SOSEnginePeerInSyncBlock notify_block) {
    ...
    SOSEngineDoOnQueue(engine, ^{
        engine->syncCompleteListener = [notify_block copy];
    });
}
...
static void SOSEngineCompletedSyncWithPeer(SOSEngineRef engine, SOSPeerRef peer) {
    ...
    if (block_to_call && engine->syncCompleteQueue) {
        ...
        SOSEnginePeerInSyncBlock btc_copy = [block_to_call copy];
...
CFDataRef SOSEngineCreateMessage_locked(SOSEngineRef engine, SOSTransactionRef txn, SOSPeerRef peer,
                                        CFMutableArrayRef *attributeList, CFErrorRef *error, SOSEnginePeerMessageCallBackInfo **sent) {
    IF_SOS_DISABLED_SERVER {
        SOSCCSetSOSDisabledError(error);
        return NULL;
    }
    ...
    if (result) {
        SOSEnginePeerMessageCallBackInfo* pmsc = [[SOSEnginePeerMessageCallBackInfo alloc] initWithEngine:engine peer:peer localManifest:local proposedManifest:proposed confirmedManifest:confirmed andMessage:message];

        SOSEnginePeerMessageCallBackInfo* pmsc2 = [pmsc copy];
        
        [pmsc setCallbackBlock:^(bool success) {
            // Have to copy pmsc so it'll still be around during the dispatch_async

            dispatch_async([pmsc2 engine]->queue, ^{
                if (success) {
...
SOSManifestRef SOSEngineCopyManifest(SOSEngineRef engine, CFErrorRef *error) {
    ...
    // This function will end up calling SOSDataSource functions, so it must have a database connection before the engine queue
    SOSEngineDoReadOnlyTxnOnQueue(engine, error, ^(SOSTransactionRef txn) {
...
CFDataRef SOSEngineCreateMessageToSyncToPeer(SOSEngineRef engine, CFStringRef peerID, CFMutableArrayRef *attributeList, SOSEnginePeerMessageCallBackInfo **sentCallback, CFErrorRef *error){
    IF_SOS_DISABLED_SERVER {
        SOSCCSetSOSDisabledError(error);
        return NULL;
    }
    __block CFDataRef message = NULL;
    __block SOSEnginePeerMessageCallBackInfo *localsentCallback = nil;
    SOSEngineForPeerID(engine, peerID, error, ^(SOSTransactionRef txn, SOSPeerRef peer) {
        message = SOSEngineCreateMessage_locked(engine, txn, peer, attributeList, error, &localsentCallback);
    });
    *sentCallback = [localsentCallback copy];
    return message;
}
...
- (id) initWithEngine:(SOSEngineRef) engine peer: (SOSPeerRef) peer localManifest: (SOSManifestRef) local proposedManifest: (SOSManifestRef) proposed confirmedManifest: (SOSManifestRef) confirmed andMessage: (SOSMessageRef) message {
    ...
    if ((self = [super init])) {
        _callbackBlock = nil;
        _engine    = CFRetainSafe(engine);
        _peer      = CFRetainSafe(peer);
        _local     = CFRetainSafe(local);
        _proposed  = CFRetainSafe(proposed);
        _confirmed = CFRetainSafe(confirmed);
        _message   = CFRetainSafe(message);
    }
    return self;
}
...
- (void) setCoder: (SOSCoderRef) coder {
    IF_PLATFORM_DOES_NOT_SUPPORTS_SOS {
        return;
    }
    _coder = CFRetainSafe(coder);
}
...
- (void) dealloc {
    ...
    if(_engine) CFReleaseNull(_engine);
    if(_peer) CFReleaseNull(_peer);
    if(_local) CFReleaseNull(_local);
    if(_proposed) CFReleaseNull(_proposed);
    if(_confirmed) CFReleaseNull(_confirmed);
    if(_message) CFReleaseNull(_message);
    CFReleaseSafe(_coder);
}
...
- (id) copyWithZone: (NSZone *) zone {
    ...
    SOSEnginePeerMessageCallBackInfo *theCopy = [SOSEnginePeerMessageCallBackInfo new];
    if(theCopy) {
        theCopy.engine = CFRetainSafe(_engine);
        theCopy.peer = CFRetainSafe(_peer);
        theCopy.local     = CFRetainSafe(_local);
        theCopy.proposed  = CFRetainSafe(_proposed);
        theCopy.confirmed = CFRetainSafe(_confirmed);
        theCopy.message   = CFRetainSafe(_message);
        [theCopy setCoder: _coder];
        [theCopy setCallbackBlock: _callbackBlock];
    }
    return theCopy;
}
