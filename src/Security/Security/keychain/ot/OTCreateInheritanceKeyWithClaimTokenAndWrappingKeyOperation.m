...
@property OTInheritanceKey* oldIK;
...
- (void)proceedWithKeys:(NSArray<CKKSKeychainBackedKeySet*>*)viewKeySets salt:(NSString*)salt
{
    ...
    NSError *error = nil;
    self.ik = [[OTInheritanceKey alloc] initWithUUID:self.uuid
                                      claimTokenData:self.claimTokenData
                                     wrappingKeyData:self.wrappingKeyData
                                               error:&error];
    if (self.ik == nil) {
        secerror("octagon: failed to create inheritance key (w/claimtoken+wrappingkey): %@", error);
        self.error = error;
    ...
    [self.deps.cuttlefishXPCWrapper createCustodianRecoveryKeyWithSpecificUser:self.deps.activeAccount
            ...
            if(error){
                secerror("octagon: Error creating inheritance w/claimtoken+wrappingkey key: %@", error);
                self.error = error;
                [self runBeforeGroupFinished:self.finishOp];
            } else {
                secnotice("octagon", "successfully created inheritance key w/claimtoken+wrappingkey");
