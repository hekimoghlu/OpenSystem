...
- (void)proceedWithKeys:(NSArray<CKKSKeychainBackedKeySet*>*)viewKeySets salt:(NSString*)salt
{
    ...
    NSError *error = nil;
    self.ik = [[OTInheritanceKey alloc] initWithUUID:self.uuid oldIK:self.oldIK error:&error];
    if (self.ik == nil) {
        secerror("octagon: failed to recreate inheritance key: %@", error);
        self.error = error;
    ...
    [self.deps.cuttlefishXPCWrapper createCustodianRecoveryKeyWithSpecificUser:self.deps.activeAccount
            ...
            if(error){
                secerror("octagon: Error recreatint inheritance key: %@", error);
                self.error = error;
                [self runBeforeGroupFinished:self.finishOp];
            } else {
                secnotice("octagon", "successfully recreated inheritance key");
